...
import com.topcoder.service.prerequisite.PrerequisiteServiceException;
...
/**
 * <p>
 * This class extends the HttpServlet class, and it will decode AJAXrequests from the JavaScript part into parameters
 * used to call API methods in the Prerequisite/Project/Studio services, which will in turn make calls to service
 * servers. They can be EJB clients to make calls. Returned values should be encoded into the AJAX response.
 * </p>
 * <p>
 * <b>Thread-safety:</b> Mutable and not thread-safe. But as its mutable variables are all initialized in the init
 * method and would never change afterwards. So this class can be used thread-safely by the container.
 * </p>
 *
...
public class AjaxBridgeServlet extends HttpServlet {
    ...
    /**
     * <p>
     * Represents the JSONEncoder to encoder the json object into string. Initialized in init method, and never changed
     * afterwards. Must be non-null after initialized. Used in doPost method.
     * </p>
     */
    ...
    /**
     * <p>
     * Represents the JSONDecoder to decode string into json object. Initialized in init method, and never changed
     * afterwards. Must be non-null after initialized. Used in doPost method.
     * </p>
     */
    ...
    /**
     * <p>
     * Represents the client to access Project Service. Initialized in init method, and never changed afterwards. Must
     * be non-null after initialized. Used in doPost method.
     * </p>
     */
    ...
    /**
     * <p>
     * Represents the client to access Prerequisite Service. Initialized in init method, and never changed afterwards.
     * Must be non-null after initialized. Used in doPost method.
     * </p>
     */
    ...
    /**
     * <p>
     * Represents the client to access Studio Service. Initialized in init method, and never changed afterwards. Must be
    ...
    /**
     * <p>
     * Represents the logger to do the logging. Initialized in init method, and never changed afterwards. Must be
     * non-null after initialized. Used in doPost method.
     * </p>
     */
    private Log logger;
    ...
    /**
     * <p>
     * Represents the configuration object used to create the MemoryFileUpload object. Initialized in init method, and
     * never changed afterwards. Must be non-null. (Though the ConfigurationObject is mutable, but it's never changed
     * after loaded.) Used in doPost method.
     * </p>
     */
    ...
    /**
     * <p>
     * Initializes the servlet. Gets the init parameters from web.xml and uses that to properly initialize the instance
     * variables. When and exception occurs while creating these objects, the exception is not thrown but only thrown
     * back to the requesting entity.
     * </p>
     *
     * @param config the servlet config object passed by the container for this servlet
     * @throws ServletException - if an exception occurs when initializing instance variables, or the configured value
     *             is invalid. All exceptions that occurred here in init are wrapped in a ServletException object since
     *             this is the only exception that can be thrown here.
     */
    @Override
     ...
     * </p>
     *
     * @param <T> type of the created service
     * @param clazz class of the desired type
     * @param service service object
     * @return service object
     * @throws Exception if the service is not the right type or any other error
     */
    ...
    /**
     * <p>
     * Handle the GET request. This method will simply call doPost(request, response).
     * </p>
     *
     * @param request the request object that comes from the web container
     * @param response the response object that comes from the web container
     * @throws ServletException this is for all the servlet related exceptions
     * @throws IOException this is for input / output errors when handling the servlet request
     */
    @Override
    ...
    /**
     * <p>
     * Handle the POST request. The request will contain parameters for "service" (corresponds to which service should
     * be called, its value can be: "project", "payment" and "studio"), "method" (corresponds to the method name of the
     * matched service. ), and some other parameters corresponding to the service method's arguments. The implementation
     * will call the matched service method with extracted parameters and get the returned value. Then it will convert
     * the returned value into json string and write to the response.
     * </p>
     *
     * @param request the request object that comes from the web container
     * @param response the response object that comes from the web container
        ...
        try {
            ...
            if ("project".equals(service)) {
                ...
                if ("createProject".equals(method)) {
                    // get the project parameter from request
                    ...
                    // do the actual creation of the Project
                    ProjectData respProj = projectService.createProject(project);
                    // now create the corresponding JSON object for the response
                ...
                } else if ("updateProject".equals(method)) { // for updateProject
                    // get the project parameter from request
                    String strProject = request.getParameter("project");
                    if (checkIfNullOrEmpty(strProject, "project", response)) {
                        return;
                    ...
                    // initialize the project using the JSON object
                    ProjectData project = getProjectFromJSON(jsonProj);
                    // do the actual updating of the Project
                ...
                } else if ("deleteProject".equals(method)) { // for deleteProject
                    // get the project parameter from request
                    String strProject = request.getParameter("project");
                    if (checkIfNullOrEmpty(strProject, "project", response)) {
                        return;
                    ...
                    // initialize the project using the JSON object
                    ProjectData project = getProjectFromJSON(jsonProj);
                    // do the actual retrieval of the Project
                ...
                } else if ("getProject".equals(method)) { // for getProject
                    // get the project parameter from request
                    ...
                    // send the JSONObject as a response
                    sendJSONObjectAsResponse(respJSON, response);
                } else if ("getProjectsForUser".equals(method)) { // for getProjectsForUser
                ...
                } else if ("getAllProjects".equals(method)) { // for getAllProjects
            ...
            } else if ("prerequisite".equals(service)) { // for Prerequisite SERVICES
                // *********************************************************************************************
                ...
                if ("createContest".equals(method)) {
                    // get the contest parameter from request
                    ...
                    studioService.createContest(contest, Long.parseLong(strProjectID));
                    printSuccessResponse(getSuccessJSONResponse(), response);
                    ...
                    // do the actual retrieval of the Contest
                    ContestData respContest = studioService.getContest(Long.parseLong(strContestID));
                    // now create the corresponding JSON object for the response
                    JSONObject respJSON = getJSONFromContest(respContest);
                    debug("getContest success!");
                    // send the JSONObject as a response
                    sendJSONObjectAsResponse(respJSON, response);

                } else if ("getContestsForClient".equals(method)) {
                    // get the clientID parameter from request
                    String strClientID = request.getParameter("clientID");
                    if (checkLongIfLessThanZero(strClientID, "clientID", response)) {
                        return;
                    }
                    // get the contestStatusID parameter from request
                    String strContestStatusID = request.getParameter("contestStatusID");
                    if (checkLongIfLessThanZero(strContestStatusID, "contestStatusID", response)) {
                        return;
                    }
                    // log the received ID
                    debug("received ID = [clientID]: " + strClientID + " [contestStatusID]: " + strContestStatusID);
                    // see http://forums.topcoder.com/?module=Thread&threadID=614386&start=0
                    // we just throw an exception to indicate that it is not supported for now
                    throw new StudioServiceException("It is not supported.");

                } else if ("getContestsForProject".equals(method)) {
                    // get the projectID parameter from request
                    ...
                    // verify here again if the form we received is multipart/form-data
                    if (isMultiPart) {
                    ...
                    // initialize the Document using the JSON object
                    UploadedDocument document = getDocumentUploadsFromJSON(jsonDocument);
                    // do the actual removal of document
                    ...
                    // do the actual update of submission
                    ...
                    ContestData contest = getContestFromJSON(jsonContest);
                    // do the actual update of the Contest
                    ...
                    // get the submissionID parameter from request
                    ...
                    // do the actual update of the Submission
                    ...
                    // iterate through contest statuses
            ...
            handleException(e, response);
        } catch (StudioServiceException e) {
            handleException(e, response);
        } catch (ProjectServiceFault e) {
            handleException(e, response);
        } catch (PrerequisiteServiceException e) {
            handleException(e, response);
            ...
            // see http://forums.topcoder.com/?module=Thread&threadID=614361&start=0
            handleException(e, response);
        }
     ...
     * </p>
     *
     * @param exception to be dealt
     * @param response http response
     * @throws IOException any io error happens
     ...
     * @throws ParseException when parsing the date gets an error
     * @throws JSONDecodingException when an exception occurs while decoding a json string
     ...
     * </p>
     *
     * @param prize the Prize object where the values will be coming from
     ...
     * </p>
     *
     * @param upDoc the UploadedDocument object where the values will be coming from
    ...
    private JSONObject getJSONFromUploadedDocument(UploadedDocument upDoc) {
        // initialize the JSON object using the UploadedDocument
        JSONObject respJSON = new JSONObject();
        respJSON.setLong("contestID", upDoc.getContestId());
        respJSON.setString("description", upDoc.getDescription());
        respJSON.setLong("documentID", upDoc.getDocumentId());
        respJSON.setString("fileName", ""); // set to blank so JS won't get error when parsed
    ...
    private ContestPayload getContestPayloadFromJSON(JSONObject jsonPayload) {
        // see http://forums.topcoder.com/?module=Thread&threadID=614371&start=0&mc=1#979330
        // no description and contest type id
     ...
     * </p>
     *
     * @param payload the ContestPayload object where the values will be coming from
    ...
    private JSONObject getJSONFromContestPayload(ContestPayload payload) {
        // initialize the JSON object using the ContestPayload
        JSONObject respJSON = new JSONObject();
        ...
        // TODO: no fields now and give constant values
        respJSON.setString("description", "description");
        respJSON.setLong("contestTypeID", -1);
    ...
    private JSONObject getJSONFromContestCategory(ContestCategoryData category) {
        // initialize the JSON object using the ContestCategory
        JSONObject respJSON = new JSONObject();
     ...
     * </p>
     *
     * @param contestStatus the Contest Status object where the values will be coming from
    ...
    private JSONObject getJSONFromContestStatus(ContestStatusData contestStatus) {
        // initialize the JSON object using the ContestStatus
        // see http://forums.topcoder.com/?module=Thread&threadID=614376&start=0 no displayIcon field
        JSONObject respJSON = new JSONObject();
        respJSON.setLong("statusID", contestStatus.getStatusId());
        respJSON.setString("name", contestStatus.getName());
        respJSON.setString("description", contestStatus.getDescription());
        // always give empty value for missing field
        respJSON.setString("displayIcon", "");
     ...
     * </p>
     *
     * @param contest the Contest object where the values will be coming from
    ...
    private JSONObject getJSONFromContest(ContestData contest) {
        ...
        // TODO: missing statusId, add a constant id now
        respJSON.setLong("statusID", -1);
    ...
    /**
     * <p>
     * Convenience method in getting a PrerequisiteDocument object from a JSONObject.
     * </p>
     *
     * @param json the JSONObject which will be used to construct the document
     * @return the document created from the json object
     * @throws ParseException if any parsing error
     * @throws IllegalArgumentException If any parameter is <code>null</code> or <code>value</code> is NaN, positive
     *             infinity, or negative infinity.
     * @throws JSONInvalidKeyException If there is no data for the given key.
     * @throws JSONDataAccessTypeException If the data associated with the key can not be retrieved as a long.
     ...
     * </p>
     *
     * @param project the Project object where the values will be coming from
    ...
    private JSONObject getJSONFromProject(ProjectData project) {
        ...
        respJSON.setString("description", project.getDescription());
     ...
     * @throws ParseException when an exception occurs while parsing the date string
     * @throws IllegalArgumentException If any parameter is <code>null</code> or <code>value</code> is NaN, positive
     *             infinity, or negative infinity.
     * @throws JSONInvalidKeyException If there is no data for the given key.
     * @throws JSONDataAccessTypeException If the data associated with the key can not be retrieved as a long.
     ...
     * </p>
     *
     * @param submission the Submission object where the values will be coming from
    ...
    private JSONObject getJSONFromSubmission(SubmissionData submission) {
        ...
        // although passedReview is not existing in current version of Submission,
    ...
    /**
     * <p>
     * Convenience method to get a JSON object from a PrerequisiteDocument instance.
     * </p>
     *
     * @param document prerequisite document
    ...
    /**
     * <p>
     * This is a helper method in getting the init parameters from the servlet config. When the init parameter is
     * null/empty and default value is null, throw an <code>IllegalArgumentException</code> or if the default value is
     * not null, return the default value. If the init parameter is neither null nor empty, return it.
     * </p>
     *
     * <p>
     * A null value in defaultValue parameter means that the init parameter is required.
     * </p>
     *
     * @param name the name of the init parameter to retrieve
     * @param defaultValue the default value to use in case the init parameter is not found
     * @return the value of the init parameter defined in web.xml
     * @throws IllegalArgumentException when a required parameter is not supplied
     */
    ...
    /**
     * <p>
     * Convenience method for sending the response of this servlet to the entity who initiated the request. This first
     * properly setups the response object before sending the response.
     * </p>
     *
     * @param s the response to be send to the requesting entity
     * @param response the response object to be operated on
     * @throws IOException when an exception occurs while sending the response to the requesting entity
     */
    ...
    /**
     * <p>
     * Convenience method for setting the Content Type of the response of this servlet. This also in turn calls
     * <code>setupAjaxCommonResponseHeaders()</code> to setup the common HTTP Headers needed for this response.
     * </p>
     *
     * @param response the response object to be operated on
     * @throws IOException when an exception occurs while setting the Content Type of the response
     */
    ...
    /**
     * <p>
     * Convenience method for setting the HTTP Headers so that the response from this servlet is not cached by the
     * receiving browser.
     * </p>
     *
     * @param response the response object where to set the HTTP Headers
     * @throws IOException when an exception occurs while setting the HTTP Headers
     */
     ...
     * @param customErrorMsg the error message in string format
     */
    ...
    /**
     * <p>
     * Convenience method to get Success <code>JSONObject</code> to return to as a response. This by default sets
     * "success" to true.
     * </p>
     *
    ...
    /**
     * <p>
     * Convenience method to get Error <code>JSONObject</code> to return to as a response. This by default sets
     * "success" to false.
     * </p>
     *
     * @param errorMsg the error message to be sent back to the user
     * @param response the HttpServletResponse object where to get the writer for the response
     * @throws IOException this exception is propagated from the other methods called inside this method
     * @throws JSONEncodingException when an error occurred while encoding the json object
     */
    ...
    /**
     * <p>
     * Convenience method to send a success JSONObject response to the requesting entity.
     * </p>
     *
     * @param respJSON the json object to be send as a response
     * @param response object where to get the print writer to send the response
     *
     * @throws IOException this exception is propagated from the other methods called inside this method
     * @throws JSONEncodingException when an error occurred while encoding the json object
     */
    ...
    /**
     * <p>
     * Convenience method to send a success JSONObject embedded with JSONArray response to the requesting entity.
     * </p>
     *
     * @param jsonArray the json array to be send as a response
     * @param response object where to get the print writer to send the response
     *
     * @throws IOException this exception is propagated from the other methods called inside this method
     * @throws JSONEncodingException when an error occurred while encoding the json object
    ...
    /**
     * <p>
     * Convenience method to send a success response to the requesting entity. Before the response is written, we first
     * log the Ajax response.
     * </p>
     *
     * @param succJson the json object to be send as a response
     * @param response object where to get the print writer to send the response
     *
     * @throws IOException this exception is thrown when an error occurs while writing the response
     * @throws JSONEncodingException when an error occurred while encoding the json object
     */
    ...
    /**
     * <p>
     * Checks if the parameter of long type passed is less than zero. This will be used to check for parameters that
    ...
    /**
     * <p>
     * Checks if the parameter of integer type passed is less than zero. This will be used to check for parameters that
     * needs to be greater than zero.
     * </p>
     *
     * @param param the parameter to be check
    ...
    /**
     * <p>
     * Checks if the parameter passed is less than zero. This will be used to check for parameters that needs to be
     * greater than zero.
     * </p>
     *
     * @param param the parameter to be check
     * @param name the name of parameter
     * @param isLongType true if it is long and false if it is integer
     * @param response where the response will be sent
     * @return true if it is null/empty, false if it is not null/empty
     * @throws JSONEncodingException when an error occurs in encoding the response
     * @throws IOException when an error occurs in sending the response
