...
js.topcoder.widgets.bridge.StudioService = function (/*String*/ servletUrlString) {
	...
	/**
	 * <p>
	 * Get the contest for client asynchronously, if the contests are retrieved successfully, onSuccess
	 * callback function will be called with the retrieved contests, otherwise onError will be called.</p>
	 *
	 * @throws IllegalArgumentException if any argument is null or clientID < 0
	 * @throws InvalidResponseException if the received response is invalid.
	 */
	this.getContestsForClient = getContestsForClient;
	function /* void */ getContestsForClient(/* long */ clientID, /* long */ contestStatusID, /* ContestsHandler */ onSuccess, /* ErrorHandler */ onError ) {
		// check first the validity of parameters
		// check clientID
		if (clientID < 0) {
			throw new js.topcoder.widgets.bridge.IllegalArgumentException("parameter.clientID","clientID should not be less than 0");
		}
		// check contestStatusID
		if (contestStatusID < 0) {
			throw new js.topcoder.widgets.bridge.IllegalArgumentException("parameter.contestStatusID","contestStatusID should not be less than 0");
		}
		// check onSuccess
		if (onSuccess == null) {
			throw new js.topcoder.widgets.bridge.IllegalArgumentException("parameter.onSuccess","onSuccess callback should not be null");
		...
		processor.request({
	    	...
	    	async: true,
	     	method: "POST",
	     	// the json string should be escaped properly here. 
	     	sendingText: "service=studio&method=getContestsForClient&clientID="+clientID+"&contestStatusID="+contestStatusID,
	     	onStateChange: function() {
	        	// Handle the response
	           	if (processor.getState() == 4 && processor.getStatus() == 200) {
	            	var response = processor.getResponseText();
	                ...
	                // check response
	                if (jsonResp == null) {
	                	throw new js.topcoder.widgets.bridge.InvalidResponseException("studio","getContestsForClient","Invalid response");
	                }
	                if (typeof(jsonResp.success) == "undefined") {
	                	throw new js.topcoder.widgets.bridge.InvalidResponseException("studio","getContestsForClient","Invalid response");
	                }	                
	...
	/**
	 * <p>
	 * Provides a list of contest types, along with the default, minimum, maximum, and other parameters, for
	 * each type of contest. Get contest categories asynchronously, if the contest categories are retrieved
	 * successfully, onSuccess callback function will be called with the retrieved contest categories, otherwise
	 * onError will be called.</p>
	 *
	 * @throws IllegalArgumentException if any argument is null or parameters is empty
	 * @throws InvalidResponseException if the received response is invalid.
	 */
	this.getContestCategories = getContestCategories;
	function /* void */ getContestCategories(/* String[] */ parameters, /* ContestCategoriesHandler */ onSuccess, /* ErrorHandler */ onError ) {
		// check first the validity of parameters
		// check parameters
		if (parameters == null || parameters.length == 0) {
			throw new js.topcoder.widgets.bridge.IllegalArgumentException("parameter.parameters","parameters should not be null or empty");
		}
		// check onSuccess
		if (onSuccess == null) {
			throw new js.topcoder.widgets.bridge.IllegalArgumentException("parameter.onSuccess","onSuccess callback should not be null");
		...
		processor.request({
	    	...
	    	async: true,
	     	method: "POST",
	     	// the json string should be escaped properly here. 
	     	sendingText: "service=studio&method=getContestCategories&parameters="+jsonParams,
	     	onStateChange: function() {
	        	// Handle the response
	           	if (processor.getState() == 4 && processor.getStatus() == 200) {
	            	var response = processor.getResponseText();
	                ...
	                // check response
	                if (jsonResp == null) {
	                	throw new js.topcoder.widgets.bridge.InvalidResponseException("studio","getContestCategories","Invalid response");
	                }
	                if (typeof(jsonResp.success) == "undefined") {
		               	throw new js.topcoder.widgets.bridge.InvalidResponseException("studio","getContestCategories","Invalid response");
		            }	                
	                ...
	                if (jsonResp.success == false) {
	                	...
	                	// get the array of JSON objects
	                	var categories = jsonResp.json;
	                	// convert the array of JSON to array of Contests Categories
	                	for(var x = 0; x < categories.length; x++) {
	                		var retCategory = new js.topcoder.widgets.bridge.ContestCategory(categories[x]);
	                		// set to new array
	                		newCategories[x] = retCategory;
	                	}
	                	// call the success callback passing the project
