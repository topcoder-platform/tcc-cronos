...
/**
 * <p>
 * An implementation of <code>RequestParser</code> interface to parse the uploaded files and parameters
 * from the servlet request using the HTTP 1.1 standard. For details of the protocol please refer to RFC
 * 1867.
 * </p>
 *
 * <p>
 * The usage of this class should follow that defined in the interface. This parser can be reused (for
 * parsing another request), but this is not recommended. Note that if any exception is caught, make sure
 * the underlying input stream is closed in any case.
 * </p>
 *
 * <p>
 ...
 * <li> The hasNextFile() method accounts for new getUploadedFileInputStream(long fileLimit) method.</li>
 * </ul>
 * </p>
 *
 * <p>
 * <b>Thread Safety: </b> This class is not thread-safe. In fact it requires a sequence of method calls to
 * work properly. But the <code>FileUpload</code> and <code>StreamFileUpload</code> class will use it in
...
public class HttpRequestParser implements RequestParser {
    ...
    /**
     * <p>
     * Represents the method getUploadedFileInputStream() has been called.
     * </p>
     *
     * @since 2.2
     */
    private static final int GET_UPLOADED_FILEINPUTSTREAM_CALLED = 4;
    ...
    /**
     * <p>
     * Represents the form file name of the last parsed file. It is initially set to null. It is reset in
     * the parseRequest() method, updated in the hasNextFile() method and accessed in the getFormFileName()
     * method.
     * </p>
     */
    ...
    /**
     * <p>
     * Represents the remote file name of the last parsed file. It is initially set to null. It is reset in
     * the parseRequest() method, updated in the hasNextFile() method and accessed in the
     ...
     * parseRequest() method, updated in the hasNextFile() method and accessed in the getContentType()
     * method.
     * </p>
     */
    ...
    /**
     * <p>
     * Represents the parameters collected in the request during the parsing process. The key should be
     * parameter name (String), the value should be a list (List) of parameter values (String). It is
     * initially set to an empty map and the reference does not change. It is reset in the parseRequest()
     * method, (the elements) updated in the hasNextFile() method and accessed in the getParameters()
     * method.
     * </p>
     */
    ...
    /**
     * <p>
     * Represents the input stream pointing to data submitted in the request. It is initially set to null.
     * It is set in the parseRequest() method and accessed in the hasNextFile() and writeNextFile() methods.
     * </p>
     */
     ...
     * 1.1). It is initially set to null. It is set in the parseRequest() method and accessed in the
     * hasNextFile() and writeNextFile() methods.
     * </p>
     */
     ...
     * method parseRequest(ServletRequest) has been called; 1 means the method hasNextFile() has been called
     * and the return value is true; 2 means the method writeNextFile(OutputStream, long) has been called; 3
     * means the end of the input stream is reached, no operations should be done any more.
     * </p>
     */
     ...
     * </p>
     *
     * @return a String containing the next line of input from the stream, or null to indicate the end of
     *         the stream.
     *
     * @throws IOException
     ...
     *             if the contentType is not multipart/form-data.
     */
    private String getContentType(ServletRequest request) throws InvalidContentTypeException {
        ...
        if (!requestContentType.toLowerCase().startsWith("multipart/form-data")) {
            throw new InvalidContentTypeException("contentType " + requestContentType
     ...
     *
     * @throws InvalidContentTypeException
     *             if the boundary can not be properly got.
     */
     ...
     *
     * @param line
     *            the given line to retrieve the form file name.
     * @param lowerCaseLine
     *            the lower case representation of the line.
     *
     * @throws RequestParsingException
     *             if the form file name can not be retrieved or it is an empty string.
     */
     ...
     * methods. The writeNextFile() method should then be called to store the file contents.
     * </p>
     *
     * <p>
     * Note that this method can be called after either parseRequest() or writeNextFile() or after the
     * getUploadedFileInputStream() returned a stream an that stream was read to the end.
     * </p>
     *
     * <p>
     * <b>Version 2.2 Changes:</b><br>
     * The method accounts for new getUploadedFileInputStream() method (it can be called prior to this
     * method, see Exceptions handling).
     * </p>
     *
     * <p>
     ...
     *             if this method is not called after parseRequest() or writeNextFile() or
     *             getUploadedFileInputStream() has not been called.
     */
    public boolean hasNextFile() throws RequestParsingException {
        if ((lastOperation != PARSER_EQUEST_CALLED) && (lastOperation != WRITE_NEXT_FILE_CALLED)
                && (lastOperation != GET_UPLOADED_FILEINPUTSTREAM_CALLED)) {
            this.closeInputStream();
            throw new IllegalStateException("This method can be called only after parseRequest()"
                    + " or writeNextFile() or getUploadedFileInputStream().");
        }
                ...
                while (((line = readTextLine()) != null) && (line.length() > 0)) {
                    ...
                    if (lowerCaseLine.startsWith("content-disposition:")) {
                        ...
                        if ((start != -1) && (end != -1)) {
                            ...
                            // Extract just file name, not directory name(consider both Windows and
                            // UNIX style slashes)
                    ...
                    if (lowerCaseLine.startsWith("content-type:")) {
                        ...
                        // that's what should be between 'content-type' and actual spec if
                        // everything is OK
                        if (start == -1) {
    ...
    /**
     * <p>
     * Reads the content value from the input stream of the request. The content can be either the parameter
     * value of the file content value.
     * </p>
     *
     * @param out
     ...
     */
    private void readContentValue(OutputStream out, long fileLimit) throws IOException,
        ...
        while (true) {
            ...
            if (!reachedBoundary) {
                ...
                // Copy last two chars back to beginning of buffer - haven't reached boundary, so
                // they are not '\r\n'
     ...
     *             if the uploaded file is too large.
     * @throws IllegalStateException
     *             if this method is not called after hasNextFile() has been called and returns true.
     */
     ...
     * @throws RequestParsingException
     *             if any error occurs during getting the file input stream(Actually never thrown in this
     *             implementation).
     * @since 2.2
     */
    public UploadedFileInputStream getUploadedFileInputStream(long fileLimit) throws RequestParsingException {
        ...
        HttpUploadedFileInputStream stream;
        try {
            stream = new HttpUploadedFileInputStream(fileLimit, in, boundary);
        } catch (IOException e) {
            this.closeInputStream();
            throw new RequestParsingException(
                    "IOException occurs while creating the HttpUploadedFileInputStream instance.");
        }

        this.lastOperation = GET_UPLOADED_FILEINPUTSTREAM_CALLED;
        return stream;
    }
