...
/**
 * <p>
 * This implementation of the ScreeningTaskDAO provides an Informix database as
 * the backing datastore.
 * </p>
 * <p>
 * Thread-Safety: Each invocation works on its own connection instance, and the
 * connection is closed after. Multiple concurrent reads and writes are expected
 * on the datastore.
 * </p>
 * 
 * @author TCSDEVELOPER
...
public class MockTaskDAO implements ScreeningTaskDAO {
	 ...
	 * executing the sql statement to load screening tasks.
	 */
	private static final DataType[] LOAD_SCREENING_TASKS_COLUMN_TYPES = new DataType[] {
			...
			DatabaseUtils.STRING_TYPE, DatabaseUtils.STRING_TYPE,
			DatabaseUtils.LONG_TYPE, DatabaseUtils.LONG_TYPE,
			DatabaseUtils.DATE_TYPE, DatabaseUtils.STRING_TYPE,
			DatabaseUtils.DATE_TYPE, DatabaseUtils.STRING_TYPE,
	...
	/**
	 * Represents the argument types for the sql statement to update screening
	 * task.
	 */
	...
	/**
	 * <p>
	 * An instance of UserRetrieval that is used to retrieve submitter
	 * information.
	 * </p>
	 */
	 ...
	 * @param userRetrieval
	 *            An instance of UserRetrieval that is used to retrieve
	 *            submitter information.
	 * @throws IllegalArgumentException
	 *             if the connectionFactory or userRetrieval is null.
	 */
	public MockTaskDAO(DBConnectionFactory connectionFactory,
			UserRetrieval userRetrieval) {
		this(connectionFactory, null, userRetrieval);
	}
	 ...
	 *            submitter information.
	 * @throws IllegalArgumentException
	 *             if the connectionFactory or userRetrieval is null or if
	 *             connectionName is an empty String.
	 */
	public MockTaskDAO(DBConnectionFactory connectionFactory,
			String connectionName, UserRetrieval userRetrieval) {
		...
		if (userRetrieval == null) {
			throw new IllegalArgumentException(
					"userRetrieval should not be null.");
		}
	 ...
	 */
	public ScreeningTask[] loadScreeningTasks(Long screenerId,
			ScreeningStatus status) throws DAOException {
		if (screenerId != null && screenerId.longValue() <= 0) {
			throw new IllegalArgumentException(
					"screenerId should be > 0, if it's not null.");
		}
		...
		Object[][] rows = DatabaseUtils.doQuery(connectionFactory,
				connectionName, LOAD_SCREENING_TASKS_SQL + whereCondition,
	...
	/**
	 * Builds the WHERE condition string for the load operation of screening
	 * tasks.
	 * 
	 * @param screenerId
	 *            The screenerId of the tasks to retrieve (may be null to
	 ...
	 */
	private String buildWhereConditionForLoad(Long screenerId,
			ScreeningStatus status) {

		StringBuffer whereCondition = new StringBuffer();
		if (screenerId != null) {
			whereCondition.append(" AND screening_task.screener_id="
					+ screenerId);
		}
		if (status != null) {
			whereCondition.append(" AND screening_task.screening_status_id="
					+ status.getId());
		}
	...
	/**
	 * Loads data items from the data row sequentially and fills them into an
	 * instance of ScreeningData.
	 * 
	 * @param screeningData
	 ...
	 * @return the start index of the remaining data items that haven't been
	 *         read
	 * @throws DAOException
	 *             if a problem occurs while accessing the datastore or setting
	 *             data to the screeningData.
	 */
	private int loadScreeningDataSequentially(ScreeningData screeningData,
			Object[] row, int startIndex) throws DAOException {

		// unwrapLongObject are used blindly although most of them are not null
		long projectId = DatabaseUtils
				.unwrapLongOjbect((Long) row[startIndex++]);
		long projectCategoryId = DatabaseUtils
				.unwrapLongOjbect((Long) row[startIndex++]);
		long uploadId = DatabaseUtils
				.unwrapLongOjbect((Long) row[startIndex++]);
		...
		try {
			...
			throw new DAOException("Unable to parse external user id ["
					+ externalUserIDString + "] to long value.", e);
		}

		// get information about external user
		ExternalUser externalUser;
		try {
			externalUser = userRetrieval.retrieveUser(externalUserID);
		} catch (Throwable e) {
			// since enternalUserID maybe negative or 0, thus
			// IllegalArgumentException be thrown from retrieveUser(String). As
			// a result we use catch Throwable to catch all such unchecked
			// exceptions as well.
			throw new DAOException(
					"Unable to get external user information for user id ["
							+ externalUserID + "].", e);
		}
		if (externalUser == null) {
			throw new DAOException(
					"There is no personal information for user id ["
							+ externalUserID + "].");
		}
		...
		try {
			...
			screeningData.setSubmitterHandle(externalUser.getHandle());
			screeningData.setSubmitterFirstName(externalUser.getFirstName());
			screeningData.setSubmitterLastName(externalUser.getLastName());
			screeningData.setSubmitterEmail(externalUser.getEmail());
			screeningData.setSubmitterAlternativeEmails(externalUser
					.getAlternativeEmails());
		} catch (IllegalArgumentException e) {
			throw new DAOException(
					"Error occurs when setting data to screeningData.", e);
		}
		return startIndex;
	}
	 ...
	 * @throws DAOException
	 *             if a problem occurs while accessing the datastore or setting
	 *             data to the screeningTask.
	 */
	private int loadScreeningTaskSequentially(ScreeningTask screeningTask,
			Object[] row, int startIndex) throws DAOException {

		// get screening data
		ScreeningData screeningData = new ScreeningData();
		startIndex = loadScreeningDataSequentially(screeningData, row,
				startIndex);
		...
		ScreeningStatus screeningStatus = (ScreeningStatus) Enum
				.getEnumByStringValue(statusName, ScreeningStatus.class);
		// screeningStatus maybe null, but we catch this by the later
		// setScreeningStatus setter

		// unwrapLongObject are used blindly although most of them are not null
		long screeningTaskId = DatabaseUtils
				.unwrapLongOjbect((Long) row[startIndex++]);
		long screenerId = DatabaseUtils
				.unwrapLongOjbect((Long) row[startIndex++]);
		...
		try {
			...
			throw new DAOException(
					"Error occurs when setting data to screeningTask.", e);
		}
		return startIndex;
	}
	 ...
	 * @throws IllegalArgumentException
	 *             if the task is null, or it does not contains screeningStatus
	 *             instance
	 * @throws UpdateFailedException
	 *             if a problem occurs due to a simultaneous update by a
	 *             different screener.
	 * @throws DAOException
	 *             if a problem occurs while accessing the datastore, or the
	 *             screening status of the task has not been specified.
	 */
	public void updateScreeningTask(ScreeningTask screeningTask)
			throws DAOException {
		if (screeningTask == null) {
			throw new IllegalArgumentException(
					"screeningTask should not be null.");
		}

		// check if the screening status of the task has been specified.
		if (screeningTask.getScreeningStatus() == null) {
			throw new DAOException(
					"Screening status of screeningTask has not been specified.");
		}
		...
		Object[] queryArgs = new Object[] {
				// status id
				DatabaseUtils.wrapLongValue(screeningTask.getScreeningStatus()
						.getId()),
				// screener id
				...
				screeningTask.getCreationUser(),
				screeningTask.getCreationDate(),
				screeningTask.getModificationUser(),
				screeningTask.getModificationDate(),
				// task id
				DatabaseUtils.wrapLongValue(screeningTask.getId()) };

		// execute the update statement
		int updateCount = DatabaseUtils.doDMLQuery(connectionFactory,
				connectionName, UPDATE_SCREENING_TASK_SQL + whereCondition,
				UPDATE_SCREENING_TASK_ARGUMENT_TYPES, queryArgs);
	...
	private String buildWhereConditionForUpdate(ScreeningTask screeningTask) {
		...
		if (screeningTask.getScreeningStatus() == ScreeningStatus.SCREENING) {
			return " AND screening_status_id="
					+ ScreeningStatus.PENDING.getId();
		} else {
			return " AND screening_status_id="
					+ ScreeningStatus.SCREENING.getId();
		}
	}
}
