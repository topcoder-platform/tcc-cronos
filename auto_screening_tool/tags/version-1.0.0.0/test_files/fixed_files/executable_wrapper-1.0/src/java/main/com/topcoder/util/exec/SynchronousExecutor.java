/*
 * SynchronousExecutor.java Copyright ? 2002, TopCoder, Inc. All rights reserved
 */
...
/**
 * <p>
 * An Executor to execute command synchronously.
 * </p>
 * @author srowen
 * @author garyk
 * @version 1.0
...
class SynchronousExecutor implements Executor {
    ...
    /**
     * <p>
     * Note that this constructor should process the given command using
     * PlatformSupport to create a command appropriate for execution on the
     * current operating system.
     * </p>
     * @param parameters
     *            the parameters of the command for the execution
     */
    SynchronousExecutor(final ExecutionParameters parameters) {
        ...
        shellCommand = platformSupport.makeShellCommand(parameters.getCommand());
        ...
        if ((envpMap = parameters.getEnvironment()) != null) {
            ...
            for (int i = 0; it.hasNext(); i++) {
                ...
                environment[i] = envpEntry.getKey() + "=" + envpEntry.getValue();
            }
        }
    }
    ...
    /**
     * <p>
     * This method invokes the command using java.lang.Runtime, then creates and
     * starts two StreamToStringThread threads to consume its standard output
     * and error stream, and then waits for the Process's completion.
     * </p>
     * <p>
     * Watch out for the interaction between this and the halt() method. It
     * could be called before this method is called; it could be called after
     * execute() but before it waits for the Process to finish; it could be
     * called while this method waits for the Process to finish. Make sure that
     * all these are handled, and that in all cases, halt() causes this method
     * to throw an ExecutionHaltedException.
     * </p>
     * @return the execution result
     * @throws ExecutionException
     *             if any Exception is thrown during execution of the underlying
     *             Process
     * @throws ExecutionHaltedException
     *             if halt() is called before the underlying Process completes
     */
    public ExecutionResult execute() throws ExecutionException {
        ...
        try {
             ...
             */
            process = Runtime.getRuntime().exec(shellCommand, environment, workingDirectory);

            /*
             * create the standard output and error output StreamToStringThreads
             * with their associated input streams.
             */
             ...
             * the thread that encapsulates this SynchronousExecutor and is
             * waiting for the process of the execution to start
             */
        ...
        /*
         * obtain the standard output and error output string from their threads
         */
        output = outThread.getOutput();
        errorOutput = errorThread.getOutput();

        // /////////////////////////////////////
        // This following piece of code is quite strange. This makes the exitValue and
        // errorOutpus useless. So it's commented.

        /*
         * throw an ExecutionHaltedException if the exitValue is not 0, or the
         * length of the errorOutput is not 0, since in Solaris if the command
         * doesn't exist, the exit value is still 0, so we need to check whether
         * there are errors through the errorOutput
         */
    ...
    /**
     * <p>
     * Requests that the command that it is executing is halted immediately.
     * </p>
     */
    public void halt() {
