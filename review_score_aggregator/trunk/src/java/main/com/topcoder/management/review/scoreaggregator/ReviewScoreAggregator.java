...
public class ReviewScoreAggregator {
     ...
     */
    public synchronized AggregatedSubmission[] aggregateScores(float[][] scores) throws InconsistentDataException {
        ...
        for (int i = 0; i < scores.length; ++i) {
            ...
            for (int j = 0; j < scores[i].length; ++j) {
                Util.checkFloatValue(scores[i][j], "scores[" + i + "][" + j + "]");
            }
        }
     ...
     *             negative/NaN/Infinite
     */
    public synchronized RankedSubmission[] calcPlacements(float[][] scores) throws InconsistentDataException {
