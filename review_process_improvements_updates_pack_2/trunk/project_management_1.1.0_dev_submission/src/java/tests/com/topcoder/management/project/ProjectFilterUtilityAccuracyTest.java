...
public class ProjectFilterUtilityAccuracyTest extends TestCase {
    ...
    /**
     * Accuracy test of <code>buildProjectTypeReviewSystemVersion(long version)</code> method.
     *
     * @throws Exception throw exception to JUnit.
     */
    public void testBuildProjectTypeReviewSystemVersionAccuracy() throws Exception {
        EqualToFilter filter = (EqualToFilter) ProjectFilterUtility.buildProjectTypeReviewSystemVersion(1);
        assertEquals("name is incorrect.", ProjectFilterUtility.PROJECT_TYPE_REVIEW_SYS_VER, filter.getName());
        assertEquals("value is incorrect.", new Long(1), filter.getValue());
    }
