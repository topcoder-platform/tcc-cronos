...
/**
 * <p>
 * Unit test for the new APIs in <code>InformixProjectPersistence</code>
 * version 1.1 and some methods in
 * <code>AbstractInformixProjectPersistence</code>.
 * </p>
 * <p>
 * Because the added methods in the class are protected, so the subclass of
 * <code>InformixProjectPersistence</code> is used for testing which make the
 * protected methods public.
 * </p>
 * <p>
 * The <code>getConnectionName()</code> and
 * <code>getConnectionFactory()</code> of
 * <code>AbstractInformixProjectPersistence</code> are also tested in this
 * class.
 * </p>
 * @author TCSDEVELOPER
 * @version 1.1
...
public class InformixProjectPersistenceV11Test extends TestCase {
    ...
    /**
     * Represents the <code>InformixProjectPersistence</code> instance which is used to
     * test the functionality of <code>InformixProjectPersistence</code>.
     */
     ...
     * created and the auto commit property is set false.
     * </p>
     * @throws Exception if there is any problem.
     */
    public void testOpenConnectionAccuracyConnNameExists() throws Exception {
        ...
        try {
            ...
            assertFalse("Fails to initialize the connection.", conn
                    .getAutoCommit());
        } finally {
            if (conn != null) {
                conn.close();
    ...
    public void testCloseConnectionAccuracy() throws Exception {
        ...
        try {
            ...
            Helper.doDMLQuery(conn, "INSERT INTO project_info_type_lu "
                    ...
                    + "'topcoder', CURRENT, 'topcoder', CURRENT, 1)",
                    new Object[] {});
            ...
            assertEquals("Fails to commit the changes in database.",
                    "property 1", rows[0][0]);
            Helper
                    .doDMLQuery(
                            conn,
                            "DELETE FROM project_info_type_lu WHERE project_info_type_id = 1",
                            new Object[] {});
            conn.close();
        } finally {
     ...
     * <p>
     * Verifies that the <code>PersistenceException</code> will be thrown if
     * fails to commit the connection.
     * </p>
     * @throws Exception if there is any problem.
     */
        ...
        try {
            ...
            Object[][] rows = Helper
                    .doQuery(
                            ...
                            new Object[] {},
                            new DataType[] {Helper.STRING_TYPE});
            assertEquals("Fails to commit the changes in database.", 0,
                    rows.length);
            conn.close();
        } finally {
            // try to close the connection if anything unexpected occurs.
     ...
     * <p>
     * Verifies that the <code>PersistenceException</code> will be thrown if
     * fails to rollback the connection.
     * </p>
     * @throws Exception if there is any problem.
     */
    ...
    /**
     * <p>
     * Accuracy test for method <code>getConnectionName()</code> in
     * <code>AbstractInformixProjectPersistence</code>.
     * </p>
     * <p>
    ...
    public void testGetConnectionNameConnNameExist() {
        assertEquals("Fails to get the connection name.",
                "informix_connection", persistence.getConnectionName());
    }
     ...
     * </p>
     * <p>
     * Verifies that <code>null</code> will be retrieved if it is not
     * configured.
     * </p>
     * @throws Exception if there is any problem.
     */
    public void testGetConnectionNameConnNameDoesNotExist() throws Exception {
        persistence = new InformixProjectPersistence(
                "InformixProjectPersistence.CustomNamespace.NoConnectionName");
        assertNull("Fails to get the connection name.", persistence
                .getConnectionName());
    }

    /**
     * <p>
     * Accuracy test for method <code>getConnectionFactory()</code> in
     * <code>AbstractInformixProjectPersistence</code>.
     * </p>
     * <p>
    ...
    public void testGetConnectionFactory() {
        assertNotNull("Fails to get the connection factory.", persistence
