...
public class XMLCOOReportSerializer extends BaseCOOReportSerializer {
    ...
    public byte[] serializeCOOReportToByteArray(COOReport report)
        ...
        List<Component> dependecyComponents;
        try {
            ...
            dependecyComponents = new ArrayList<Component>();
            for (ComponentDependency dep : dependencies) {
                dependecyComponents.add(dep.getComponent());
            }
            ...
            for (int i = 0; i < nodes.length; i++) {
                ...
                if (nodes[i] instanceof Field) {
                    ...
                    processLoop(field, report, dependencies);
                }
            }
            // Set it back to null
            Helper.setBackValue(dependecyComponents);
            // return the result
            ...
            throw Helper.logError(getLogger(), new COOReportSerializerException("invalid tempalte format.", e));
        } finally {
    ...
    private void processField(Field field, COOReport report) {
        if (field.getName().equals("projectId")) {
            ...
            field.setValue(report.getContestData().getContestEndDate().toString());
        } else if (field.getName().equals("designWinner")) {
            field.setValue(report.getContestData().getDesignWinner());
        } else if (field.getName().equals("designSecondPlace")) {
            field.setValue(report.getContestData().getDesignSecondPlace());
        } else if (field.getName().equals("designReviewer1")) {
            field.setValue(report.getContestData().getDesignReviewers().get(0));
        } else if (field.getName().equals("designReviewer2")) {
            field.setValue(report.getContestData().getDesignReviewers().get(1));
        } else if (field.getName().equals("designReviewer3")) {
            field.setValue(report.getContestData().getDesignReviewers().get(2));
        } else if (field.getName().equals("developmentWinner")) {
            field.setValue(report.getContestData().getDevelopmentWinner());
        } else if (field.getName().equals("developmentSecondPlace")) {
            field.setValue(report.getContestData().getDevelopmentSecondPlace());
        } else if (field.getName().equals("developmentReviewer1")) {
            field.setValue(report.getContestData().getDevelopmentReviewers().get(0));
        } else if (field.getName().equals("developmentReviewer2")) {
            field.setValue(report.getContestData().getDevelopmentReviewers().get(1));
        } else if (field.getName().equals("developmentReviewer3")) {
            field.setValue(report.getContestData().getDevelopmentReviewers().get(2));
        }
     ...
     */
    private void processLoop(Loop field, COOReport report,
