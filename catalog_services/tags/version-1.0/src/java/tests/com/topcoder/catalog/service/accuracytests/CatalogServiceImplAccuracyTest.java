...
public class CatalogServiceImplAccuracyTest extends CatalogServiceImplBaseTest {
    ...
    public void testFindAssetAccuracy() throws Exception {
        ...
        List<Category> categories = Arrays.asList(getEntityManager().find(Category.class, 1L), getEntityManager()
                .find(Category.class, 3L));
        ...
        for (int i = 1; i <= 0x1F; i++) {
            ...
            String pName = (i & 0x02) != 0 ? name : (i & 0x01) == 0 ? " " : null;
            String pDescription = (i & 0x01) != 0 ? descriptions[(i / 2) % 3] : (i & 0x02) == 0 ? " " : null;
            SearchCriteria criteria = new SearchCriteria(pUserId, pClientId, pCategories, pName, pDescription);
            boolean current = i % 2 == 1;
            List<AssetDTO> assets = getRemote().findAssets(criteria, current);
