...
public class TestHelper {
    /**
     * <p>This constant indicates that tests are being executed against mockups.</p> <p>If this constant has value
     * <code>false</code> then {@link #getCatalogService} ()} returns looked up remote bean from JNDI.</p>
     */
    static final boolean EXECUTE_AGAINST_MOCKS =
        Boolean.valueOf(System.getProperty("test.against.mocks", "true"));
    /**
     * <p><code>CatalogService</code> JNDI name.</p>
     */
    ...
    /**
     * <p>Returns <code>CatalogService</code>'s mockup, if
     * {@link #EXECUTE_AGAINST_MOCKS} or <code>failTest</code> is set to true. Does nothing otherwise.</p>
     * @return if {@link #EXECUTE_AGAINST_MOCKS} is true or <code>failTest</code> is true
     *         then local <code>MockupCatalogService</code> instance,
     *         else looked up CatalogService (deployed in an EAR into JBoss).
     * @param failTest set <code>true</code> to inject <code>FailTestEntityManager</code>,
    ...
    public static CatalogService getCatalogService(boolean failTest) {
        if (EXECUTE_AGAINST_MOCKS || failTest) {
            return new MockupCatalogService(new CatalogServiceImpl(), failTest);
        } else {
            try {
