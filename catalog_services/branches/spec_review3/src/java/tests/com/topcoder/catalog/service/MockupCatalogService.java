...
public class MockupCatalogService implements CatalogService, Serializable {
    ...
    public MockupCatalogService(CatalogServiceImpl catalogService, boolean failTest) {
        ...
        try {
            ...
            field.set(catalogService, failTest ? new FailTestEntityManager() : TestHelper
                    .getEntityManager());
        } catch (NoSuchFieldException e) {
            throw new RuntimeException("Invalid implementation of CatalogService: "
                    + e.getMessage(), e);
    ...
    public AssetDTO getAssetById(long id, boolean currentVersion) throws EntityNotFoundException,
        ...
        tx.begin();
        try {
            AssetDTO res = catalogService.getAssetById(id, currentVersion);
            tx.commit();
            return res;
        } catch (RuntimeException e) {
            throw new EJBException(e);
     ...
     * {@inheritDoc}
     */
    public AssetDTO getAssetByVersionId(long id) throws EntityNotFoundException,
        PersistenceException {
        try {
     ...
     * {@inheritDoc}
     */
    public AssetDTO createVersion(AssetDTO asset) throws EntityNotFoundException,
        PersistenceException {
        final EntityTransaction tx = getEntityTransaction();
        final AssetDTO dto;
        tx.begin();
     ...
     * {@inheritDoc}
     */
    public void assignUserToAsset(long userId, long componentId) throws EntityNotFoundException,
        PersistenceException {
        final EntityTransaction tx = getEntityTransaction();
        tx.begin();
        try {
