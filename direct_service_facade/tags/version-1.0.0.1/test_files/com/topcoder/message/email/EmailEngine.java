...
/**
 * <p>
 * The EmailEngine class is responsible for sending email.
 * </p>
 * 
 * <p>
 * Version 3.0 has added support for priority messages via message headers.
 * </p>
 * <p>
 * In version 3.1, the send methods are updated to handle the content type property from TCSEmailMethod, and an
 * explicit default constructor is added to be consistent with current TC standards. The changes have no effect on the
 * thread safety of any method.
 * </p>
 * 
 * @author sord
 * @author BEHiker57W
 ...
 * 
 *           $Log: EmailEngine.java,v $ Revision 1.3 2005/10/04 19:54:19 ivern no message
 * 
 *           Revision 1.2 2005/10/04 14:38:57 mgmg *** empty log message ***
 * 
 * 
 *           Revision 1.1.1.1 2004/03/23 23:03:25 gpaul no message
 * 
 *           Revision 1.3 2003/04/01 18:58:08 marcg Updates for 2.0
 * 
 *           Revision 1.1.1.1 2003/01/09 22:31:19 marcg no message
 * 
 *           Revision 1.12 2002/11/28 15:30 yellow_gecko added attachment functionality
 * 
 *           Revision 1.1.1.1 2002/10/28 17:24:47 marcg no message
 * 
 *           Revision 1.11 2002/10/08 16:19:12 marcg removed reference to BasicConfigurator
 * 
 *           Revision 1.10 2002/09/30 16:11:58 seaniswise added javadocs
 * 
 *           Revision 1.9 2002/09/30 15:07:57 seaniswise no message
 * 
 *           Revision 1.8 2002/09/29 19:28:12 marcg changed
 * 
 *           Revision 1.7 2002/09/27 23:40:44 marcg updated use of configmanager
 * 
 *           Revision 1.6 2002/08/08 16:40:08 marcg minor updates
 * 
 *           Revision 1.5 2002/08/08 01:16:41 marcg Added ConfigManager Interface and corresponding methods.
 * 
 *           Revision 1.4 2002/06/27 14:32:04 marcg author update
 * 
 *           Revision 1.3 2002/06/13 20:46:31 marcg Modified class to use the Configuration Manager component. Set
 *           default SMTP IP to 127.0.0.1, not Topcoders ip address Added configuration for smtp username/password
 *           instead of always passing empty strings.
 * 
 *           Revision 1.2 2002/05/21 21:51:59 tcs change package name
 * 
 *           Revision 1.1 2002/05/10 13:14:06 tcs no message
 * 
 *           Revision 1.1.2.4 2002/05/01 05:48:52 apps SB
 * 
 *           Revision 1.1.2.4 2002/04/30 23:58:57 chuck fixed line that calls send to use configurable host and port
 *           (not constants SMTP_HOST_PORT and SMPT_HOST_ADDR)
 * 
 *           Revision 1.1.2.3 2002/04/27 01:45:57 sord Added test for at least on TO address.
 * 
 *           Revision 1.1.2.2 2002/04/17 12:43:17 apps Fixed some compiler issues.
 * 
 *           Revision 1.1.2.1 2002/04/17 12:34:11 apps Initial implementation.
 */
public class EmailEngine implements ConfigManagerInterface {

    /**
     * AttachmentDataSource class implements DataSource to be wrapped with a DataHandler for the creation of
     * MimeBodyPart instances to add into the Multipart instance that make up the body of the MimeMessage.
     * 
     *@author yellow_gecko
     *@created November 28, 2002
     *@version $Revision: 124213 $
     *@internal Log of Changes: $Log: EmailEngine.java,v $ Revision 1.3 2005/10/04 19:54:19 ivern no message
     * 
     *           Revision 1.2 2005/10/04 14:38:57 mgmg *** empty log message ***
     * 
     *           Revision 1.1.1.1 2004/03/23 23:03:25 gpaul no message
     ...
     *           Revision 1.1.1.1 2003/01/09 22:31:19 marcg no message
     * 
     *           Revision 1.1 2002/11/28 15:30 yellow_gecko initial implementation
     */
    public static class AttachmentDataSource implements DataSource {
        ...
        /**
         * The constuctor extracts the raw bytes from the given <code>InputStream</code> and finds the MIME type of
         * the attachment.
         * 
         *@param inputStream
         *            the <code>InputStream</code> from which the attachment is read
         *@param name
         *            the name of the attachment
         *@exception throws IOException if the <code>InputStream</code> cannot be read
         */
        public AttachmentDataSource(InputStream inputStream, String name) throws IOException {
            /*
             * Create a byte [] the length of the number of bytes in the input stream and read the data from the input
             * stream into the byte [].
             */
         ...
         *@exception IOException
         *                as this method is not meant to be used
         */
    ...
    /** The logger. */
    private static Logger trace = Logger.getLogger(EmailEngine.class.toString());
     ...
     * 
     * <p>
     * The email message must contain at least one primary address, otherwise IllegalArgumentException will be thrown.
     * The method attempts to send the message via the internally configured SMTP settings.
     * </p>
     * 
     * <p>
     * Version 3.0 of Email Engine have added message header support for priority messages.
     * </p>
     * <p>
     ...
     *             if there is no to address or no from address associated with the message
     * @throws ConfigManagerException
     *             if there is a problem with configuration
     * @throws SendingException
     *             if message could not be send properly to the remote server
     */
     ...
     * 
     * <p>
     * Version 3.0 of Email Engine have added support for priority messages. The priority of message is implemented as
     * message headers and the headers are passed as parameter in the headers <code>Map</code>.
     * </p>
     * <p>
     * Version 3.1 of Email Engine have added content type support
     * </p>
     ...
     * @param headers
     *            the custom headers to set for the message, as defined in TCSEmailMessage. No headers if the map is
     *            <code>null</code> or empty
     * @param contentType
     *            the message content type, for example "text/plain
     * @throws NullPointerException
     *             if any of from, to, subject, contentType, data or attachments is <code>null</code>
     * @throws ConfigManagerException
     *             if the engine cannot load properties from the ConfigManager
     * @throws SendingException
    ...
    protected static void send(Address from, Address[] to, Address[] cc, Address[] bcc, String subject, String data,
        ...
        trace.info("Getting configuration for Email Engine...");
        ...
        if (!cm.existsNamespace(PROPERTIES_NAMESPACE)) {
            trace.info("Cannot add the namespace.");
        ...
        try {
            ...
            e.printStackTrace();
            host = null;
            trace.info(PROPERTY_SMTP_HOST_ADDR + " cannot be found in the EmailEngine configuration file");
        }
        if (host == null || host.equals("")) {
            trace.info(PROPERTY_SMTP_HOST_ADDR + " has an invalid value in the EmailEngine configuration file:"
                    + host);
            host = SMTP_HOST_ADDR;
            trace.info(PROPERTY_SMTP_HOST_ADDR + " set to default:" + host);
        }
        String tempPort = "";
        try {
            tempPort = cm.getString(PROPERTIES_NAMESPACE, PROPERTY_SMTP_HOST_PORT);
        } catch (Exception e) {
            trace.info(PROPERTY_SMTP_HOST_PORT + " cannot be found in the EmailEngine configuration file");
        }
        try {
            port = Integer.parseInt(tempPort);
        } catch (Exception e) {
            trace.info(PROPERTY_SMTP_HOST_PORT + " has an invalid value in the EmailEngine configuration file:"
                    + port);
            port = SMTP_HOST_PORT;
            trace.info(PROPERTY_SMTP_HOST_PORT + " set to default:" + port);
        }
        try {
            emailUser = cm.getString(PROPERTIES_NAMESPACE, PROPERTY_SMTP_USER);
        } catch (Exception e) {
            emailUser = null;
            trace.info(PROPERTY_SMTP_USER + " cannot be found in the EmailEngine configuration file");
        }
        if (emailUser == null) {
            trace.info("EmailEngine configuration property " + PROPERTY_SMTP_USER + " is null.");
            emailUser = "";
        }
        try {
            emailPassword = cm.getString(PROPERTIES_NAMESPACE, PROPERTY_SMTP_PASSWORD);
        } catch (Exception e) {
            emailPassword = null;
            trace.info(PROPERTY_SMTP_PASSWORD + " cannot be found in the EmailEngine configuration file");
        }
        if (emailPassword == null) {
            trace.info("EmailEngine configuration property " + PROPERTY_SMTP_PASSWORD + " is null.");
            emailPassword = "";
        }
        ...
        try {
            ...
            eMailTransport.connect(host, port, emailUser, emailPassword);
            ...
            while (namesIterator.hasNext()) {
                ...
                /*
                 * Create a datasource containing the attachment which will be used to create the datahandler set to
                 * this MimeBodyPart.
                 */
            ...
            if (headers != null) {
                trace.info("Adding message headers...");
            ...
            trace.error("Failed to get attachment", e);
    ...
    /**
     * <p>
     * Checks whether object is <code>null</code>, <code>NullPointerException</code> is thrown when it is.
     * <p>
     * 
     * @param object
