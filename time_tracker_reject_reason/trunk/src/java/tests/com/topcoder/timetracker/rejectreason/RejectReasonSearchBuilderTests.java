...
/**
 * Unit test for <code>RejectReasonSearchBuilder</code> class. Instantiation tests and basic functionalities tests.
 *
 * @author TCSDEVELOPER
 * @version 3.2
 */
public class RejectReasonSearchBuilderTests extends TestCase {
    ...
    private RejectReasonSearchBuilder builder = new RejectReasonSearchBuilder();

    /**
     * Tests constructor, verifies the RejectEmail is instantiated correctly.
     */
    public void testConstructor() {
        assertNotNull("Unable to instantiate RejectReasonSearchBuilder.", builder);
    }
    ...
    public void testHasCompanyIdFilter_accuracy() {
        ...
        assertEquals("Filter is not created correctly.", RejectReasonDAO.SEARCH_COMPANY_ID, filter.getName());
        assertEquals("Filter is not created correctly.", companyId, filter.getValue());
    }

    /**
     * Tests hasDescriptionFilter method with a null argument. IllegalArgumentException should be thrown.
     */
    public void testHasDescriptionFilter_null() {
        try {
    ...
    /**
     * Tests hasDescriptionFilter method with an empty string. IllegalArgumentException should be thrown.
     */
    public void testHasDescriptionFilter_empty() {
        try {
    ...
    /**
     * Tests hasDescriptionFilter method accuracy.
     */
    public void testHasDescriptionFilter_accuracy() {
        String description = "This is a description.";
        LikeFilter filter = (LikeFilter) builder.hasDescriptionFilter(description);
        assertEquals("Filter is not created correctly.", RejectReasonDAO.SEARCH_DESCRIPTION, filter.getName());
        assertTrue("Filter is not created correctly.", filter.getValue().toString().matches(".*" + description));
    }

    /**
     * Tests hasActiveStatusFilter method with a negative integer. IllegalArgumentException should be thrown.
     */
    public void testHasActiveStatusFilter_negative() {
        try {
            builder.hasActiveStatusFilter(-1);
            fail("IllegalArgumentException should be thrown.");
        } catch (IllegalArgumentException e) {
            // Ok.
        }
    ...
    /**
     * Tests hasActiveStatusFilter method with an integer greater than 1. IllegalArgumentException should be thrown.
     */
    public void testHasActiveStatusFilter_greaterThanOne() {
        try {
    ...
    /**
     * Tests hasActiveStatusFilter method accuracy. Passes in 0.
     */
    public void testHasActiveStatusFilter_withZero() {
        EqualToFilter filter = (EqualToFilter) builder.hasActiveStatusFilter(0);
        assertEquals("Filter is not created correctly.", RejectReasonDAO.SEARCH_ACTIVE_STATUS, filter.getName());
        assertEquals("Filter is not created correctly.", new Integer(0), filter.getValue());
    }

    /**
     * Tests hasActiveStatusFilter method accuracy. Passes in 1.
     */
    public void testHasActiveStatusFilter_withOne() {
        EqualToFilter filter = (EqualToFilter) builder.hasActiveStatusFilter(1);
        assertEquals("Filter is not created correctly.", RejectReasonDAO.SEARCH_ACTIVE_STATUS, filter.getName());
        assertEquals("Filter is not created correctly.", new Integer(1), filter.getValue());
    }
