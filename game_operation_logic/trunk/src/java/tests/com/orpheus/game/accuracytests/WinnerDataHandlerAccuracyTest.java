...
public class WinnerDataHandlerAccuracyTest extends TestCase {
    ...
    /**
     * The session attribute name where the LoginHandler expects to find the
    ...
    protected void setUp() throws Exception {
        ...
        try { // try to get key for logged-in UserProfile from config manager
            userProfileKey = ConfigManager.getInstance().getString(
                    LoginHandler.class.getName(), "profile_session_key");
            if (userProfileKey == null || userProfileKey.trim().length() == 0) {
    ...
    public void testWinnerDataHandler_Execute_CheckUpdated() throws Exception {
        ...
        session.setAttribute(userProfileKey,
                userProfileManager.getUserProfile(1));
