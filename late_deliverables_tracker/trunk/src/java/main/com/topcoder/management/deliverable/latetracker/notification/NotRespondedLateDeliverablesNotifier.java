...
public class NotRespondedLateDeliverablesNotifier {
    ...
    private Map<Long, List<LateDeliverableDetails>> getLateDeliverableDetailsByUser()
        ...
        for (com.topcoder.management.deliverable.late.LateDeliverable lateDeliverable : lateDeliverables) {
            ...
            if (deliverables.length == 0) {
                throw new NotRespondedLateDeliverablesNotificationException(
                    "There is no matching deliverable in the persistence.");
            }
    ...
    private LateDeliverableDetails getLateDeliverableDetails(
        ...
        lateDeliverableDetails.setDeliverableName(deliverable.getName());
    ...
    private Set<Long> getManagerUserIds(Filter resourceRoleIdFilter, long projectId)
        ...
        for (Resource resource : resources) {
            try {
                ...
                managerUserIds.add(Long.parseLong((String) resource.getProperty("External Reference ID")));
            } catch (NumberFormatException e) {
                throw new NotRespondedLateDeliverablesNotificationException("External Reference ID property value"
