...
public final class Helper {
	...
	/**
	 * <p>
	 * Gets a property value of <code>String</code> type with given key from given <code>ConfigurationObject</code>, or
	 * null if not found when the property is not required.
	 * </p>
	 * 
	 * @param config
	 ...
	 */
	public static String getPropertyValue(ConfigurationObject config, String key, boolean isRequired, boolean canBeEmpty) {
		try {
			...
			if (!(value instanceof String)) {
				throw new LateDeliverablesTrackerConfigurationException("The '" + key
				    + "' property value in configuration object '" + config.getName() + "' should be an instance of '"
				    + String.class.getName() + "', but found '" + value.getClass().getName() + "'.");
			}
	 ...
	 * @throws LateDeliverablesTrackerConfigurationException
	 *             if any error occurs while accessing <code>ConfigurationObject</code>, or if the child with given key
	 *             is <code>null</code>.
	 */
	 ...
	 */
	public static <T> T createObject(ConfigurationObject config, ObjectFactory objectFactory, String key, Class<T> type) {
		try {
	 ...
	 *            The log object.
	 * @param message
	 *            The message to log (not <code>null</code>).
	 */
