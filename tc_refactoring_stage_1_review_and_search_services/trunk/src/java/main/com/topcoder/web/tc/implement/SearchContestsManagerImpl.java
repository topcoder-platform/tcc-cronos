...
/**
 * <p>
 * This is the default implementation of SearchContestsManager. It uses Hibernate to perform the searching.
 * </p>
 * <p>
 ...
 * @version 1.0
 */
public class SearchContestsManagerImpl extends AbstractManagerImpl implements SearchContestsManager {
    ...
    private static final String BASE_HQL = "SELECT pgc.name AS type, pc.name AS subtype, pi.value AS contestName, c.name AS catalog,"
            ...
            + " (SELECT MAX(scheduledStartTime) FROM ProjectPhase WHERE projectId = p.projectId AND phaseTypeId = 1) AS contestStartTime"
            ...
            + " AND wpi.projectId = p.projectId AND wpi.projectInfoTypeId = 2";
     ...
     * @return a list of ContestDTO that matches given name. It will be empty if no such contest can be found.
    ...
    public List<ContestDTO> searchContests(String columnName, SortingOrder sortingOrder, int pageNumber, int pageSize,
        ...
        String methodName = getClass().getName() + ".searchContests(String, SortingOrder, int, int, String)";
        Log4jUtility.logEntrance(getLogger(), methodName, new String[] { "columnName", "sortingOrder", "pageNumber",
                "pageSize", "name" }, new Object[] { columnName, sortingOrder, pageNumber, pageSize, name });
        ...
        try {
            result = searchContests(columnName, sortingOrder, pageNumber, pageSize, filter);
    ...
    public List<ContestDTO> searchContests(String name) throws ContestsServiceManagerException {
        ...
        Log4jUtility.logEntrance(getLogger(), methodName, new String[] { "name" }, new Object[] { name });
     ...
     * @return a list of ContestDTO that matches given filter. It will be empty if no such contest can be found.
     * @throws IllegalArgumentException
    ...
    public List<ContestDTO> searchContests(String columnName, SortingOrder sortingOrder, int pageNumber, int pageSize,
        ...
        String methodName = getClass().getName() + ".searchContests(String, SortingOrder, int, int, ContestsFilter)";
        Log4jUtility.logEntrance(getLogger(), methodName, new String[] { "columnName", "sortingOrder", "pageNumber",
        ...
        try {
            getHibernateTemplate().execute(
                    new SearchContestsHibernateCallback(columnName, sortingOrder, pageNumber, pageSize, filter,
                            contests));
    ...
    public List<ContestDTO> searchContests(ContestsFilter filter) throws ContestsServiceManagerException {
        String methodName = getClass().getName() + ".searchContests(ContestsFilter)";
        ...
        List<ContestDTO> result = null;
        try {
            result = searchContests(null, null, Filterable.RETURN_ALL_PAGES, Filterable.RETURN_ALL_PAGES, filter);
        } catch (ContestsServiceManagerException e) {
            throw ServicesHelper.logException(getLogger(), methodName, e);
        }
    ...
    /**
     * <p>
     * This class implements the HibernateCallback interface to perform query for searching contests.
     * </p>
     * <p>
     ...
     * @version 1.0
     */
    private class SearchContestsHibernateCallback implements HibernateCallback<Object> {
         ...
         */
        public SearchContestsHibernateCallback(String columnName, SortingOrder sortingOrder, int pageNumber,
                int pageSize, ContestsFilter filter, List<ContestDTO> contests) {
        ...
        public Object doInHibernate(Session session) throws HibernateException {
            ...
            if (filter.getContestFinalization() != null) {
                ...
                case AFTER:
                case ON:
                    query.setDate("contestEnd", filter.getContestFinalization().getFirstDate());
                    break;
                case BETWEEN_DATES:
                    query.setDate("contestEnd1", filter.getContestFinalization().getFirstDate());
                    query.setDate("contestEnd2", filter.getContestFinalization().getSecondDate());
                    break;
                }
            }
            if (filter.getWinnerHandle() != null) {
            ...
            for (Object[] row : result) {
                ...
                dto.setWinnerProfileLink(WINNER_PROFILE_LINK_PREFIX + (String) row[7]);
