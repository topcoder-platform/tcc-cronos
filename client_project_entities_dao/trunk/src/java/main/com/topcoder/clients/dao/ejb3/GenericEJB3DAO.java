 ...
 * </p>
 * <p>
 * This base class implements the generic methods available for all the concrete DAOs: retrieve
 * entity by id, retrieve all entities, search entities by name, search entities using the given
 * filter, save an given entity, delete an given entity.
 * </p>
 * <p>
 * It is configured with an EntityManager (initialized by the EJB container through dependency
 * injection) needed to perform operations on the persistence and also uses TopCoder Search Builder
 * 1.4 component to perform the search using the given Filter.
 * </p>
 * <p>
 ...
 * ProjectDAORemote bean =
 *     (ProjectDAORemote) ctx.lookup(&quot;client_project_entities_dao/ProjectDAOBean/remote&quot;);
 *
 ...
 *
 * // new methods added in version 1.2
 * // Increase the budget of project with id 100 with 500 with user ivern.
 * double newBudget = bean.updateProjectBudget(&quot;ivern&quot;, 100, 500);
 *
 * // get the user names who have access of the billing project with id 123.
 ...
 * @version 1.1
 */
public abstract class GenericEJB3DAO<T extends AuditableEntity, Id extends Serializable> implements
    GenericDAO < T, Id > {
    /**
     * <p>
     ...
     * </p>
     * <p>
     * It is initialized during deploying with not null value using {@link PersistenceContext}
     * annotation (we do not check this because depends on configuration of EJB, but if it does not
     * correspond to the given configuration container will report about deployment error).
     * </p>
     * <p>
     * Used by the all the methods that operates on the entities and in flush() and clear() methods.
     * </p>
     * <p>
     * Using corresponding setter, clear and flush methods. Should not be changed after injection.
     * </p>
     * <p>
     ...
     * </p>
     * <p>
     * Injected by the EJB container with non null and not empty value and never changed afterwards.
     * </p>
     * <p>
     ...
     * </p>
     * <p>
     * Initialized in the constructor with non null value and never changed afterwards.
     * </p>
     * <p>
    ...
    /**
     * <p>
     * Represents the search by filter utility needed to perform the search with a given filter.
     * </p>
     * <p>
     * Initialized in the initialize() method with non null value and never changed afterwards.
     * </p>
     * <p>
    ...
    /**
     * <p>
     * Initialize the searchByFilterUtility and makes sure the initialization of this session bean
     * is ok.
     * </p>
     * <p>
     * This method is called after this bean is constructed by the EJB container.
     * </p>
     * @throws DAOConfigurationException if the String fields are null or empty, if needed
     *         configurations are missing or entityManager is invalid (invalid means null here), if
     *         Object Factory cannot be initialized or cannot create SearchByFilterUtility specific
     *         object.
     */
    @PostConstruct
    @SuppressWarnings("unchecked")
    protected void initialize() {
        ...
        try {
            ...
            ConfigurationFileManager cfm = new ConfigurationFileManager(configFileName);
            ...
            if (config == null) {
                throw new DAOConfigurationException(
                    "Cannot found ConfigurationObject with namespace:" + configNamespace);
            }
            ConfigurationObject configObject = config.getChild(configNamespace);
            if (configObject == null) {
                throw new DAOConfigurationException("Cannot found ConfigurationObject with name:"
                    + configNamespace);
            }
            ...
            try {
                ...
                throw new DAOConfigurationException("The '" + tokenKey + "' should be String.", e);
            }
            ...
            ConfigurationObjectSpecificationFactory configurationObjectSpecificationFactory =
                new ConfigurationObjectSpecificationFactory(configObject);
            // get ObjectFactory
            ObjectFactory objectFactory =
                new ObjectFactory(configurationObjectSpecificationFactory);
            String searchBundleName = "HibernateSearchBundle_" + entityBeanType.getSimpleName();

            Object obj =
                objectFactory.createObject(token, null, (ClassLoader) null, new String[] {
                    searchBundleManagerNamespace, searchBundleName }, new Class[] {String.class,
                    String.class }, ObjectFactory.BOTH);

            if (!(obj instanceof SearchByFilterUtility)) {
                throw new DAOConfigurationException("The object configed use key:" + token
            ...
            if (!(e instanceof DAOConfigurationException)) {
                throw new DAOConfigurationException(
                    "Failed to initialize searchByFilterUtility fields of DAO.", e);
            }
    ...
    private void checkRequiredFields() {
        ...
        if (configFileName == null || configFileName.trim().length() == 0) {
            throw new DAOConfigurationException("The 'configFileName' should not be null.");
        }
        if (configNamespace == null || configNamespace.trim().length() == 0) {
            throw new DAOConfigurationException("The 'configNamespace' should not be null.");
        }
        ...
        if (entityManager == null) {
            throw new DAOConfigurationException("The 'entityManager' should not be null.");
        }
    }

    /**
     * <p>
     * Setter used to set the entityManager. Please refer to the related 'entityManager' field for
     * more information.
     * </p>
     * <p>
     * Tag removed in version 1.2.
     * </p>
     * @param entityManager the entityManager used in persistence.
     */
    ...
    /**
     * Getter for 'entityManager' property. Please refer to the related 'entityManager' field for
     * more information. Protected because it is used by subclasses only.
    ...
    /**
     * Getter for 'entityBeanType' property. Please refer to the related 'entityBeanType' field for
     * more information.
    ...
    /**
     * Flush operation over the configured EntityManager. Synchronize the persistence context to the
     * underlying database.
     */
    public void flush() {
        entityManager.flush();
    ...
    /**
     * Clear operation over the configured EntityManager. Clear the persistence context, causing all
    ...
    /**
     * Performs the retrieval of an entity using the given id from the persistence. Return null if
     * the entity is deleted.
     * @param id the identifier of the entity that should be retrieved. Should be positive and not
     *        null.
     * @return the entity with the given id retrieved from the persistence.
     * @throws IllegalArgumentException if id <= 0 or id is null.
     * @throws EntityNotFoundException if an entity for the given id is not found in the
     *         persistence.
    ...
    public T retrieveById(Id id) throws EntityNotFoundException, DAOException {
        ...
        String queryString =
            "select e from " + entityBeanType.getCanonicalName()
        ...
        try {
            ...
            throw new EntityNotFoundException("Cannot find entity with id:" + id + ".", e);
        } catch (Exception e) {
    ...
    /**
     * Performs the retrieval of all entities from the persistence. If nothing is found, return an
     * empty list. Return only the entities that are not marked as deleted.
     * @return the list of entities found in the persistence. If nothing is found, return an empty
     *         list.
     * @throws DAOConfigurationException if the configured entityManager is invalid (invalid means
     *         null here).
     * @throws DAOException if any error occurs while performing this operation.
    ...
    public List < T > retrieveAll() throws DAOException {
        String queryString =
            "select e from " + entityBeanType.getCanonicalName() + " as e"
                + " where e.deleted is null or e.deleted = false";
        Query query = Helper.checkEntityManager(entityManager).createQuery(queryString);
    ...
    /**
     * Performs the retrieval of all entities that have the given name in the persistence. If
     * nothing is found, return an empty list. Return only the entities that are not marked as
     * deleted.
     * @param name the name of the entity that should be retrieved. Should be not empty and not
     *        null.
     * @return the list with entities with the given name retrieved from the persistence. If nothing
    ...
    /**
     * Performs the retrieval of all entities that match the given filter in the persistence. If
     * nothing is found, return an empty list. Return only the entities that are not marked as
     * deleted.
     * @param filter the filter that should be used to search the matched entities. Should not be
     *        null.
     * @return the list with entities that match the given filter retrieved from the persistence. If
     ...
     * @param entity the entity that should be saved. Should not be null.
     ...
     * @param entity the entity that should be deleted from the persistence. Should not be null.
     * @throws IllegalArgumentException if entity is null.
     * @throws EntityNotFoundException if entity is not found in the persistence.
     * @throws DAOConfigurationException if the configured entityManager is invalid (invalid means
     *         null here).
     * @throws DAOException if any error occurs while performing this operation.
     */
    ...
    public void delete(T entity) throws EntityNotFoundException, DAOException {
        ...
        try {
            ...
            throw Helper.wrapWithDAOException(e, "Failed to delete entity.");
        }
    }
