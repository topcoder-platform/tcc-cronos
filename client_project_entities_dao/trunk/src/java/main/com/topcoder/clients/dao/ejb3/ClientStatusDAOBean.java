 ...
 * </p>
 * <p>
 * This class implements the method available for the ClientStatusDAO business interface: get the
 * clients with the corresponding status.
 * </p>
 * <p>
...
public class ClientStatusDAOBean extends GenericEJB3DAO < ClientStatus, Long > implements
    ClientStatusDAO, ClientStatusDAOLocal, ClientStatusDAORemote {
    /**
     * The query string used in method getClientsWithStatus.
     */
    private static final String QUERYSTRING =
        "select c from Client c" + " where c.clientStatus = :status and c.deleted = false";

    /**
     * Default no-arg constructor. Constructs a new 'ClientStatusDAOBean' instance.
     */
    public ClientStatusDAOBean() {
    }

    /**
     * Performs the retrieval of the list with clients with the given status from the persistence.
     * If nothing is found, return an empty list. Return only the entities that are not marked as
     * deleted.
     * @param status the given client status to retrieve it's clients. Should not be null.
    ...
    public List < Client > getClientsWithStatus(ClientStatus status)
        ...
        try {
            ...
            throw Helper.wrapWithDAOException(e, "Failed to get Clients with Status.");
        }
    }
}
