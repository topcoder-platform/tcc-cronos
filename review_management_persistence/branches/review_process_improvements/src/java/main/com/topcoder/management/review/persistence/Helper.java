...
class Helper {
     ...
     * used in the query methods to specify that a <code>ResultSet</code> column of a query result
     * should be returned as
     * value of type <code>String</code> or as <code>null</code> in case the <code>ResultSet</code>
     * value was
     * <tt>null</tt>.
     */
     ...
     * used in the query methods to specify that a <code>ResultSet</code> column of a query result
     * should be returned as
     * value of type <code>Long</code> or as <code>null</code> in case the <code>ResultSet</code>
     * value was
     * <tt>null</tt>.
     */
     ...
     * used in the query methods to specify that a <code>ResultSet</code> column of a query result
     * should be returned as
     * value of type <code>Date</code> or as <code>null</code> in case the <code>ResultSet</code>
     * value was
     * <tt>null</tt>.
     */
     ...
     * used in the query methods to specify that a <code>ResultSet</code> column of a query result
     * should be returned as
     * value of type <code>Float</code> or as <code>null</code> in case the <code>ResultSet</code>
     * value was
     * <tt>null</tt>.
     */
     ...
     * database-dependent (e.g. for a BLOB column the MySQL driver returns a <code>byte[]</code> and
     * the Oracle driver
     * returns a <code>Blob</code>)).
     * <p/>
     * This class contains a private constructor to make sure all implementations of this class are
     * declared inside
     * <code>Helper</code>. Instances are provided to users via constants declared in
     * <code>Helper</code> - so this
    ...
    /**
     * This class is a wrapper for type safe retrieval of values from a <code>ResultSet</code>. The
     * values retrieved by
     * the <code>getValue(java.sql.ResultSet, int)</code> implementation of this
     * <code>DataType</code> are assured to be
     * of type <code>String</code> or to be <tt>null</tt> in case the <code>ResultSet</code> value
     * was <tt>null</tt>.
     * @author urtks
     * @version 1.0
     */
    private static class StringType extends DataType {
         ...
         * @param index
         *            the index at which to retrieve the value
         * @return the retrieved value as <code>String</code> or <code>null</code> if the value in
         *         the
     ...
     * the <code>getValue(java.sql.ResultSet, int)</code> implementation of this
     * <code>DataType</code> are assured to be
     * of type <code>Long</code> or
     * to be <tt>null</tt> in case the <code>ResultSet</code> value was <tt>null</tt>.
     * @author urtks
     * @version 1.0
     */
    ...
    private static class FloatType extends DataType {
         ...
         * @param index
         *            the index at which to retrieve the value
         * @return the retrieved value as <code>Float</code> or <code>null</code> if the value in
         *         the
         *         <code>ResultSet</code> was <code>null</code>.
         * @throws IllegalArgumentException
         *             if resultSet is <code>null</code>
     ...
     * given connection using the given query arguments. The <code>ResultSet</code> returned from
     * the query is expected
     * to contain
     * ONE row containing ONE value, which is then returned. This approach
     * assured that all resources (<code>PreparedStatement</code> and the <code>ResultSet</code>)
     * allocated in this
     * method are also de-allocated
     ...
     *         by the given columnType or <code>null</code> if the <code>ResultSet</code> value was
     *         <code>null</code>
     * @throws IllegalArgumentException
     ...
     * that all resources (the <code>PreparedStatement</code> and the <code>ResultSet</code>)
     ...
     * @return the result of the query as List containing an <code>Object[]</code> for each
     *         <code>ResultSet</code> row
     *         The
     *         elements of the array are of the type represented by the <code>DataType</code>
     *         specified at the
    ...
    static long countEntities(String tableName, String columnName, long id, Connection conn)
        ...
        Long entryCount =
                (Long) Helper.doSingleValueQuery(conn, "SELECT COUNT(*) FROM " + tableName
                        + " WHERE " + columnName + "=?", new Object[] {new Long(id)},
                        Helper.LONG_TYPE);
    ...
    static String getConfigurationParameterValue(ConfigManager cm, String namespace, String name,
        ...
        if (value == null) {
            logger.log(Level.FATAL, "Configuration parameter [" + name + "] under namespace ["
            ...
            logger.log(Level.FATAL, "Configuration parameter [" + name + "] under namespace ["
                    + namespace + "] is empty (trimmed).");
            throw new ConfigurationException("Configuration parameter [" + name
                    + "] under namespace [" + namespace + "] is empty (trimmed).");
        }
        logger.log(Level.INFO, "Read required propery[" + name + "] with value[" + value
