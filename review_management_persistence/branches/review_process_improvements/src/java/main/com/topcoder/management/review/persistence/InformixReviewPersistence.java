...
public class InformixReviewPersistence implements ReviewPersistence {
    ...
    private static final String CREATE_REVIEW_COMMENT_SQL = "INSERT INTO " + REVIEW_COMMENT_TABLE
            + " (review_comment_id, resource_id, review_id, comment_type_id, evaluation_type_id, content, extra_info, sort,"
            + " create_user, create_date, modify_user, modify_date)" + " values (?,?,?,?,?,?,?,?,?,CURRENT,?,CURRENT)";
    ...
    private static final String CREATE_REVIEW_ITEM_COMMENT_SQL = "INSERT INTO " + REVIEW_ITEM_COMMENT_TABLE
            + " (review_item_comment_id, resource_id, review_item_id, comment_type_id, evaluation_type_id, content, extra_info, sort,"
            + " create_user, create_date, modify_user, modify_date)" + " values (?,?,?,?,?,?,?,?,?,CURRENT,?,CURRENT)";
    ...
    private static final String UPDATE_REVIEW_SQL = "UPDATE " + REVIEW_TABLE
            + " SET resource_id=?, submission_id=?, scorecard_id=?, committed=?, score=?, initial_score=?,"
            + " review_evaluation_id=?, modify_user=?, modify_date=CURRENT" + " WHERE review_id=?";
    ...
    private static final String UPDATE_REVIEW_COMMENT_SQL = "UPDATE " + REVIEW_COMMENT_TABLE
            + " SET resource_id=?, comment_type_id=?, evaluation_type_id=?, content=?, extra_info=?, sort=?,"
    ...
    private static final String QUERY_REVIEWS_SQL = "SELECT "
            + "review_id, resource_id, submission_id, scorecard_id, committed, score, initial_score, "
            + "create_user, create_date, modify_user, modify_date, review_evaluation_id FROM " + REVIEW_TABLE + " WHERE review_id IN ("
    ...
    private static final String QUERY_REVIEW_COMMENTS_SQL = "SELECT "
            + "review_comment_id, resource_id, review_id, comment_type_id, evaluation_type_id, content, extra_info, modify_date " + "FROM "
    ...
    private static final String QUERY_REVIEW_ITEM_COMMENTS_SQL = "SELECT "
            + "ric.review_item_comment_id, ric.resource_id, ric.review_item_id, ric.comment_type_id, ric.evaluation_type_id, "
            + "ric.content, ric.extra_info, ric.modify_date " + "FROM " + REVIEW_ITEM_COMMENT_TABLE + " ric " + "INNER JOIN "
    ...
    private void createReview(Review review, String operator, Connection conn, Map<Object, Long> changeTable)
        ...
        if (review.getSubmission() > 0) {
            queryArgs =
                    new Object[] {newId, review.getAuthor(), review.getSubmission(), review.getScorecard(),
                                  review.isCommitted() ? 1 : 0, review.getScore(), review.getInitialScore(), operator,
                                  operator};
        } else {
            queryArgs =
                    new Object[] {newId, review.getAuthor(), null, review.getScorecard(),
                                  review.isCommitted() ? 1 : 0, review.getScore(), review.getInitialScore(), operator,
                                  operator};
        }
    ...
    private void createReviewComments(Comment[] comments, Long[] indices, Long reviewId, String operator,
        ...
        try {
            ...
            for (int i = 0; i < comments.length; ++i) {
                ...
                Object[] queryArgs =
                {newId, comment.getAuthor(), reviewId, comment.getCommentType().getId(), comment.getEvaluationType() != null ?
    ...
    private void createReviewItemComments(Comment[] comments, Long[] indices, Long itemId, String operator,
        ...
        try {
            ...
            for (int i = 0; i < comments.length; ++i) {
                ...
                Object[] queryArgs =
                {newId, comment.getAuthor(), itemId, comment.getCommentType().getId(), comment.getEvaluationType() != null ? 
                                new Long(comment.getEvaluationType().getId()) : null, comment.getComment(),
                 comment.getExtraInfo(), indices[i], operator, operator};
    ...
    public void updateReview(Review review, String operator) throws ReviewPersistenceException {
        ...
        Date modifyDate;
        LOGGER.log(Level.INFO, new LogMessage(review.getId(), operator, "Update Review."));
        try {
            // create the connection
            conn = createTransactionalConnection();
            // check whether the review id is already in the database
    ...
    private void updateReview(Review review, String operator, Connection conn, Map<Object, Long> changeTable)
        ...
        if (review.getSubmission() > 0) {
            queryArgs =
                    new Object[] {review.getAuthor(), review.getSubmission(), review.getScorecard(),
                                  review.isCommitted() ? 1 : 0, review.getScore(), review.getInitialScore(), 
                                  review.getReviewEvaluation() > 0 ? review.getReviewEvaluation() : null, operator,
                                  reviewId};
        } else {
            queryArgs =
                    new Object[] {review.getAuthor(), null, review.getScorecard(), review.isCommitted() ? 1 : 0,
                                  review.getScore(), review.getInitialScore(),
                                  review.getReviewEvaluation() > 0 ? review.getReviewEvaluation() : null, operator, reviewId};
        }
        LOGGER.log(Level.INFO, "update record in the  " + REVIEW_TABLE + " table with id:" + reviewId);
    ...
    private static void updateReviewComment(Comment comment, Long index, String operator, Connection conn)
        ...
        Object[] queryArgs =
        {comment.getAuthor(), comment.getCommentType().getId(), comment.getEvaluationType() != null ?
                        new Long(comment.getEvaluationType().getId()) : null, comment.getComment(), comment.getExtraInfo(),
    ...
    private static void deleteReviewComments(Set<Long> reviewCommentIDs, Connection conn)
        throws ReviewPersistenceException {
        // enumerate each id
        LOGGER.log(Level.INFO, "delete entry from " + REVIEW_COMMENT_TABLE + " with the review comment id:"
    ...
    private static void deleteReviewItems(Set<Long> reviewItemIDs, Connection conn) throws ReviewPersistenceException {
        ...
        LOGGER.log(Level.INFO, "delete entries from " + REVIEW_ITEM_COMMENT_TABLE + " with the review item ids: "
                + ids);
        Helper.deleteEntities(REVIEW_ITEM_COMMENT_TABLE, "review_item_id", reviewItemIDs, conn);
        LOGGER.log(Level.INFO, "delete entries from " + REVIEW_ITEM_TABLE + " with the review item ids: " + ids);
        Helper.deleteEntities(REVIEW_ITEM_TABLE, "review_item_id", reviewItemIDs, conn);
        // delete item uploads
        if (!reviewItemUploadIDs.isEmpty()) {
            LOGGER.log(Level.INFO, "delete entries from " + UPLOAD_TABLE + " with the upload ids: "
    ...
    private static void deleteReviewItemComments(Set<Long> reviewItemCommentIDs, Connection conn)
        throws ReviewPersistenceException {
        // enumerate each id
        LOGGER.log(Level.INFO, "delete entries from " + REVIEW_ITEM_COMMENT_TABLE
    ...
    public Review getReview(long id) throws ReviewPersistenceException {
        ...
        Connection conn = null;
        LOGGER.log(Level.INFO, new LogMessage(id, null, "Retrieve Review."));
        try {
            // create the connection
            conn = createTransactionalConnection();
            // check whether the review id is already in the database
    ...
    public Review[] searchReviews(Filter filter, boolean complete) throws ReviewPersistenceException {
        ...
        LOGGER.log(Level.INFO, new LogMessage(null, null, "Search " + (complete ? " complete " : " not completed")
                + " Reviews "));
        try {
        ...
        try {
            ...
            Map<Long, EvaluationType> evaluationTypeMap = makeIdEvaluationTypeMap(getAllEvaluationTypes(conn));
            ...
            getReviewItemComments(idsList, conn, makeIdItemMap(items), commentTypeMap, evaluationTypeMap);
            return reviews;
        } catch (ReviewPersistenceException e) {
    ...
    private static Review[] getReviews(String idList, Connection conn) throws ReviewPersistenceException {
        ...
        DataType[] columnTypes =
                new DataType[] {Helper.LONG_TYPE, Helper.LONG_TYPE, Helper.LONG_TYPE, Helper.LONG_TYPE,
                                Helper.LONG_TYPE, Helper.FLOAT_TYPE, Helper.FLOAT_TYPE, Helper.STRING_TYPE,
        ...
        for (int i = 0; i < rows.length; ++i) {
            ...
            if (row[index] != null){
                review.setReviewEvaluation(((Long) row[index]).longValue());
            }
    ...
    private static Review getReview(CustomResultSet resultSet) throws InvalidCursorStateException {
        ...
        try {
            ...
            if (resultSet.getObject("initial_score") != null) {
                review.setInitialScore(resultSet.getFloat("initial_score"));
            } else {
     ...
     * @param evaluationTypeMap the Id-EvaluationType map
     * @throws ReviewPersistenceException if any error occurs during the query
     */
    private static void getReviewComments(String idList, Connection conn, Map<Long, Review> reviewMap,
            Map<Long, CommentType> commentTypeMap, Map<Long, EvaluationType> evaluationTypeMap) throws ReviewPersistenceException {
        ...
        Object[][] rows =
                Helper.doQuery(conn, QUERY_REVIEW_COMMENTS_SQL.replaceFirst(ID_ARRAY_PARAMETER_REGULAR_EXP, idList),
                        new Object[] {}, columnTypes);
        // enumerate each comment
        for (int i = 0; i < rows.length; ++i) {
            Object[] row = rows[i];
            ...
            comment.setEvaluationType(evaluationTypeMap.get(row[index++]));
            comment.setComment((String) row[index++]);
            comment.setExtraInfo((String) row[index++]);
            comment.setModificationTimestamp((Date) row[index]);
        }
    }
    ...
    private static void getReviewItemComments(String idList, Connection conn, Map<Long, Item> itemMap,
        ...
        Object[][] rows =
                Helper.doQuery(conn,
                        QUERY_REVIEW_ITEM_COMMENTS_SQL.replaceFirst(ID_ARRAY_PARAMETER_REGULAR_EXP, idList),
                        new Object[] {}, columnTypes);
        // enumerate each comment
        for (int i = 0; i < rows.length; ++i) {
            Object[] row = rows[i];
     ...
     */
    private static Map<Long, EvaluationType> makeIdEvaluationTypeMap(EvaluationType[] evaluationType) {
        Map<Long, EvaluationType> map = new HashMap<Long, EvaluationType>();
    ...
    public void addReviewComment(long reviewId, Comment comment, String operator) throws ReviewPersistenceException {
        ...
        Map<Object, Long> changeTable = new HashMap<Object, Long>();
        Connection conn = null;
        LOGGER.log(Level.INFO, new LogMessage(reviewId, operator, "Add comment to the review."));
        try {
            // create the connection
            conn = createTransactionalConnection();
    ...
    public void removeReview(long id, String operator) throws ReviewPersistenceException {
        Helper.assertLongPositive(id, "id");
        Helper.assertStringNotNullNorEmpty(operator, "operator");
        LOGGER.log(Level.INFO, new LogMessage(id, operator, "Delete Review."));
        Connection conn = null;
        try {
            // Create the connection:
            conn = createTransactionalConnection();
