...
public final class Helper {
    ...
    /**
     * <p>
     * Throw IllegalArgumentException if the parameter is not positive or not -1.
     * </p>
     *
     * @param param
     *            the argument to be checked
    ...
    public static void checkPageNumberAndSize(String name, int param) {
        if (param <= 0 && param != -1) {
            throw new IllegalArgumentException("The argument " + name + " must be positive or -1.");
        }
    }
    ...
    public static void checkNotNullForConfiguration(String name, Object param) {
        if (param == null) {
            throw new ContestServicesConfigurationException("The argument " + name + " cannot be null.");
        }
    }
    ...
    public static void checkNotNegativeForConfiguration(String name, long param) {
        if (param < 0) {
            throw new ContestServicesConfigurationException("The argument " + name + " must be non-negative.");
        }
    }
    ...
    public static String addFilterForSQL(BaseFilter filter, String contestName) {
        ...
        if (checkListNotNullAndNotEmpty(filter.getTypes())) {
            sql += addListToFilter("projectGroupCategory.name", filter.getTypes());
        }
        ...
        if (checkListNotNullAndNotEmpty(filter.getCatalogs())) {
            sql += Helper.addListToFilter("projectCatalog.name", filter.getCatalogs());
        }

        // Add filter for contestName
        if (checkStringNotNullAndNotEmptyOfBoolean(filter.getContestName())) {
            sql += " AND projectNameInfo.value LIKE '%" + filter.getContestName() + "%'";
        }

        // Add filter for registrationStartDate
        if (filter.getRegistrationStartDate() != null) {
            sql += addDateToFilter("registrationPhase.scheduledStartTime", filter.getRegistrationStartDate());
        }

        // Add filter for submissionEndDate
        if (filter.getSubmissionEndDate() != null) {
            sql += addDateToFilter("submissionPhase.scheduledEndTime", filter.getSubmissionEndDate());
        }
        ...
        if (filter.getContestFinalizationDate() != null) {
            if ("ActiveContest".equals(contestName)) {
                ...
                sql += addDateToFilter("finalReviewPhase.scheduledEndTime", filter
                                .getContestFinalizationDate());
            } else if ("PastContest".equals(contestName)) {
                sql += addDateToFilter("approvalPhase.actualEndTime", filter.getContestFinalizationDate());
            } else if ("ContestStatus".equals(contestName)) {
                sql += addDateToFilter("finalReviewPhase.actualEndTime", filter.getContestFinalizationDate());
            }
        }
     ...
     * @return the sql string
     */
    public static String addOrderForSQL(String columnName, SortingOrder sortingOrder) {
        String sql = "";
        if ((columnName != null && !columnName.trim().isEmpty() && sortingOrder != null)) {
     ...
     *            the page size
     */
    public static void setPageForQuery(final Query query, int pageNumber, int pageSize) {
     ...
     */
    public static <T extends Object> List<T> getPageForPrize(int pageNumber, int pageSize, List<T> list) {
        // Get the correct page for prize filter
     ...
     *             if any error occurs
     */
    private static String addDateToFilter(String name, DateIntervalSpecification dateInterval) {
        if (dateInterval.getIntervalType() == null) {
            throw new HibernateException("The dateInterval.IntervalType can not be null.");
        }
        StringBuffer sb = new StringBuffer();
        sb.append(" AND ");
        sb.append(name);
        if (dateInterval.getIntervalType() == DateIntervalType.BEFORE) {
            sb.append(" < '");
            sb.append(dateToString(DateIntervalType.BEFORE.toString(), dateInterval.getFirstDate()));
            sb.append("'");
        } else if (dateInterval.getIntervalType() == DateIntervalType.AFTER) {
            sb.append(" > '");
            sb.append(dateToString(DateIntervalType.AFTER.toString(), dateInterval.getFirstDate()));
            sb.append("'");
        } else if (dateInterval.getIntervalType() == DateIntervalType.ON) {
            sb.append(" = '");
            sb.append(dateToString(DateIntervalType.ON.toString(), dateInterval.getFirstDate()));
            sb.append("'");
        } else if (dateInterval.getIntervalType() == DateIntervalType.BETWEEN_DATES) {
            sb.append(" BETWEEN '");
            sb.append(dateToString(DateIntervalType.BETWEEN_DATES.toString(), dateInterval.getFirstDate()));
            sb.append("' AND '");
            sb.append(dateToString(DateIntervalType.BETWEEN_DATES.toString(), dateInterval.getSecondDate()));
            sb.append("'");
    ...
    private static String dateToString(String name, Date date) {
        if (date == null) {
            throw new HibernateException("The date with " + name + " can not be null.");
        }
    ...
    public static Map<String, Object> parseJsonToParam(JSONObject jsonObject)
        ...
        try {
            ...
            throw new ContestServicesActionException("pageNumber or pageSize in JSON is invalid number.", e);
        } catch (IllegalArgumentException e) {
            throw new ContestServicesActionException("sortingOrder in JSON is invalid.", e);
        }
    }
     ...
     *             if any error occurs
     */
    public static void parseJsonToBaseFilter(BaseFilter filter, JSONObject jsonObject, String dateFormat)
        throws ContestServicesActionException {
        try {
            if (jsonObject.isNestedKeyDefined("filter")) {
                ...
                filter.setRegistrationStartDate(getDateIntervalFromJson(jsonFilter, "registrationStartDate",
                    dateFormat));
                filter.setSubmissionEndDate(getDateIntervalFromJson(jsonFilter, "submissionEndDate",
                    dateFormat));
                filter.setContestFinalizationDate(getDateIntervalFromJson(jsonFilter,
                    "contestFinalizationDate", dateFormat));

                if (filter instanceof BasePrizeFilter) {
                    ((BasePrizeFilter) filter).setPrizeLowerBound(getIntFromJson(jsonFilter,
                        "prizeLowerBound"));
                    ((BasePrizeFilter) filter).setPrizeUpperBound(getIntFromJson(jsonFilter,
                        "prizeUpperBound"));
                }

                if (filter instanceof PastContestFilter) {
                    ((PastContestFilter) filter)
                                    .setWinnerHandle(getStringFromJson(jsonFilter, "winnerHandle"));
                }
                if (filter instanceof ContestStatusFilter) {
                    ((ContestStatusFilter) filter).setWinnerHandle(getStringFromJson(jsonFilter,
                        "winnerHandle"));
                }
            }
        } catch (NumberFormatException e) {
            throw new ContestServicesActionException(
                "prizeLowerBound or prizeUpperBound in JSON is invalid number.", e);
        }
    }
    ...
    private static DateIntervalSpecification getDateIntervalFromJson(JSONObject jsonObject, String key,
        ...
        try {
            if (jsonObject.isKeyDefined(key) && jsonObject.isNestedKeyDefined(key)) {
                ...
                if (type == null) {
                    throw new ContestServicesActionException(key + "'s intervalType in JSON can not be null.");
                }
                ...
                try {
                    ...
                    throw new ContestServicesActionException(key + "'s intervalType in JSON is invalid.", e);
                }
                ...
                if (intervalType == DateIntervalType.AFTER || intervalType == DateIntervalType.BEFORE
                    ...
                    if (firstDate == null) {
                        throw new ContestServicesActionException(key + "'s type is " + intervalType
                            + ", first date in JSON can not be null.");
                    }
                ...
                if (intervalType == DateIntervalType.BETWEEN_DATES) {
                    if (secondDate == null) {
                        throw new ContestServicesActionException(key
                            + "'s type is BETWEEN_DATES, second date in JSON can not be null.");
                    }
            ...
            throw new ContestServicesActionException(
                "dateFormat[" + dateFormat + "] in instance is invalid.", e);
        } catch (ParseException e) {
            throw new ContestServicesActionException("ParseException occurs while parsing date.", e);
        }
     ...
     *            the output result
     */
    public static void logExit(Logger logger, String signature, long startTime, Object outputResult) {
        long endTime = System.currentTimeMillis();
        logger.debug("[Exiting the method {" + signature + "}.  Time of execution {" + (endTime - startTime)
            + "ms}]");
     ...
     */
    public static <T extends Throwable> T logException(Logger logger, T exception, String method) {
        // Log exception at ERROR level
        logger.error("[Error in method {" + method + "}: Details {" + exception.getMessage() + "}]",
            exception);
        return exception;
    }
     ...
     */
    private static String parametersToString(String[] paramNames, Object[] params) {
