...
/**
 * This class is used to manage the log in state in request for the
 * <code>SecurityManagerAuthenticator</code>, so it¡¯s expected that this parser should
 * stay with <code>SecurityManagerAuthenticator</code> within the configuration file for
 * <code>LoginActions</code>.
 * <p>
 * In the <code>setLoginState</code> method, if authentication succeeded, it will store
 * in the user identifier in the session attributes. In the <code>unsetLoginState</code>
 * method, it will remove the user identifier from the session and invalidate the session.
 * </p>
 * <p>
...
public class SecurityManagerAuthResponseParser implements AuthResponseParser {
    ...
    /**
     * The key name of the user identifier. The value of this key is the user id retrieved
     * from TCSubject. The key-value pair will be stored in the request session in the
     * setLogin method if authentication succeeded.
     * <p>
     ...
     * <p>
     * <em>user_identifer_key</em> will be retrieved from the given namespace. It will
     * be used as a key to store user id in session.
     * </p>
     *
     * @param namespace
     ...
     */
    public SecurityManagerAuthResponseParser(String namespace)
        throws ConfigurationException {
        Util.validateNotNullOrEmpty(namespace, "namespace");

        userIdentifierKey = Util.getRequiredPropertyString(namespace,
                "user_identifer_key");
    }
     ...
     * <p>
     * If the given <code>authResponse</code> is successful, a new session will be retrieved from
     * the <code>request</code>, and the user id will be stored as an attribute. Otherwise, nothing would
     * happen.
     * </p>
     *
     * @param principal
     ...
     *             if <code>authResponse#getDetails()</code> is not of type
     *             <code>com.topcoder.security.TCSubject</code>, or any other error occurred
     */
    public void setLoginState(Principal principal, Response authResponse,
        ...
        if (!TCSubject.class.isInstance(subjectObj)) {
            throw new AuthResponseParsingException(
                    "The details of response is not instanceof " + TCSubject.class);
        }
