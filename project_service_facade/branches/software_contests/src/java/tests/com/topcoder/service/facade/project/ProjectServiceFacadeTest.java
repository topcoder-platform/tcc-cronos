...
public class ProjectServiceFacadeTest extends TestCase {
    ...
    /**
     * <p>Accuracy test. Tests the {@link ProjectServiceFacade#getAllProjects()} functionality for producing accurate
     * results.</p>
     *
    ...
    public void testGetAllProjects_User() throws Exception {
        ...
        List<ProjectData> actualProjects = client.getAllProjects();
    ...
    /**
     * <p>Accuracy test. Tests the {@link ProjectServiceFacade#getProject(long)} functionality for producing accurate
     * results.</p>
     *
    ...
    public void testGetProject_User_OwnProjects() throws Exception {
        ...
        for (ProjectData expectedProject : this.userProjects) {
            ProjectData actualproject = client.getProject(expectedProject.getProjectId());
            verifyProjects(expectedProject, actualproject);
        }
    }

    /**
     * <p>Failure test. Tests the {@link ProjectServiceFacade#getProject(long)} functionality for proper handling the
     * case when a call is made for getting details for the project which the user is not granted access permission for.
     * </p>
     *
     * <p>Authenticates to web service as user granted <code>Cockpit User</code> role, passes IDs of projects belonging
    ...
    public void testGetProject_User_AlienProjects() throws Exception {
        ...
        for (ProjectData expectedProject : this.adminProjects) {
            try {
                client.getProject(expectedProject.getProjectId());
    ...
    /**
     * <p>Failure test. Tests the {@link ProjectServiceFacade#getProject(long)} functionality for proper handling the
     * case when a call is made for getting details for the non-existing project.</p>
     *
    ...
    /**
     * <p>Failure test. Tests the {@link ProjectServiceFacade#getProjectsForUser(long)} functionality for proper
     * handling the case when a call is made by the user who is not granted <code>Cockpit Administrator</code> role.</p>
     *
    ...
    public void testGetProjectsForUser_User_OwnId() throws Exception {
        ProjectServiceFacade client = createWebServiceClient(USER_HANDLE);
        try {
            client.getProjectsForUser(USER_ID);
    ...
    public void testGetProjectsForUser_User_AlienId() throws Exception {
        ProjectServiceFacade client = createWebServiceClient(USER_HANDLE);
        try {
            client.getProjectsForUser(ADMIN_ID);
            Assert.fail("SOAPFaultException should have been thrown");
        } catch (SOAPFaultException e) {
            // Expected behavior
        }
    ...
    /**
     * <p>Accuracy test. Tests the {@link ProjectServiceFacade#getProjectsForUser(long)} functionality for producing
     * accurate results.</p>
     *
    ...
    public void testGetProjectsForUser_Admin_OwnId() throws Exception {
        ...
        List<ProjectData> actualProjects = client.getProjectsForUser(ADMIN_ID);
        Assert.assertEquals("Wrong number of projects retrieved", expectedProjects.size(), actualProjects.size());
        for (ProjectData expectedProject : expectedProjects) {
            boolean found = false;
            for (ProjectData actualProject : actualProjects) {
    ...
    public void testGetProjectsForUser_Admin_AlienId() throws Exception {
        ProjectServiceFacade client = createWebServiceClient(ADMIN_HANDLE);
        List<ProjectData> expectedProjects = this.userProjects;
        List<ProjectData> actualProjects = client.getProjectsForUser(USER_ID);
        Assert.assertEquals("Wrong number of projects retrieved", expectedProjects.size(), actualProjects.size());
        for (ProjectData expectedProject : expectedProjects) {
            boolean found = false;
            for (ProjectData actualProject : actualProjects) {
    ...
    public void testGetProjectsForUser_Admin_NonExistingUser() throws Exception {
        ProjectServiceFacade client = createWebServiceClient(ADMIN_HANDLE);
        try {
            client.getProjectsForUser(-235735L);
    ...
    /**
     * <p>Accuracy test. Tests the {@link ProjectServiceFacade#deleteProject(long)} functionality for producing
     * accurate results.</p>
     *
     * <p>Authenticates to web service as user granted <code>Cockpit User</code> role, passes IDs of projects (with no
     * associated competitions) belonging to user and verifies that the service deletes existing project data correctly.
     * </p>
     *
     * @throws Exception if an unexpected error occurs.
     */
    public void testDeleteProject_User_OwnProject() throws Exception {
        Map<Long, Long> competitions = getCompetitions(USER_ID);
        ProjectServiceFacade client = createWebServiceClient(USER_HANDLE);
        for (ProjectData project : this.userProjects) {
            long projectid = project.getProjectId();
            if (!competitions.containsKey(projectid)) {
                boolean result = client.deleteProject(projectid);
                Assert.assertTrue("The project is not reported to be deleted successfully", result);
                Assert.assertFalse("The project was not deleted from database", projectExists(projectid));
            }
        }
    }
     ...
     * accurate results.</p>
     *
     * <p>Authenticates to web service as user granted <code>Cockpit User</code> role, passes ID of non-existing
     * project and verifies that the service handles this case correctly.</p>
     *
     * @throws Exception if an unexpected error occurs.
     */
    public void testDeleteProject_User_NonExistingProject() throws Exception {
        ProjectServiceFacade client = createWebServiceClient(USER_HANDLE);
        boolean result = client.deleteProject(-5748948L);
        Assert.assertFalse("The project is reported to be deleted successfully", result);
    }

    /**
     * <p>Failure test. Tests the {@link ProjectServiceFacade#deleteProject(long)} functionality for proper handling
     * the case when a call is made for deleteing the project which the user is not granted access permission for. </p>
     *
     * <p>Authenticates to web service as user granted <code>Cockpit User</code> role, passes IDs of projects belonging
     * to another user and expects the <code>AuthorizationFailedFault</code> to be raised by service.</p>
     *
     * @throws Exception if an unexpected error occurs.
     */
    public void testDeleteProject_User_AlienProjects() throws Exception {
        ProjectServiceFacade client = createWebServiceClient(USER_HANDLE);
        for (ProjectData expectedProject : this.adminProjects) {
            try {
                client.deleteProject(expectedProject.getProjectId());
                Assert.fail("AuthorizationFailedFault should have been thrown");
            } catch (AuthorizationFailedFault e) {
                // Expected behavior
            }
    ...
    /**
     * <p>Failure test. Tests the {@link ProjectServiceFacade#deleteProject(long)} functionality for proper handling
     * the case when a call is made for deleteing the project which the user is granted access permission for and which
     * has a competition associated with it. </p>
     *
     * <p>Authenticates to web service as user granted <code>Cockpit User</code> role, passes IDs of projects belonging
     * to user and having the competitions associated and expects the <code>ProjectHasCompetitionsFault</code> to be
     * raised by service.</p>
     *
     * @throws Exception if an unexpected error occurs.
     */
    public void testDeleteProject_User_OwnProjectsWithCompetition() throws Exception {
        Map<Long, Long> competitions = getCompetitions(USER_ID);
        ProjectServiceFacade client = createWebServiceClient(USER_HANDLE);
        for (ProjectData project : this.userProjects) {
            long projectId = project.getProjectId();
            if (competitions.containsKey(projectId)) {
                try {
                    ...
                    Assert.fail("ProjectHasCompetitionsFault should have been thrown");
                } catch (ProjectHasCompetitionsFault e) {
                    // Expected behavior
                    Assert.assertTrue("The project was deleted from database", projectExists(projectId));
                }
            }
        }
    }
     ...
     *
     * <p>Authenticates to web service as user granted <code>Cockpit Administrator</code> role, passes IDs of projects
     * (with no associated competitions) belonging to user and verifies that the service deletes existing project data
     * correctly.</p>
     *
     * @throws Exception if an unexpected error occurs.
     */
    public void testDeleteProject_Admin_OwnProject() throws Exception {
        Map<Long, Long> competitions = getCompetitions(ADMIN_ID);
        ProjectServiceFacade client = createWebServiceClient(ADMIN_HANDLE);
        for (ProjectData project : this.adminProjects) {
            long projectid = project.getProjectId();
            if (!competitions.containsKey(projectid)) {
                boolean result = client.deleteProject(projectid);
                Assert.assertTrue("The project is not reported to be deleted successfully", result);
     ...
     *
     * <p>Authenticates to web service as user granted <code>Cockpit Administrator</code> role, passes IDs of projects
     * (with no associated competitions) belonging to another user and verifies that the service deletes existing
     * project data correctly.</p>
     *
     * @throws Exception if an unexpected error occurs.
     */
    public void testDeleteProject_Admin_AlienProject() throws Exception {
        Map<Long, Long> competitions = getCompetitions(USER_ID);
        ProjectServiceFacade client = createWebServiceClient(ADMIN_HANDLE);
        for (ProjectData project : this.userProjects) {
            long projectid = project.getProjectId();
     ...
     *
     * <p>Authenticates to web service as user granted <code>Cockpit Administrator</code> role, passes ID of
     * non-existing project and verifies that the service handles this case correctly.</p>
     *
     * @throws Exception if an unexpected error occurs.
     */
    public void testDeleteProject_Admin_NonExistingProject() throws Exception {
        ProjectServiceFacade client = createWebServiceClient(ADMIN_HANDLE);
        boolean result = client.deleteProject(-92427L);
        Assert.assertFalse("The project is reported to be deleted successfully", result);
    }
     ...
     *
     * <p>Authenticates to web service as user granted <code>Cockpit Administrator</code> role, passes IDs of projects
     * belonging to user and having the competitions associated and expects the <code>ProjectHasCompetitionsFault</code>
     * to be raised by service.</p>
     *
     * @throws Exception if an unexpected error occurs.
     */
    public void testDeleteProject_Admin_OwnProjectsWithCompetition() throws Exception {
        Map<Long, Long> competitions = getCompetitions(ADMIN_ID);
        ProjectServiceFacade client = createWebServiceClient(ADMIN_HANDLE);
        for (ProjectData project : this.adminProjects) {
            long projectId = project.getProjectId();
     ...
     *
     * <p>Authenticates to web service as user granted <code>Cockpit Administrator</code> role, passes IDs of projects
     * belonging to another user and having the competitions associated and expects the
     * <code>ProjectHasCompetitionsFault</code> to be raised by service.</p>
     *
     * @throws Exception if an unexpected error occurs.
     */
    public void testDeleteProject_Admin_AlienProjectsWithCompetition() throws Exception {
        Map<Long, Long> competitions = getCompetitions(USER_ID);
        ProjectServiceFacade client = createWebServiceClient(ADMIN_HANDLE);
        for (ProjectData project : this.userProjects) {
            long projectId = project.getProjectId();
    ...
    /**
     * <p>Accuracy test. Tests the {@link ProjectServiceFacade#updateProject(ProjectData)} functionality for accurate
     * behavior.</p>
     *
    ...
    public void testUpdateProject_User_OwnProjects() throws Exception {
        ...
        for (ProjectData expectedProject : this.userProjects) {
            ...
            client.updateProject(expectedProject);
    ...
    /**
     * <p>Failure test. Tests the {@link ProjectServiceFacade#updateProject(ProjectData)} functionality for proper
     * handling the case when a call is made for updating details for the project which the user is not granted access
     * permission for.</p>
     *
     * <p>Authenticates to web service as user granted <code>Cockpit User</code> role, passes updated data for projects
    ...
    /**
     * <p>Failure test. Tests the {@link ProjectServiceFacade#updateProject(ProjectData)} functionality for proper
     * handling the case when a call is made for updating details for the non-existing project.</p>
     *
    ...
    /**
     * <p>Accuracy test. Tests the {@link ProjectServiceFacade#createProject(ProjectData)} functionality for accurate
     * behavior.</p>
     *
    ...
    public void testCreateProject_User() throws Exception {
        ...
        expectedProject.setName(expectedProject.getName() + " Updated");
        expectedProject.setDescription(expectedProject.getDescription() + " Updated");
        expectedProject = client.createProject(expectedProject);
    ...
    /**
     * <p>Failure test. Tests the {@link ProjectServiceFacade#createProject(ProjectData)} functionality for proper
     * handling the invalid input.</p>
     *
     * <p>Authenticates to web service as user granted <code>Cockpit User</code> role, passes <code>null</code> as
     * project and expects the <code>IllegalArgumentFault</code> to be raised by service.</p>
