...
public class StartSpecificationReviewAction extends SpecificationReviewAction {
    ...
    public String execute() {
        try {
            ...
            // determine the start date for the specification review based on start mode
            Date startDate = null;
            if (startMode.equals(START_MODE_NOW)) {
                // use current date for start date
                startDate = new Date();
            } else {
                // determine the start date based on contest type
                if (isStudio()) {
                    StudioCompetition contest = contestServiceFacade.getContest(getTCSubject(), getContestId());
                    startDate = contest.getStartTime().toGregorianCalendar().getTime();
                } else {
                    SoftwareCompetition contest =
                        contestServiceFacade.getSoftwareContestByProjectId(getTCSubject(), getContestId());
                    startDate = contest.getProjectData().getStartDate();
                }

                // decrement the start date by proper number of minutes
                GregorianCalendar gregorianStartDate = new GregorianCalendar();
                gregorianStartDate.setTime(startDate);
                gregorianStartDate.add(Calendar.MINUTE, -specificationReviewStartPeriodBeforeContest);
                startDate = gregorianStartDate.getTime();
            }

            // schedule the specification review using the start date
            specificationReviewService.scheduleSpecificationReview(getTCSubject(), getContestId(), startDate);
