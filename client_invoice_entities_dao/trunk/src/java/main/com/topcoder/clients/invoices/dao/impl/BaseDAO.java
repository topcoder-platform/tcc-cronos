...
public abstract class BaseDAO {
    ...
    protected void checkInit() {
        ...
        if (loggerName != null) {
            ...
            if (loggerName.trim().length() == 0) {
                throw new InvoiceDAOInitializationException("The 'loggerName' property must be not empty string");
            }
     ...
     */
    protected Session getSession() throws InvoiceDAOException {
        try {
            // return current session from session factory
            return sessionFactory.getCurrentSession();
        } catch (HibernateException e) {
            throw new InvoiceDAOException("Could not retrieve hibernate session.", e);
        }
    }
    ...
    protected <T extends IdentifiableEntity> List<T> getAllEntities(Class<T> entityType) throws InvoiceDAOException {
        ...
        try {
            ...
            throw new InvoiceDAOException(MessageFormat.format("Error occurred when retrieving entities of {0}",
