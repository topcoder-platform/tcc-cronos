...
js.topcoder.ajax.BufferedAjaxProcessor = function(parameters) {
    ...
    /**
     * <p>
     * Represents the array holding the sending data. Each elements in this array will represent the data to be sent for each request.
     * </p>
     * <p>
     * Can not be null, can not contain null element. Can be empty.
     * </p>
     * <p>
     * Initialized in constructor to empty array. Used by #sendRequest, and changed by #addSendingData.
     * </p>
     *
     * @private
     * @type Array
    ...
    /**
     * <p>
     * Represents the beginning to aggregate data.
     * </p>
     * <p>
     * It will set to true if this is the beginning to aggregate data. Otherwise, set to false. Set to true
     * initially.
     * </p>
     *
     * @private
     * @type boolean
     */
    var _startAggregate = true;

    /**
     * <p>Get the accumulative mode of the buffered processor.</p>
     *
     * @returns {boolean} if the accumulative mode is on, return true; otherwise, return false.
     */
    this.isAccumulative = function () {
        return _parameters.accumulative;
    }

    /**
     * <p>Set the accumulative mode of the buffered processor.</p>
     *
     * @param {boolean} accumulative the accumulative mode to set. If it's undefined/null,it means accumulative is false.
     * @throws js.topcoder.ajax.IllegalArgumentException if the parameter is not a boolean.
     */
    this.setAccumulative = function (accumulative) {
        if ((accumulative != null) && (typeof(accumulative) != "boolean")) {
            throw new js.topcoder.ajax.IllegalArgumentException("accumulative", "The accumulative parameter should be a boolean instance");
        }
        if (accumulative == null) {
            accumulative = false;
        }
        if (accumulative && (!_parameters.accumulative)) {
            _startAggregate = true;
        }
        _parameters.accumulative = accumulative;
    }
    ...
    function sendRequest() {
        if (_remainingRetries < _parameters.retryCount) {
            ...
            if (_sendingDataArray.length != 0) {
                _parameters.sendingData = _sendingDataArray.shift();
                _ajaxProcessor.request(_parameters);
            } else {
                _running = false;
            }
    ...
    this.addSendingData = function(sendingData) {
        ...
        if (_parameters.accumulative == true) {
            ...
            if (_startAggregate || (_sendingDataArray.length == 0)) {
                //set the _startAggregate to false
                _startAggregate = false;
                // aggregate data.
                if (currentSendingDataType == "jsonObject") {
                    //wrap the sendingData to an array, and insert it into _sendingDataArray
                    _sendingDataArray.push([sendingData]);
                } else {
                    _sendingDataArray.push(sendingData);
                }
            } else {
                var lastAggregatedData = _sendingDataArray.pop();
                if (currentSendingDataType == "jsonObject") {
                    lastAggregatedData.push(sendingData);
                    ...
                    for (var idx = 0; idx < sendingData.childNodes.length; idx++) {
                        lastAggregatedData.appendChild(sendingData.childNodes[idx]);
                    }
                } else {
                    lastAggregatedData = lastAggregatedData + "&" + sendingData;
                }
                _sendingDataArray.push(lastAggregatedData);
            }
        } else {
