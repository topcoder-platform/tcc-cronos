-- SQL file for Informix database, sequences added according to the only submission
...
FOREIGN KEY(document_id) REFERENCES document(document_id), -- foreign key to document table
FOREIGN KEY(document_name_id) REFERENCES document_name(document_name_id) -- foreign key to document_name table
);
...
FOREIGN KEY(document_version_id) REFERENCES document_version(document_version_id) -- foreign key to document_version table
);
...
FOREIGN KEY(competition_document_id) REFERENCES competition_document(competition_document_id) -- foreign key to competition document table
);

CREATE SEQUENCE CompetitionDocumentSeq MINVALUE 1 START WITH 1 INCREMENT BY 1;
CREATE SEQUENCE DocumentSeq MINVALUE 1 START WITH 1 INCREMENT BY 1;
CREATE SEQUENCE DocumentNameSeq MINVALUE 1 START WITH 1 INCREMENT BY 1;
CREATE SEQUENCE DocumentVersionSeq MINVALUE 1 START WITH 1 INCREMENT BY 1;
CREATE SEQUENCE MemberAnswerSeq MINVALUE 1 START WITH 1 INCREMENT BY 1;