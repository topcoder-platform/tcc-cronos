/*
 * Copyright (C) 2008 TopCoder Inc., All Rights Reserved.
 */
package com.topcoder.clientcockpit.phases;

import java.util.Date;
import java.util.HashSet;
import java.util.Set;

import com.topcoder.clientcockpit.phases.TestHelper.CockpitContestStatus;
import com.topcoder.clientcockpit.phases.TestHelper.CockpitPhaseType;
import com.topcoder.project.phases.Phase;
import com.topcoder.project.phases.PhaseStatus;
import com.topcoder.service.studio.contest.Contest;
import com.topcoder.service.studio.submission.ContestResult;
import com.topcoder.service.studio.submission.Submission;

/**
 * <p>
 * Accuracy tests for <code>ActionRequiredPhaseHandler</code>.
 * </p>
 *
 * @author TCSDEVELOPER
 * @version 1.0
 */
public class ActionRequiredPhaseHandlerTestAcc extends BaseTestCase {

    /**
     * <p>
     * Test constructor {@link ActionRequiredPhaseHandler#ActionRequiredPhaseHandler()}.
     * </p>
     *
     * <p>
     * The instance of <code>ActionRequiredPhaseHandler</code> should be created.
     * </p>
     *
     * @throws Exception to JUnit.
     */
    public void testActionRequiredPhaseHandler_Ctor1() throws Exception {
        assertNotNull("ActionRequiredPhaseHandler should be created.",
            new ActionRequiredPhaseHandler());
    }

    /**
     * <p>
     * Test constructor {@link ActionRequiredPhaseHandler#ActionRequiredPhaseHandler(
     * com.topcoder.service.studio.contest.ContestManager)}.
     * </p>
     *
     * <p>
     * The instance of <code>ActionRequiredPhaseHandler</code> should be created.
     * </p>
     *
     * @throws Exception to JUnit.
     */
    public void testActionRequiredPhaseHandler_Ctor2() throws Exception {
        assertNotNull("ActionRequiredPhaseHandler should be created.",
            new ActionRequiredPhaseHandler(TestHelper.CONTEST_MANAGER));
    }

    /**
     * <p>
     * Test constructor {@link ActionRequiredPhaseHandler#ActionRequiredPhaseHandler(String)}.
     * </p>
     *
     * <p>
     * The instance of <code>ActionRequiredPhaseHandler</code> should be created.
     * </p>
     *
     * @throws Exception to JUnit.
     */
    public void testActionRequiredPhaseHandler_Ctor3() throws Exception {
        assertNotNull("ActionRequiredPhaseHandler should be created.",
            new ActionRequiredPhaseHandler(TestHelper.ANOTHER_NAMESPACE));
    }

    /**
     * <p>
     * Test constructor {@link ActionRequiredPhaseHandler#ActionRequiredPhaseHandler(
     * String, com.topcoder.service.studio.contest.ContestManager)}.
     * </p>
     *
     * <p>
     * The instance of <code>ActionRequiredPhaseHandler</code> should be created.
     * </p>
     *
     * @throws Exception to JUnit.
     */
    public void testActionRequiredPhaseHandler_Ctor4() throws Exception {
        assertNotNull("ActionRequiredPhaseHandler should be created.",
            new ActionRequiredPhaseHandler(TestHelper.ANOTHER_NAMESPACE, TestHelper.CONTEST_MANAGER));
    }

    /**
     * <p>
     * Test method {@link ActionRequiredPhaseHandler#canPerform(com.topcoder.project.phases.Phase)}.
     * </p>
     *
     * <p>
     * The phase type does not match, the phase can not be performed.
     * </p>
     *
     * @throws Exception to JUnit.
     */
    public void testCanPerform_PhaseTypeNotMatch() throws Exception {
        //PhaseStatus.SCHEDULED means the phase is about to start
        Phase phase = this.createPhase(PhaseStatus.SCHEDULED, CockpitPhaseType.NO_SUCH_PHASE);

        //Set contest attribute
        phase.getProject().setAttribute(TestHelper.PROJECT_ATTR_CONTEST,
            this.createContest(null, null, null, null));

        assertFalse("The ActionRequired phase can not be performed.",
            this.getActionRequiredPhaseHandler().canPerform(phase));
    }

    /**
     * <p>
     * Test method {@link ActionRequiredPhaseHandler#canPerform(com.topcoder.project.phases.Phase)}.
     * </p>
     *
     * <p>
     * The phase has already been closed, the phase can not be performed.
     * </p>
     *
     * @throws Exception to JUnit.
     */
    public void testCanPerform_PhaseAlreadyClosed() throws Exception {
        Phase phase = this.createPhase(PhaseStatus.CLOSED, CockpitPhaseType.NO_SUCH_PHASE);

        //Set contest attribute
        phase.getProject().setAttribute(TestHelper.PROJECT_ATTR_CONTEST,
            this.createContest(null, null, null, null));

        assertFalse("The ActionRequired phase can not be performed.",
            this.getActionRequiredPhaseHandler().canPerform(phase));
    }

    /**
     * <p>
     * Test method {@link ActionRequiredPhaseHandler#canPerform(com.topcoder.project.phases.Phase)}.
     * </p>
     *
     * <p>
     * The minimum number of submissions has been met, the contest end date has been reached,
     * the ActionRequired phase can start.
     * </p>
     *
     * @throws Exception to JUnit.
     */
    public void testCanPerform_CanStart() throws Exception {
        //PhaseStatus.SCHEDULED means the phase is about to start
        Phase phase = this.createPhase(PhaseStatus.SCHEDULED, CockpitPhaseType.ACTION_REQUIRED);

        Contest contest = this.createContest(new Date(System.currentTimeMillis() - 2 * ONE_HOUR),
                new Date(System.currentTimeMillis() - ONE_HOUR), null, null);
        //Set contest attribute
        phase.getProject().setAttribute(TestHelper.PROJECT_ATTR_CONTEST, contest);

        //Set minimum number of submissions to 1
        phase.getProject().setAttribute(TestHelper.PROJECT_ATTR_MINIMUM_SUBMISSIONS, 1);

        @SuppressWarnings("unchecked")
        Set < Submission > submissions = new HashSet();
        submissions.add(new Submission());
        contest.setSubmissions(submissions);

        assertTrue("The ActionRequired phase can start.", this.getActionRequiredPhaseHandler().canPerform(phase));
    }

    /**
     * <p>
     * Test method {@link ActionRequiredPhaseHandler#canPerform(com.topcoder.project.phases.Phase)}.
     * </p>
     *
     * <p>
     * The minimum number of submissions has not been met,
     * the ActionRequired phase can not start.
     * </p>
     *
     * @throws Exception to JUnit.
     */
    public void testCanPerform_CanNotStart_1() throws Exception {
        //PhaseStatus.SCHEDULED means the phase is about to start
        Phase phase = this.createPhase(PhaseStatus.SCHEDULED, CockpitPhaseType.ACTION_REQUIRED);

        Contest contest = this.createContest(new Date(System.currentTimeMillis() - 2 * ONE_HOUR),
                new Date(System.currentTimeMillis() - ONE_HOUR), null, null);
        //Set contest attribute
        phase.getProject().setAttribute(TestHelper.PROJECT_ATTR_CONTEST, contest);

        //Set minimum number of submissions to 2
        phase.getProject().setAttribute(TestHelper.PROJECT_ATTR_MINIMUM_SUBMISSIONS, 2);

        @SuppressWarnings("unchecked")
        Set < Submission > submissions = new HashSet();
        submissions.add(new Submission());
        contest.setSubmissions(submissions);

        assertFalse("The ActionRequired phase can not start.", this.getActionRequiredPhaseHandler().canPerform(phase));
    }

    /**
     * <p>
     * Test method {@link ActionRequiredPhaseHandler#canPerform(com.topcoder.project.phases.Phase)}.
     * </p>
     *
     * <p>
     * The contest end date has not been reached,
     * the ActionRequired phase can not start.
     * </p>
     *
     * @throws Exception to JUnit.
     */
    public void testCanPerform_CanNotStart_2() throws Exception {
        //PhaseStatus.SCHEDULED means the phase is about to start
        Phase phase = this.createPhase(PhaseStatus.SCHEDULED, CockpitPhaseType.ACTION_REQUIRED);

        Contest contest = this.createContest(new Date(System.currentTimeMillis() + ONE_HOUR),
                new Date(System.currentTimeMillis() + 2 * ONE_HOUR), null, null);
        //Set contest attribute
        phase.getProject().setAttribute(TestHelper.PROJECT_ATTR_CONTEST, contest);

        //Set minimum number of submissions to 1
        phase.getProject().setAttribute(TestHelper.PROJECT_ATTR_MINIMUM_SUBMISSIONS, 1);

        @SuppressWarnings("unchecked")
        Set < Submission > submissions = new HashSet();
        submissions.add(new Submission());
        contest.setSubmissions(submissions);

        assertFalse("The ActionRequired phase can not start.", this.getActionRequiredPhaseHandler().canPerform(phase));
    }

    /**
     * <p>
     * Test method {@link ActionRequiredPhaseHandler#canPerform(com.topcoder.project.phases.Phase)}.
     * </p>
     *
     * <p>
     * Less than 24 hours remains before the Contest.winnerAnnouncementDeadline,
     * the ActionRequired phase can end.
     * </p>
     *
     * @throws Exception to JUnit.
     */
    public void testCanPerform_CanEnd_1() throws Exception {
        //PhaseStatus.OPEN means the phase is about to end
        Phase phase = this.createPhase(PhaseStatus.OPEN, CockpitPhaseType.ACTION_REQUIRED);

        Contest contest = this.createContest(new Date(System.currentTimeMillis() - 2 * ONE_HOUR),
                new Date(System.currentTimeMillis() - ONE_HOUR),
                new Date(System.currentTimeMillis() + ONE_HOUR), null);
        //Set contest attribute
        phase.getProject().setAttribute(TestHelper.PROJECT_ATTR_CONTEST, contest);

        assertTrue("The ActionRequired phase can end.", this.getActionRequiredPhaseHandler().canPerform(phase));
    }

    /**
     * <p>
     * Test method {@link ActionRequiredPhaseHandler#canPerform(com.topcoder.project.phases.Phase)}.
     * </p>
     *
     * <p>
     * The client has chosen a winner (Contest.results is not empty),
     * the ActionRequired phase can end.
     * </p>
     *
     * @throws Exception to JUnit.
     */
    public void testCanPerform_CanEnd_2() throws Exception {
        //PhaseStatus.OPEN means the phase is about to end
        Phase phase = this.createPhase(PhaseStatus.OPEN, CockpitPhaseType.ACTION_REQUIRED);

        final int moreThan24 = 25;
        Contest contest = this.createContest(new Date(System.currentTimeMillis() - 2 * ONE_HOUR),
                new Date(System.currentTimeMillis() - ONE_HOUR),
                new Date(System.currentTimeMillis() + moreThan24 * ONE_HOUR), null);
        //Set contest attribute
        phase.getProject().setAttribute(TestHelper.PROJECT_ATTR_CONTEST, contest);

        @SuppressWarnings("unchecked")
        Set < ContestResult > results = new HashSet();
        results.add(new ContestResult());
        contest.setResults(results);

        assertTrue("The ActionRequired phase can end.", this.getActionRequiredPhaseHandler().canPerform(phase));
    }

    /**
     * <p>
     * Test method {@link ActionRequiredPhaseHandler#canPerform(com.topcoder.project.phases.Phase)}.
     * </p>
     *
     * <p>
     * The client has not chosen a winner, and more than 24 hours remains before the Contest.winnerAnnouncementDeadline,
     * the ActionRequired phase can not end.
     * </p>
     *
     * @throws Exception to JUnit.
     */
    public void testCanPerform_CanNotEnd() throws Exception {
        //PhaseStatus.OPEN means the phase is about to end
        Phase phase = this.createPhase(PhaseStatus.OPEN, CockpitPhaseType.ACTION_REQUIRED);

        final int moreThan24 = 25;
        Contest contest = this.createContest(new Date(System.currentTimeMillis() - 2 * ONE_HOUR),
                new Date(System.currentTimeMillis() - ONE_HOUR),
                new Date(System.currentTimeMillis() + moreThan24 * ONE_HOUR), null);
        //Set contest attribute
        phase.getProject().setAttribute(TestHelper.PROJECT_ATTR_CONTEST, contest);


        assertFalse("The ActionRequired phase can not end.", this.getActionRequiredPhaseHandler().canPerform(phase));
    }


    /**
     * <p>
     * Test method {@link ActionRequiredPhaseHandler#perform(Phase, String)}.
     * </p>
     *
     * <p>
     * The ActionRequired phase starts, start email should be sent,
     * contest status should be updated to "ActionRequired".
     * </p>
     *
     * @throws Exception to JUnit.
     */
    public void testPerform_Start() throws Exception {

        //PhaseStatus.SCHEDULED means the phase is about to start
        Phase phase = this.createPhase(PhaseStatus.SCHEDULED, CockpitPhaseType.ACTION_REQUIRED);

        Contest contest = this.createContest(null, null, null, null);

        this.populateAttributes(phase, contest);

        //Mock ContestManager.getAllContestStatuses()
        this.mockContestManager(false);

        this.startSMTPServer();

        try {
            this.getActionRequiredPhaseHandler().perform(phase, "operator");

            //Assert start email is sent
            this.assertStartEmailSent();

            //Assert the contest status is updated
            assertEquals("Contest status should be updated", contest.getStatus(),
                CockpitContestStatus.ACTION_REQUIRED);
        } finally {
            this.stopSMTPServer();
        }
    }
    /**
     * <p>
     * Test method {@link ActionRequiredPhaseHandler#perform(Phase, String)}.
     * </p>
     *
     * <p>
     * The ActionRequired phase ends, end email should be sent, contest status should NOT be updated.
     * </p>
     *
     * @throws Exception to JUnit.
     */
    public void testPerform_End() throws Exception {

        //PhaseStatus.OPEN means the phase is about to end
        Phase phase = this.createPhase(PhaseStatus.OPEN, CockpitPhaseType.ACTION_REQUIRED);

        Contest contest = this.createContest(null, null, null, CockpitContestStatus.ACTION_REQUIRED);

        this.populateAttributes(phase, contest);

        //Mock ContestManager.getAllContestStatuses()
        this.mockContestManager(false);

        this.startSMTPServer();

        try {
            this.getActionRequiredPhaseHandler().perform(phase, "operator");

            //Assert end email is sent
            this.assertEndEmailSent();

            //Assert the contest status is not changed
            assertEquals("Contest status should not be changed", contest.getStatus(),
                    CockpitContestStatus.ACTION_REQUIRED);
        } finally {
            this.stopSMTPServer();
        }
    }
}
