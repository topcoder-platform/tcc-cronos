...
/**
 * This interface defines the contract that any project persistence must implement. The
 * implementation classes will be used by ProjectManagerImpl to perform project persistence
 * operations. The implementation classes should have a constructor that receives a namespace string
 * parameter so that they're exchangeable with each other by changing configuration settings in the
 * manager.
 * <p>
...
public interface ProjectPersistence {
    ...
    /**
     * Retrieves the project instance from the persistence given its id. The project instances is
     * retrieved with its properties.
