...
public class DefaultProjectPilot implements ProjectPilot {
    ...
    /**
     * <p>
     * Represents the default logger name to be used for auditing. Referenced in ctor().
     * </p>
     */
    public static final String DEFAULT_LOG_NAME = DefaultProjectPilot.class.getName();
    ...
    /**
     * <p>
     * Represents the logger used to do auditing whenever a phase is started/ended. The audit log
     ...
     * @throws ConfigurationException if any error occurs instantiating the object factory or the
     *             phase manager instance or the logger
     */
    public DefaultProjectPilot(String namespace, String phaseManagerKey,
        ...
        Log tmpLogger;
        try {
            tmpLogger = LogFactory.getLogWithExceptions(logName);
        } catch (LogException e) {
            throw new ConfigurationException("fail to get log with '" + logName + "'", e);
        }
    ...
    public DefaultProjectPilot(PhaseManager phaseManager, String scheduledStatusName,
        String openStatusName, Log logger) {
        // Check arguments.
    ...
    /**
     * <p>
     * Return the logger instance used to do auditing of phase start/end.
     * </p>
     * @return A non-null logger instance used to audit phase change
     */
     ...
     * @throws PhaseOperationException if any error occurs while ending/starting a phase or error
     *             retrieving project phases
     */
    public AutoPilotResult advancePhases(long projectId, String operator)
        ...
        try {
            ...
            throw new PhaseOperationException(projectId, null,
                "fail to get phases cause of phase manager exception", e);
        }

        // Process phases until no phases changed.
        boolean isPhaseChanged = true;
        while (isPhaseChanged) {
            int prevCountEnd = countEnd;
            int prevCountStart = countStart;
            set.clear();
            ...
            // Phases not changed. End the loop.
            if (prevCountEnd == countEnd && prevCountStart == countStart) {
     ...
     * can be started. Phase changes should be audited using the logger and operator name.
     * </p>
