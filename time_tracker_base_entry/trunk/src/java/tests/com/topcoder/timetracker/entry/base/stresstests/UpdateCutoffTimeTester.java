...
/**
 * This thread is used to test the updateCutoffTime method.
 *
 * @author vividmxx
 * @version 3.2
 */
public class UpdateCutoffTimeTester extends Thread {
    ...
    /**
     * Creates an UpdateCutoffTimeTester instance.
     *
     * @param bean
     ...
     */
    public UpdateCutoffTimeTester(EntrySessionBean bean, CutoffTimeBean[] cutoffTimeBeans) {
        this.bean = bean;
    ...
    public void run() {
        for (int i = 0; i < StressTestHelper.RUN_NUMBER / StressTestHelper.THREAD_NUMBER; i++) {
            try {
                bean.updateCutoffTime(cutoffTimeBeans[i], true);
            } catch (Exception e) {
                failed = true;
            }
        }
