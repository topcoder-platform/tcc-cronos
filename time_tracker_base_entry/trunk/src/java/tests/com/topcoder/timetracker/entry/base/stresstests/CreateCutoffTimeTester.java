...
/**
 * This thread is used to test the createCutoffTime method.
 *
 * @author vividmxx
 * @version 3.2
 */
public class CreateCutoffTimeTester extends Thread {
    ...
    /**
     * Creates a CreateCutoffTimeTester instance.
     *
     * @param bean
     ...
     */
    public CreateCutoffTimeTester(EntrySessionBean bean, CutoffTimeBean[] cutoffTimeBeans) {
        this.bean = bean;
    ...
    public void run() {
        for (int i = 0; i < StressTestHelper.RUN_NUMBER / StressTestHelper.THREAD_NUMBER; i++) {
            try {
                bean.createCutoffTime(cutoffTimeBeans[i], true);
            } catch (Exception e) {
                failed = true;
            }
        }
