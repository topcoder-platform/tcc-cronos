...
/**
 * <p>
 * This is a convenience class that may be used to build filters for performing searches in the RejectEmailDAO.
...
public class RejectEmailSearchBuilder {
    ...
    /**
     * <p>
     * This method is called to indicate that the built search filter should restrict the search to Reject Emails
     ...
     * RejectEmailDAO.SEARCH_COMPANY_ID as the field name and the provided value as the search value.
    ...
    public void hasCompanyId(long companyId) {
        addToFilter(new EqualToFilter(RejectEmailDAO.SEARCH_COMPANY_ID, new Long(companyId)));
    }
     ...
     * endDate or none of them are null. The field name to be used is RejectEmailDAO.SEARCH_CREATED_DATE. - If the
    ...
    public void createdWithinDateRange(java.util.Date startDate, java.util.Date endDate) {
        addToFilter(Utils.createRangerFilter(RejectEmailDAO.SEARCH_CREATED_DATE, startDate, endDate));
    }
     ...
     * RejectEmailDAO.SEARCH_CREATED_USER as the field name and the provided value as the search value. - If the
     * andFilter is not initialized, then initialize it with an AndFilter containing the built filter.
     * </p>
     *
     *
    ...
    public void createdByUser(String username) {
        ...
        addToFilter(new EqualToFilter(RejectEmailDAO.SEARCH_CREATED_USER, username));
    }
     ...
     * Implementation Notes: - Build the following filter and add it to the andFilter: -> The filter that is built
     * may either be a GreaterThanFilter, LessThanFilter or BetweenFilter depending on whether the startDate or
     * endDate or none of them are null. The field name to be used is RejectEmailDAO.SEARCH_MODIFICATION_DATE. - If
    ...
    public void modifiedWithinDateRange(Date startDate, Date endDate) {
        addToFilter(Utils.createRangerFilter(RejectEmailDAO.SEARCH_MODIFICATION_DATE, startDate, endDate));
    }
     ...
     * <p>
     * Implementation Notes: - Build the following filter and add it to the andFilter: -> EqualsFilter with
     * RejectEmailDAO.SEARCH_MODIFICATION_USER as the field name and the provided value as the search value. - If
     * the andFilter is not initialized, then initialize it with an AndFilter containing the built filter.
     * </p>
     *
     *
    ...
    public void modifiedByUser(String username) {
        Utils.checkString(username, "username", false);
        addToFilter(new EqualToFilter(RejectEmailDAO.SEARCH_MODIFICATION_USER, username));
    }
