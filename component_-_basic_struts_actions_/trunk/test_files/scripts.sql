...
alter table 'informix'.user_profile add constraint primary key
	(user_id) constraint pk_user_profile;
	
alter table 'informix'.user_profile add constraint foreign key
	(user_id)
...
insert into 'informix'.user_profile(user_id,first_name,last_name,middle_name,timezone_id) values (20, 'first_name', 'last_name',
'middle_name', 128);
...
drop trigger 'informix'.trig_audit_user;

database informixoltp;
create synonym "informix".user_profile for common_oltp:"informix".user_profile;
create synonym "informix".audit_record for common_oltp:"informix".audit_record;

database common_oltp;

-- change the view of email user
drop view 'informix'.email_user;

create view "informix".email_user (user_id, create_date,modify_date,handle,last_login,status,activation_code,email) as
   select x0.user_id, x0.create_date, x0.modify_date, x0.handle, x0.last_login, x0.status, x0.activation_code, x1.address 
...
-- change the procedure "informix".proc_user_update
drop procedure "informix".proc_user_update(decimal,varchar,varchar,varchar,varchar,varchar,varchar,varchar,varchar,varchar,varchar,varchar,varchar,decimal,decimal);
...
revoke execute on procedure proc_user_update(decimal,varchar,varchar,varchar,varchar,varchar,varchar) from 'public';

create trigger "informix".trig_audit_user update of handle,last_login,status,activation_code,last_site_hit_date on "informix".user referencing old as old new as new                                          for each row
    (
    execute procedure "informix".proc_user_update(old.user_id ,old.handle ,new.handle ,old.status ,new.status ,old.activation_code ,new.activation_code ));



database tcs_catalog;

-- change the view 'informix'.user_customer
drop view 'informix'.user_customer;
...
grant update(activation_code,city,company,country_code,email_address,login_id,postal_code,telephone_nbr,user_customer_id) on user_customer to 'informix' with grant option ;
