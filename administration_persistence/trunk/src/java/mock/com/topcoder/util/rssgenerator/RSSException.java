...
/**
 * Purpose: This class acts as the generic exception for this component and all exceptions (ignoring run-time exceptions) of this component must extend this exception. Applications need only catch this exception in case they aren't interested in dealing with specific exceptions (ignoring run-time exceptions). <p>Implementation: We extend BaseException so that we are provided with the cause and message properties. We add three constructors that allow construction with no args, with a message, and with a message as well as a cause.</p> <p>Thread Safety: This class is thread safe as it is has no state and its super class is also thread safe.</p>
 * 
 */
