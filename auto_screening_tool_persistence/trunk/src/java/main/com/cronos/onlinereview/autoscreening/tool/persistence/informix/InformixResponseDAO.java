 ...
 * to create an screening response.
 * </p>
 * <p>
 * Thread-Safety: Each invocation works on its own connection instance, and the connection is closed
 * after. Unlike InformixTaskDAO, the threading issues here are less, because this is only involved
 * in creating new instances.
 * </p>
 * @author ShindouHikaru, TCSDEVELOPER
 * @version 1.0
 */
public class InformixResponseDAO implements ScreeningResponseDAO {

	/**
	 * Represents the sql statement to create screening response onto the database.
	 */
	...
	/**
	 * Represents the argument types for the sql statement to create screening response.
	 */
	private static final DataType[] CREATE_SCREENING_RESPONSE_ARGUMENT_TYPES = new DataType[] {
			...
			DatabaseUtils.STRING_TYPE, DatabaseUtils.STRING_TYPE, DatabaseUtils.DATE_TYPE,
	...
	/**
	 * <p>
	 * This is the id generator that is used to provide ids to the created ScreeningResponses.
	 * </p>
	 */
	 ...
	 * connection name which is null will be used.
	 ...
	 */
	public InformixResponseDAO(DBConnectionFactory connectionFactory, IDGenerator idGenerator) {
	 ...
	 *            The id generator to use.
	 * @throws IllegalArgumentException
	 *             if the connectionFactory or idGenerator is null or if connectionName is an empty
	 *             String.
	 */
	public InformixResponseDAO(DBConnectionFactory connectionFactory, String connectionName,
		...
		if (idGenerator == null) {
			throw new IllegalArgumentException("idGenerator should not be null.");
		}
	 ...
	 * </p>
	 * <p>
	 * If the id of screeningResponse is not set (i.e. Long.MIN_VALUE), it will be set using the id
	 * generator; otherwise, the id that screeningResponse already has will be used.
	 * </p>
	...
	public void createScreeningResponse(ScreeningResponse screeningResponse) throws DAOException {
		if (screeningResponse == null) {
			throw new IllegalArgumentException("screeningResponse should not be null.");
		}

		// get the new Id for the screening response if the old one is not set
		long id = screeningResponse.getId() == Long.MIN_VALUE ? getNextID() : screeningResponse
				.getId();
		...
		Object[] queryArgs = new Object[] {
				...
				new Long(id),
				// task id
				DatabaseUtils.wrapLongValue(screeningResponse.getScreeningTaskId()),
				// response code id
				DatabaseUtils.wrapLongValue(screeningResponse.getResponseCodeId()),
				// detail message
				screeningResponse.getDetailMessage(),
				// create user, create date, modify user, modify date
				screeningResponse.getCreateUser(), screeningResponse.getCreateDate(),
				screeningResponse.getModificationUser(), screeningResponse.getModificationDate() };

		// execute the insert statement
		DatabaseUtils.doDMLQuery(connectionFactory, connectionName, CREATE_SCREENING_RESPONSE_SQL,
	...
	private long getNextID() throws DAOException {
		...
		try {
			...
			throw new DAOException("Unable to generate id for the screening response.", e);
		}
	}
}
