...
import com.cronos.onlinereview.autoscreening.tool.persistence.informix.DatabaseUtils.DataType;
 ...
 * datastore to load and update screening tasks.
 * </p>
 * <p>
 ...
 * @version 1.0
 */
public class InformixTaskDAO implements ScreeningTaskDAO {
    /**
    ...
    private static final String LOAD_SCREENING_TASKS_SQL = "SELECT "
            + "project.project_id, project.project_category_id, project_info.value, "
            ...
            + "WHERE resource_info.resource_info_type_id=1 AND project_info.project_info_type_id=5";
    ...
    private static final DataType[] LOAD_SCREENING_TASKS_COLUMN_TYPES = new DataType[] {
            ...
            DatabaseUtils.LONG_TYPE, DatabaseUtils.STRING_TYPE, DatabaseUtils.STRING_TYPE,
            DatabaseUtils.STRING_TYPE, DatabaseUtils.LONG_TYPE, DatabaseUtils.LONG_TYPE,
            DatabaseUtils.DATE_TYPE, DatabaseUtils.STRING_TYPE, DatabaseUtils.DATE_TYPE,
    ...
    private static final DataType[] UPDATE_SCREENING_TASK_ARGUMENT_TYPES = new DataType[] {
        ...
        /**
         * Represents the sql statement to update submission status onto the database.
         */
        private static final String UPDATE_SUBMISSION_STATUS_SQL = "UPDATE submission "
                        + "SET submission_status_id = ? "
                        + "WHERE upload_id = ?";
        
        /**
         * Represents the argument types for the sql statement to update submission status.
         */
        private static final DataType[] UPDATE_SUBMISSION_STATUS_ARGUMENT_TYPES = new DataType[] {
     ...
     * present, then the default connection is used. The connectionName can be null, but not empty
     * string.
     * </p>
     */
    ...
    /**
     * <p>
     * An instance of UserRetrieval that is used to retrieve submitter information.
     * </p>
     */
     ...
     */
    public InformixTaskDAO(DBConnectionFactory connectionFactory, UserRetrieval userRetrieval) {
     ...
     *             empty String.
     */
    public InformixTaskDAO(DBConnectionFactory connectionFactory, String connectionName,
        ...
        if (userRetrieval == null) {
            throw new IllegalArgumentException("userRetrieval should not be null.");
        }
    ...
    private int loadScreeningDataSequentially(ScreeningData screeningData, Object[] row,
        ...
        long projectCatalogId = DatabaseUtils.unwrapLongOjbect((Long) row[startIndex++]);
        ...
        try {
            ...
            // exceptions as well.
            throw new DAOException("Unable to get external user information for user id ["
                    + externalUserID + "].", e);
        }
        if (externalUser == null) {
            throw new DAOException("There is no personal information for user id ["
                    + externalUserID + "].");
        }
        ...
        try {
            ...
            screeningData.setProjectCatalogId(projectCatalogId);
            ...
            screeningData.setSubmitterAlternativeEmails(externalUser.getAlternativeEmails());
    ...
    public void updateScreeningTask(ScreeningTask screeningTask) throws DAOException {
        ...
        // update submission status if the auto screening fails
        if ("Failed".equals(screeningTask.getScreeningStatus().getName())) {
            long uploadId = screeningTask.getScreeningData().getUploadId();
            queryArgs = new Object[] {
                    // submission_status_id, 2 indicates "Failed Screening"
                    DatabaseUtils.wrapLongValue(2),
                    // upload id
                    DatabaseUtils.wrapLongValue(uploadId) };
            updateCount = DatabaseUtils.doDMLQuery(connectionFactory, connectionName,
                    UPDATE_SUBMISSION_STATUS_SQL, UPDATE_SUBMISSION_STATUS_ARGUMENT_TYPES,
                    queryArgs);
        }
    }
