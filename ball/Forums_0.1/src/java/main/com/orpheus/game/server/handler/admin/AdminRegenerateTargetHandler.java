...
import com.orpheus.game.TargetUpdateListener;
...
/**
 * <p>A custom {@link Handler} implementation to be used for re-generating the desired target from selected slot.</p>
 *
 * @author isv
 * @version 1.0
 */
public class AdminRegenerateTargetHandler extends AbstractGameServerHandler implements Handler {
    ...
    /**
     * <p>Constructs new <code>AdminRegenerateTargetHandler</code> instance initialized based on the configuration
     ...
     *         missing or invalid.
     */
    public AdminRegenerateTargetHandler(Element element) {
        if (element == null) {
        ...
        try {
            ...
            for (int i = 0; i < targets.length; i++) {
                if (targets[i].getSequenceNumber() == seqNumber) {
                    ...
                    TargetUpdateListener targetUpdateListener = (TargetUpdateListener) gameManager;
                    HttpUtility http = new HttpUtility(HttpUtility.GET);
                    String contents = http.execute(updatedTarget.getUriPath());
                    SiteStatistics siteStatistics
                        = GameDataUtility.getConfiguredSiteStatisticsInstance("SiteStatistics");
                    siteStatistics.accumulateFrom(contents, "document1");
                    newTarget = targetUpdateListener.targetUpdated(siteStatistics.getElementContentStatistics(),
                                                                   updatedTarget);
                    targets[i] = newTarget;
                }
            }
