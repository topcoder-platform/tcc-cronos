...
/**
 * <p>A custom {@link Handler} implementation to be used for retrieving the list of all games (completed, not started,
 * in-progress) which currently exist. Such a handler is provided as there is currently no such handler provided by any
 * of existing components.</p>
 *
 * @author isv
 * @version 1.0
 */
public class AllGamesHandler extends AbstractGameServerHandler implements Handler {
    ...
    /**
     * <p>Constructs new <code>AllGamesHandler</code> instance initialized based on the configuration parameters
     ...
     *         missing or invalid.
     */
    public AllGamesHandler(Element element) {
        if (element == null) {
    ...
    public String execute(ActionContext context) throws HandlerExecutionException {
        ...
        try {
            ...
            context.getRequest().setAttribute(getString(GAMES_ATTR_NAME_CONFIG),
                                              gameDataEJBAdapter.findGames(null, null));
            return null;
        } catch (Exception e) {
