 ...
 * confirming new or updated bid. The purpose of this handler is to obtain the details for hosting block, auction
 * and sponsor domain and image and put them to request. Currently there is no such handler provided by any of
 * existing components.</p>
 *
 * @author isv
 * @version 1.0
 */
public class SponsorConfirmBidPreHandler extends AbstractAuctionHandler implements Handler {
    ...
    /**
     * <p>Constructs new <code>SponsorConfirmBidPreHandler</code> instance initialized based on the configuration
     ...
     *         is missing.
     */
    public SponsorConfirmBidPreHandler(Element element) {
        if (element == null) {
    ...
    public String execute(ActionContext context) throws HandlerExecutionException {
        ...
        try {
            // Load details for sponsor domain and the hosting block
            Domain domain;
            HostingBlock block;
            if (getBoolean(USER_REMOTE_INTERFACE_CONFIG).booleanValue()) {
                GameData gameData = (GameData) getGameDataEJB(this.jndiContext);
                domain = gameData.getDomain(getLong(DOMAIN_ID_PARAM_NAME_CONFIG, request));
            ...
            for (int i = 0; (image == null) && (i < images.length); i++) {
                if (images[i].getId().longValue() == imageId) {
                    image = images[i];
                }
            }

            // Get details for requested auction
            ...
            request.setAttribute(UPDATE_BID_CONFIG, getBoolean(UPDATE_BID_CONFIG));
            return null;
        } catch (Exception e) {
