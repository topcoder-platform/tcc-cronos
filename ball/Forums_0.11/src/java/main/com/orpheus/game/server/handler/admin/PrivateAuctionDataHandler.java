 ...
 * placing new bid by administrator in context of private auction. The purpose of this handler is to obtain the details
 * for existing sponsor, their domains and images and put them to request.</p>
 *
 * @author isv
 * @version 1.0
 */
public class PrivateAuctionDataHandler extends AbstractGameServerHandler implements Handler {
    ...
    /**
     * <p>A <code>UserProfileManager</code> instance which will be used to search for domain sponsors.</p>
     */
    private final UserProfileManager userProfileManager;

    /**
     * <p>Constructs new <code>PrivateAuctionDataHandler</code> instance initialized based on the configuration parameters
     ...
     *         is missing.
     */
    public PrivateAuctionDataHandler(Element element) {
        if (element == null) {
            throw new IllegalArgumentException("The parameter [element] is NULL");
        }
        readAsString(element, SPONSORS_ATTR_NAME_CONFIG, true);
    ...
    public String execute(ActionContext context) throws HandlerExecutionException {
        ...
        try {
            // Get the sponsor profiles
            Map searchParams = new HashMap();
            searchParams.put("sponsor-is-approved", "Y");
            UserProfile[] sponsors = this.userProfileManager.searchUserProfiles(searchParams);
            ...
            DropDownDataImpl sponsorsDropDownData = new DropDownDataImpl();
            DropDownDataImpl domainsDropDownData = new DropDownDataImpl();
            DropDownDataImpl imagesDropDownData = new DropDownDataImpl();
            for (int i = 0; i < sponsors.length; i++) {
                UserProfile sponsor = sponsors[i];
                long sponsorId = ((Long) sponsor.getIdentifier()).longValue();
                Domain[] sponsorDomains = gameDataEjbAdapter.findDomainsForSponsor(sponsorId);
                DropDownItemImpl sponsorItem
                    = new DropDownItemImpl(String.valueOf(sponsorId), OrpheusFunctions.getHandle(sponsor));
                sponsorsDropDownData.addDropDownItem(sponsorItem);
                buildDomainsDropDownData(sponsorDomains, sponsorsDropDownData, domainsDropDownData);
                buildImagesDropDownData(sponsorDomains, domainsDropDownData, imagesDropDownData);
            }
            
            // Put all data to request
            request.setAttribute(getString(SPONSORS_ATTR_NAME_CONFIG), sponsorsDropDownData);
     ...
     * @param sponsorsDropDown a <code>DropDownData</code> providing the details for sponsors drop-down list.
     * @param data a <code>DropDownData</code> to be used for presenting the domains as drop-down list.
     */
    private static void buildDomainsDropDownData(Domain[] domains, DropDownDataImpl sponsorsDropDown,
                                                 DropDownDataImpl data) {
        DropDownItemImpl domain;
        for (int i = 0; i < domains.length; i++) {
            domain = new DropDownItemImpl(String.valueOf(domains[i].getId()), domains[i].getDomainName());
            domain.addParent(sponsorsDropDown.getDropDownItem(String.valueOf(domains[i].getSponsorId())));
     ...
     * @param data a <code>DropDownData</code> to be used for presenting the domains as drop-down list.
     */
    private static void buildImagesDropDownData(Domain[] domains, DropDownDataImpl domainsDropDown,
                                                DropDownDataImpl data) {
