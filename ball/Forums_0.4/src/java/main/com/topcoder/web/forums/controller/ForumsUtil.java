...
public class ForumsUtil {
    ...
    public static Hashtable getCollapseReadPostTable(User user, ForumThread thread, Date lastReadDate) {
        ...
        Iterator itMessages = thread.getMessages();
        while (itMessages.hasNext()) {
            ...
            table.put(message, Boolean.valueOf(collapseMessage));
            if (showRepliedPosts && !collapseMessage) {
                if (message.getParentMessage() != null &&
                        (table.get(message.getParentMessage()) == null || ((Boolean)table.get(message.getParentMessage())).booleanValue() == true)) {
                    message = message.getParentMessage();
                    table.put(message, Boolean.FALSE);
                }
            }
        }

        Iterator itValues = table.values().iterator();
        int numCollapsedPosts = 0;
        while (itValues.hasNext()) {
            if (((Boolean)itValues.next()).booleanValue() == true) {
                numCollapsedPosts++;
            }
        }
    ...
    public static ForumCategory getMasterCategory(ForumCategory category) throws ForumCategoryNotFoundException {
        ForumFactory masterFactory = ForumFactory.getInstance(new TCAuthToken(1));
        ...
        for (int i = 0; ca != null && i < ca.length; i++) {
            ...
            if (ca[i].getName().equals(cookieName)) {
                ...
                try {
                    ...
                    log.debug("--> username: " + username + " | pwd: " + pwd + " | hash: " + hash + " | hashCheck: " + hashCheck);
