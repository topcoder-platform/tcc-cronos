    ...
    public List<SimpleProjectContestData> getSimpleProjectContestDataForUser(
        ...
        try {
            ...
            String qstr="select p.project_id , p.name as pname, c.contest_id,  c.name as cname, "
                    ...
                    /* contest payment*/
                    /* contest payment*/
                    + " (select sum(cast(nvl(property_value, '0') as DECIMAL(10,2))) "
                    + "        +  sum(p.amount) "
                    + "        + nvl( sum(cast(m.amount*number_of_submissions as DECIMAL(10,2))), 0) "
                    + "  from contest_prize_xref x, prize p, contest_config cfg, contest cc  "
                    + "    left outer join contest_milestone_prize m on m.contest_milestone_prize_id = cc.contest_milestone_prize_id "
                    + "    where cfg.contest_id = c.contest_id and property_id = 25 "
                    + "    and x.prize_id = p.prize_id and x.contest_id = cfg.contest_id "
                    + "    and cc.contest_id = cfg.contest_id) as contest_payment "
