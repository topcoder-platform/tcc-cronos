...
/**
 * <p>
 * This class implements the <code>DocumentContentManager</code> interface, and it acts as
 * a socket client to save document content to a file server or get document content information
 * from a file server.
 * </p>
 *
 * <p>
...
public class SocketDocumentContentManager implements DocumentContentManager {
    /**
     * <p>The key for server address in the map attributes.</p>
     */
    private static final String SERVERADDRESS = "serverAddress";

    /**
     * <p>The key for server port in the map attributes.</p>
     */
    private static final String SERVERPORT = "serverPort";

    /**
     * <p>The count of bytes of a int value.</p>
     */
    private static final int INT_LENGTH = 4;

    /**
     * <p>The first byte value of existDocumentContent request.</p>
     */
    private static final byte COMMAND_EXIST = 3;

    /**
     * <p>
     * This field represents the server ip address. It's initialized in the constructor,
    ...
    /**
     * <p>
     * This field represents the server port. It's initialized in the constructor,
     * and never changed afterwards. It must be int value in range [0, 65535].
     * </p>
     */
    ...
    /**
     * <p>
     * Creates a <code>SocketDocumentContentManager</code> instance. Initializes the server
     * address and port from the passed-in map. The argument <code>attributes</code> should
     * contain two key-value pairs.<br/>
     * 1) "serverAddress" -- the value must be a non-null, non-empty string, it's used as
     * the server address for the manager.<br/>
     * 2) "serverPort" -- the value must be non-null java.lang.Integer object, whose value
     * should be in range [0, 65535], it's used as the server port for the manager.
     * </p>
     *
     * @param attributes a map of attributes to initialize fields
     *
     * @throws IllegalArgumentException if the argument is null, or
     *         if it doesn't contain any attribute mentioned above, or the attribute value is invalid,
     *         or it contains additional attributes.
     */
    public SocketDocumentContentManager(Map<String, Object> attributes) {
        ...
        if (!(obj instanceof String)) {
            throw new IllegalArgumentException(
                "The value for 'serverAddress' isn't instance of String.");
        }

        String address = (String) obj;
        Helper.checkString(address, "serverAddress");
        ...
        if (!(obj instanceof Integer)) {
            throw new IllegalArgumentException(
                "The value for 'serverPort' isn't instance of Integer.");
        }

        Integer port = (Integer) obj;
        Helper.checkPort(port.intValue());
     ...
     *         if serverAddress is null or empty string, or serverPort is not in range [0, 65535]
     */
    public SocketDocumentContentManager(String serverAddress, int serverPort) {
        Helper.checkString(serverAddress, "serverAddress");
        Helper.checkPort(serverPort);
     ...
     *
     * @throws IllegalArgumentException
     *         if any argument is null, or path argument is empty string, or documentContent arg is empty array.
     * @throws IOException
     ...
     *         (e.g. if the received response is invalid, or failure response is received. )
     */
    public void saveDocumentContent(String path, byte[] documentContent)
        ...
        try {
            ...
            if (array[0] == 0) {
                ...
                throw new DocumentContentManagementException(
                    "The first byte should be either 0 or 1.");
            }
        } finally {
            if (channel != null) {
                try {
    ...
    /**
     * <p>
     * Gets the content of document given by path. Returns null if the document content doesn't exist.
        ...
        try {
            ...
            if (array[0] == 0) {
                ...
                if (count < length) {
                    ...
                    throw new DocumentContentManagementException(
                        "The content has ending additional bytes.");
                }
    ...
    public boolean existDocumentContent(String path)
        ...
        try {
            ...
            if (array[0] == 0) {
                ...
                throw new DocumentContentManagementException(
                    "Only 0 and 1 is valid for exist flag.");
            } else {
                throw new DocumentContentManagementException(
                    "The first byte should be either 0 or 1.");
            }
     ...
     * @throws IOException if any error occurs
     */
    private void doCommand(byte command, String path, SocketChannel channel)
        throws IOException {
        // Encode the path using utf-8 format.
        ByteBuffer pathBuf = encodePath(path);
     ...
     */
    private void commonErrorHandling(int count, ByteBuffer buffer,
        ...
        if (count < size) {
            throw new DocumentContentManagementException(
                "The error message content doesn't have enough bytes.");
        } else if (count > size) {
            throw new DocumentContentManagementException(
                "The error message content has ending additional bytes.");
        }
     ...
     * @throws IOException if any io error occurs
     */
    private void specialErrorHandling(int count, ByteBuffer buffer,
        SocketChannel channel)
