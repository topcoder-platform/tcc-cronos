...
/**
 * <p>
 * This class represents the Time Entry entity. Extends <code>BaseDataObject</code>. Uses its primaryId member
 * for holding the TimeEntriesID defined in the database. This entity is used in <code>TimeEntryDAO</code>. As such,
 * when the <code>DAO</code> creates this record, it will ignore the creation and modification fields and assign its
 * own values. It will also ignore the primary Id vaue as it will assign its own. Simlarly, the <code>DAO</code>
 * will ignore all of these fields when updating. The creation fields and the primary fields are never updated,
 * and the <code>DAO</code> will assign its own values to the modification fields. As such, from the perspective of
 * the application using this entity, these fields can be considered to be effectively read-only (as assigning fresh
