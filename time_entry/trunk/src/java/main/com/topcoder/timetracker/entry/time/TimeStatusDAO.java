...
/**
 * <p>
 * This class extends <code>BaseDAO</code> and provides basic persistence functionality for the <code>TimeStatus</code>
...
public class TimeStatusDAO extends BaseDAO {
    ...
    /**
     * <p>
     * Creates a new instance of <code>TimeStatusDAO</code> class. Sets the connection name and namespace that
     ...
     */
    public TimeStatusDAO(String connName, String namespace) {
    ...
    protected void fillCreatePreparedStatement(PreparedStatement statement, DataObject dataObject, String creationUser,
        ...
        try {
            TimeStatus timeStatus = (TimeStatus) dataObject;
            ...
            int i = 0;
            statement.setInt(++i, timeStatus.getPrimaryId());
            statement.setString(++i, TimeEntryHelper.truncatesString(timeStatus.getDescription()));
    ...
    protected void setCreationParametersInDataObject(DataObject dataObject, String creationUser, Date creationDate)
        ...
        try {
            ...
            timeStatus.setCreationUser(creationUser);
            timeStatus.setModificationUser(creationUser);

            // set the creationDate and  modificationDate members of the dataObject
            timeStatus.setCreationDate(creationDate);
            timeStatus.setModificationDate(creationDate);
        } catch (Exception e) {
            // wrap any exception with DAOActionException
    ...
    protected void verifyUpdateDataObject(final DataObject dataObject) throws DAOActionException {
        this.validateDataObject(dataObject);
        if (((TimeStatus) dataObject).getCreationDate() == null) {
    ...
    /**
     * <p>
     * Returns the sql string that will be used in the prepared statement for updating a time status.
    ...
    /**
     * <p>
     * Returns the sql string that will be used in the prepared statement for deleting a time status.
    ...
    /**
     * <p>
     * Returns the sql string that will be used in the prepared statement for retrieving a time status record.
    ...
    /**
     * <p>
     * Retrieves all column fields from the ResultSet, and returns a <code>TimeStatus</code> instance containing
     * them. Returns null if no records were retrieved. If for some reason more than one record is retrieved, only
     * the first is used.
     * </p>
     *
    ...
    /**
     * <p>
     * Returns the sql string that will be used in the statement for retrieving selected time status records.
    ...
    /**
     * <p>
     * Retrieves all column fields from the ResultSet for each record, and returns a List of TimeStatus instances
     ...
     * description in the data Object is null.
     */
    private void validateDataObject(DataObject dataObject) {
        // check null
        TimeEntryHelper.checkNull(dataObject, "dataObject");

        // assert dataObject is the instance of TimeStatus
        if (!(dataObject instanceof TimeStatus)) {
            throw new IllegalArgumentException("dataObject is not instance of TimeStatus");
        }

        // only to check the decription field of the dataObject
        if (((TimeStatus) dataObject).getDescription() == null) {
