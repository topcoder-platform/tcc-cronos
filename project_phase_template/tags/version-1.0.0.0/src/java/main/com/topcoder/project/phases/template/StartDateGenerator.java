...
/**
 * <p>
 * <code>StartDateGenerator</code> interface defines the contract to generate
 * a default start date for a project according to a specific generation logic,
 * so that the <code>{@link DefaultPhaseTemplate}</code> can employ different
 * start date generation logic without code changes.
 * </p>
 * <p>
 * It will be used in the <code>{@link DefaultPhaseTemplate}</code> as the
 * default project start date generation logic if no start date specified in the
