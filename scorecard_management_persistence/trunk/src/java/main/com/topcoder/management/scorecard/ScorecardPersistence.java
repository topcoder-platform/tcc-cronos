...
/**
 * This interface defines the contract that any scorecard persistence must implenment. The implementation classes
 * will be used by ScoreCardManagerImpl to performs scorecard persistence operations. The implementation classes
 * should have a constructor that receive a namespace string parameter so that they're exchangeable with each other
 * by changing configuration settings in the manager.
 *
 */
public interface ScorecardPersistence {
    /**
     * Create the scorecard in the persistence using the given scorecard instance. The scorecard instance can
     * include sub items such as groups, sections and questions. Those sub items will be persisted as well. The
     * operator parameter is used as the creation/modification user of the scorecard and its subitems. The creation
     * date and modification date will be the current date time when the scorecard is created.
     *
     *
     * @param scorecard The scorecard instance to be created in the persistence.
    ...
    /**
     * Update the given scorecard instance into the persistence. The scorecard instance can include sub items such
     * as groups, sections and questions. Those sub items will be updated as well. If sub items are removed from
     * the scorecard, they will be deleted from the persistence. Likewise, if new sub items are added, they will be
     ...
     *
     *
     * @param scorecard The scorecard instance to be updated into the persistence.
     ...
     * @throws PersistenceException if error occurred while accessing the persistence.
     */
    Scorecard[] getScorecards(long[] ids, boolean complete) throws PersistenceException;
}
