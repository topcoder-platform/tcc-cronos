...
/**
 * <p>Exception used by the Rates component whenever there is a problem within a RateManager instance.
 * For example, if its pluggable persistence encounters an error, that will be re-thrown as a RateManager exception.
 * Two constructors are provided that both take a reason why the error occurs, and one taking the error which caused
 * this to be thrown.</p>
...
/**
 * Cause Constructor for the exception, taking a message detailing why the exception occurred as well as the initial
 * problem causing this to be raised.
 * @param message Descriptive reason the error occurred.
