...
/**
 * <p>
 * The <code>Review</code> class is the class at the root of the review model hierarchy. It represents an entire review,
 * which is composed primarily of a list of items. Comments can also be associated with a review, and each review also
 * possesses a few simple data fields. The <code>Review</code> class is simply a container for these basic data fields
 * (along with the comment and item lists). All data fields are mutable, and each data field has a three method
 * (get/set/reset) combination for that data field. Each of the comment and item lists is manipulated through nine
 * methods (two for adding, three for removing, one for clear, and three getters).
 * </p>
 * <p>
 ...
 * </p>
 * @author aubergineanode, vividmxx, saarixx, TCSDEVELOPER
 * @version 1.0
 */
public class Review implements Serializable {
     ...
     * </p>
     * <p>
     * The default (unassigned value) for this field is -1, which indicates that the id of the <code>Review</code> has
     * not yet been set through the constructor or a call to <code>setId</code> (or has been set back to this value by a
     * call to <code>resetId</code>). Positive values indicate that the client using the component has assigned an id
     * (in the constructor or through the <code>setId</code> method). It is expected that all <code>Review</code>
     ...
     * </p>
     * <p>
     * The author is only an identifier, this component does not define an abstract <code>Author</code> class, and it is
     * up to the application to determine how the author identifier is used.
     * </p>
     * <p>
     ...
     * <code>Review</code> has not yet been set through the constructor or a call to <code>setSubmission</code> (or has
     * been set back to this value by a call to <code>resetSubmission</code>). Positive values indicate that the client
     * using the component has assigned a submission (in the constructor or through the <code>setSubmission</code>
     * method).
     * </p>
     * <p>
     * This field is initialized to -1 (default value), and also can be initialized in the constructor. It is mutable,
     ...
     * </p>
     * <p>
     * The scorecard is only an identifier, this component does not define an abstract <code>Scorecard</code> class, and
     * it is up to the application to determine how the scorecard identifier is used.
     * </p>
     * <p>
     * The default (unassigned value) for this field is -1, which indicates that the scorecard of the
     * <code>Review</code> has not yet been set through the constructor or a call to <code>setScorecard</code> (or has
     * been set back to this value by a call to <code>resetScorecard</code>). Positive values indicate that the client
     * using the component has assigned a scorecard (in the constructor or through the <code>setScorecard</code>
     * method).
     * </p>
     * <p>
     * This field is initialized to -1 (default value), and also can be initialized in the constructor. It is mutable,
     * and can be positive or equal to -1 (default value). This field can be set using the <code>setScorecard</code> and
     * <code>resetScorecard</code> methods and retrieved using the <code>getScorecard</code> method.
     * </p>
     */
     ...
     * non-negative value (and not NaN or POSITIVE_INFINITY). This field can be set using the <code>setScore</code> and
     * <code>resetScore</code> methods and retrieved using the <code>getScore</code> method.
     * </p>
     */
     ...
     * <code>clearItems</code> methods). All items in the list are required to be non-null <code>Item</code> instances,
     * and no duplicates (by reference equality) are allowed. Items are retrieved through the <code>getItem</code> and
     * <code>getAllItems</code> methods.
     * </p>
     */
     ...
     * <p>
     * This field is default initialized to an empty list, and is immutable, although its contents may be changed (use
     * <code>addComment</code>, <code>removeComment</code>, <code>addComments</code>, <code>removeComments</code> and
     * <code>clearComments</code> methods). All items in the list are required to be non-null <code>Comment</code>
     * instances, and no duplicates (by reference equality) are allowed. Comments are retrieved through the
     * <code>getComment</code> and <code>getAllComments</code> methods.
     * </p>
     */
     ...
     * <code>setCreationTimestamp</code>. Non-null values indicate that the client using the component has assigned the
     * creation datetime (through the constructor or a call to <code>setCreationTimestamp</code> method).
     * </p>
     * <p>
     ...
     * <code>Review</code> has not yet been set through a call to <code>setModificationTimestamp</code>. Non-null values
     * indicate that the client using the component has assigned the modification datetime (through a call to
    ...
    /**
     * <p>
     * Initial score associated with the Review. The initial score is the value before any appeals changes the score.
     * The default (unassigned value) for this field is null, which indicates that the initial score of the Review has
     * not yet been set through a call to setInitialScore (or has been set back to this value by a call to
     * resetInitialScore). Non-null values indicate that the client using the component has assigned the initial score
     * (through the setInitialScore method).
     * </p>
     * <p>
     * This field is initialized to null (default value), it is mutable. It can be null (default value) or wrap a
     * non-negative value (and not NaN or POSITIVE_INFINITY). This field can be set using the setInitialScore and
     * resetInitialScore methods and retrieved using the getInitialScore method.
     * </p>
     */
     ...
     * creationTimestamp to the current datetime and leaving all other fields as their default unassigned values (-1 for
     * author, submission and scorecard, null for score, modificationUser and modificationTimestamp).
     * </p>
     * @param id
     *            The identifier for the <code>Review</code>
     * @param creationUser
     ...
     * @throws IllegalArgumentException
     *             if <code>id</code>, <code>author</code>, <code>submission</code> or <code>scorecard</code> is not
     *             positive
     */
     ...
     * @throws IllegalArgumentException
     *             if <code>id</code>, <code>author</code>, <code>submission</code> or <code>scorecard</code> is not
     *             positive, or <code>creationUser</code> is null
     */
     ...
     * <code>Scorecard</code> class, and it is up to the application to determine how the scorecard identifier is used.
     * </p>
     * <p>
    ...
    /**
     * <p>
     * Retrieves the flag which indicates whether the <code>Review</code> has or has not been committed. It is up to the
     * application to define what "committed" means.
     * </p>
    ...
    public void setScore(Float score) {
        if (score != null
                && (score.floatValue() < 0 || Float.isNaN(score.floatValue()) || Float
                        .isInfinite(score.floatValue()))) {
            throw new IllegalArgumentException(
                    "the wrapped value should not be negative, NaN or POSITIVE_INFINITY.");
        }
        this.score = score;
    }
     ...
     * </p>
     * @return The score to associate with the <code>Review</code>
     */
    ...
    /**
     * <p>
     * Sets the initial score associated with this Review. The initialScore may be null, indicating that no initial
     * score is associated with the review. If the value is non-null, the wrapped value must be non-negative (and not
     * NaN or POSITIVE_INFINITY).
     * </p>
     * @param initialScore
     *            The initial score to associate with the Review
     * @throws IllegalArgumentException
     *             If initialScore wraps a negative value, NaN or POSITIVE_INFINITY
     */
    public void setInitialScore(Float initialScore) {
        if (initialScore != null
                && (initialScore.floatValue() < 0 || Float.isNaN(initialScore.floatValue()) || Float
                        .isInfinite(initialScore.floatValue()))) {
            throw new IllegalArgumentException(
                    "the wrapped value should not be negative, NaN or POSITIVE_INFINITY.");
        }
    ...
    /**
     * <p>
     * Gets the initial score associated with this Review. This method may return null, indicating that there is no
     * initial score associated with the review. If the value is non-null, the wrapped value will be non-negative (and
     * not NaN or POSITIVE_INFINITY).
     * </p>
     * @return
     *         The initial score associated with the Review
     */
    ...
    /**
     * <p>
     * Resets the initial score associated with the Review to its undefined value, which is null.
     * </p>
     */
