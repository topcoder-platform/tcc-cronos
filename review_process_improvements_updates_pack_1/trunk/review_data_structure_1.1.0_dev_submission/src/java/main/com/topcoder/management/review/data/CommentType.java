...
public class CommentType implements Serializable {
     ...
     * has not yet been set through the constructor or a call to <code>setId</code> (or has been set back to this value
     * by a call to <code>resetId</code>). Positive values indicate that the client using the component has assigned an
     * id (in the constructor or through the <code>setId</code> method). It is expected that all
     * <code>CommentType</code> instances will have unique ids. This is up to the client using the component to enforce,
     ...
     * <code>CommentType</code> has not yet been set through the constructor or a call to <code>setName</code> (or has
     * been set back to this value by a call to <code>resetName</code>). Non-null values indicate that the client using
     * the component has assigned a name (in the constructor or through the <code>setName</code> method).
     * </p>
     * <p>
     ...
     * This field can be set using the <code>setName</code> and <code>resetName</code> methods and retrieved using the
