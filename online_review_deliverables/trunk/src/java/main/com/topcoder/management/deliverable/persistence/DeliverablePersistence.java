...
/**
 * <p>
 * The DeliverablePersistence interface defines the methods for persisting and retrieving the object model in this
 * component. This interface handles the persistence of the deliverable related classes that make up the object
 * model – this consists only of the Deliverable class. Unlike UploadPersistence, the DeliverablePersistence
 * interface (currently) has no support for adding items. Only retrieval is supported. This interface is not
 * responsible for searching the persistence for the various entities. This is instead handled by a
 * DeliverableManager implementation.
 * </p>
 *
 * <p>
...
public interface DeliverablePersistence {
    /**
     * loadDeliverables: Loads the deliverables associated with the given deliverable id. There may be more than
     ...
     */
    Deliverable[] loadDeliverables(long deliverableId) throws DeliverablePersistenceException;

    /**
     * loadDeliverable: Loads the deliverable associated with the given submission. The deliverable must be a "per
     ...
     */
    Deliverable loadDeliverable(long deliverableId, long submissionId) throws DeliverablePersistenceException;

    /**
     * loadDeliverables: Loads all Deliverables with the given ids from persistence. May return a 0-length array.
     *
     *
     ...
     * @throws DeliverablePersistenceException if there is an error reading the persistence data
     */
    Deliverable[] loadDeliverables(long[] deliverableIds) throws DeliverablePersistenceException;

    /**
     * loadDeliverables: Loads per-submission deliverables that have the given deliverable id and submission id.
     * The two input arrays must have the same length. Only "per-submission" deliverables will be returned.
     *
     *
     * @param deliverableIds The ids of deliverables to load
     ...
     * @throws DeliverablePersistenceException if there is an error reading the persistence data
     */
    Deliverable[] loadDeliverables(long[] deliverableIds, long[] submissionIds)
