...
public class PayByCreditCardAction extends PayContestAction {
    ...
    protected PaymentResult processPayment() throws Exception {
        ...
        if (isStudioCompetition()) {
            ...
            ContestPaymentResult result = getContestServiceFacade().processContestCreditCardPayment(
                null, studioCompetition, creditCardPaymentData);
            ...
            SoftwareCompetition softwareCompetition = getContestServiceFacade().getSoftwareContestByProjectId(
                null, getProjectId());
            SoftwareContestPaymentResult result = getContestServiceFacade().processContestCreditCardSale(
                null, softwareCompetition, creditCardPaymentData);
            return result.getPaymentResult();
        }
    }
     ...
     * <li>If card type is American Express, the card security code must be 4 digits, otherwise, it must be
     * 3 digits</li>
    ...
    public String execute() {
        ...
        if (!getCardType().trim().equalsIgnoreCase("American Express")) {
            ...
            if (getCsc().trim().length() != AMEX_CSC_DIGITS) {
                addFieldError("csc", "For American Express, the card security code must be "
                    + AMEX_CSC_DIGITS + " digits");
            }
        }
     ...
     */
    @FieldExpressionValidator(key = KEY_PREFIX + "cardNumberNotNullOrEmpty",
        fieldName = "cardNumber", expression = "cardNumber != null && cardNumber.trim().length() != 0",
        message = ActionHelper.CANNOT_BE_NULL_OR_EMPTY)
    ...
    /**
     * Setter for the card type. Struts 2 validation is used to check that the argument is either Visa, MasterCard
     * or American Express.
     *
     * @param cardType the card type
     */
    @FieldExpressionValidator(key = KEY_PREFIX + "cardTypeValues",
        fieldName = "cardType",
        expression = "cardType != null && "
            + "(cardType.trim().equalsIgnoreCase(\"Visa\") || cardType.trim().equalsIgnoreCase(\"MasterCard\") || "
            + "cardType.trim().equalsIgnoreCase(\"American Express\"))",
        message = "cardType must be Visa, MasterCard or American Express")
     ...
     */
    @FieldExpressionValidator(key = KEY_PREFIX + "firstNameNotNullOrEmpty",
        fieldName = "firstName", expression = "firstName != null && firstName.trim().length() != 0",
        message = ActionHelper.CANNOT_BE_NULL_OR_EMPTY)
    ...
    /**
     * Setter for the last name of payer. Struts 2 validation is used to check that the argument is not null or
     * empty.
     *
     * @param lastName the last name of payer
     */
    @FieldExpressionValidator(key = KEY_PREFIX + "lastNameNotNullOrEmpty",
        fieldName = "lastName", expression = "lastName != null && lastName.trim().length() != 0",
        message = ActionHelper.CANNOT_BE_NULL_OR_EMPTY)
    ...
    /**
     * Setter for the zip code of payer. Struts 2 validation is used to check that the argument is between
     * 1 and 10 digits inclusive.
     *
     * @param zipCode the zip code of payer
     */
    @FieldExpressionValidator(key = KEY_PREFIX + "zipCodeNotNullOrEmpty",
        fieldName = "zipCode", expression = "zipCode != null && zipCode.trim().length() != 0",
        message = ActionHelper.CANNOT_BE_NULL_OR_EMPTY,
        shortCircuit = true)
    @RegexFieldValidator(key = KEY_PREFIX + "zipCodeValid",
        fieldName = "zipCode", expression = "[0-9]{1,10}",
        message = "zipCode is required and must be between 1 and 10 digits")
    ...
    /**
     * Setter for the address of payer. Struts 2 validation is used to check that the argument is between
     * 1 and 100 digits inclusive.
     *
     * @param address the address of payer
     */
    @FieldExpressionValidator(key = KEY_PREFIX + "addressRange",
        fieldName = "address",
        expression = "address != null && address.trim().length() >= 1 && address.trim().length() <= 100",
        message = ActionHelper.BETWEEN_1_AND_100_CHARS)
    ...
    /**
     * Setter for the city of payer. Struts 2 validation is used to check that the argument is between
     * 1 and 100 characters inclusive.
     *
     * @param city the city of payer
     */
    @FieldExpressionValidator(key = KEY_PREFIX + "cityRange",
        fieldName = "city", expression = "city != null && city.trim().length() >= 1 && city.trim().length() <= 100",
        message = ActionHelper.BETWEEN_1_AND_100_CHARS)
    ...
    /**
     * Setter for the country of payer. Struts 2 validation is used to check that the argument is between
     * 1 and 100 characters inclusive.
     *
     * @param country the country of payer
     */
    @FieldExpressionValidator(key = KEY_PREFIX + "countryRange",
        fieldName = "country",
        expression = "country != null && country.trim().length() >= 1 && country.trim().length() <= 100",
        message = ActionHelper.BETWEEN_1_AND_100_CHARS)
    ...
    /**
     * Setter for the phone of payer. Struts 2 validation is used to check that the argument exactly
     * matches this format: <code>(xxx) xxx-xxxx</code> where <code>x</code> is a digit.
     *
     * @param phone the phone of payer
     */
    @FieldExpressionValidator(key = KEY_PREFIX + "phoneNotNullOrEmpty",
        fieldName = "phone", expression = "phone != null && phone.trim().length() != 0",
        message = ActionHelper.CANNOT_BE_NULL_OR_EMPTY,
        shortCircuit = true)
    @RegexFieldValidator(key = KEY_PREFIX + "phoneValid",
        fieldName = "phone", expression = "^\\([0-9]{3}\\) [0-9]{3}-[0-9]{4}$",
    ...
    /**
     * Setter for the email of payer. Struts 2 validation is used to check that the argument is a valid
     * non empty email address between 1 and 50 characters inclusive.
     *
     * @param email the email of payer
     */
    @FieldExpressionValidator(key = KEY_PREFIX + "emailRange",
        fieldName = "email",
        expression = "email != null && email.trim().length() >= 1 && email.trim().length() <= 50",
        message = "email must not be null or empty and must be <= 50 characters",
        shortCircuit = true)
    @EmailValidator(key = KEY_PREFIX + "emailValid",
        fieldName = "email",
    ...
    /**
     * Setter for the card security code. Struts 2 validation is used to check that the argument contains
     * only digits and has length of 3 or 4.
     *
     * @param csc the card security code
     */
    @FieldExpressionValidator(key = KEY_PREFIX + "cscValid",
        fieldName = "csc",
        expression = "csc != null && csc.trim().matches(\"^[0-9]{3,4}$\")",
        message = "card security code is required and must consist of 3 or 4 digits")
