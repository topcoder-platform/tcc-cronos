...
/**
 * A simple validator that checks that a member has not exceeded the maximum amount of projects one can register for.
 * ProjectServices is used here.
 * <p>
 * This class extends AbstractConfigurableValidator that implements most of the validation methods, so this class only
 * needs to implement the getMessage method.
 * </p>
 * 
 * <p>
 * To provide a good view as the steps are progressing in each method, the <b>Logging Wrapper</b> component is used in
 * each method. To configure this component, the <b>ConfigManager</b> component is used.
 * </p>
 * 
 * <p>
 ...
 * 
 * <p>
 * The DataValidationRegistrationValidator manager, available via the getRegistrationValidator method, provides all
 * necessary managers and services for this validator. It also provides the Log for logging.
 * </p>
 * <p>
 * Thread Safety: This class is mutable and thus is not thread-safe, but the actual properties and managers used to
 * validate are immutable, so this class will be effectively thread-safe in the context of its usage in this component.
 * </p>
 * 
 * @author argolite, TCSDEVELOPER
 * @version 1.0
 */
public class MemberNotExceededMaxProjectRegistrationLimitValidator extends AbstractConfigurableValidator {
    /**
     * <p>
    ...
    /**
     * <p>
     * Name of the property that stores the maximum number of projects the member can be registered for.
     * </p>
     * <p>
     * This property is required. This must can be parsed into a non-negative integer value.
     * </p>
     * 
     */
    ...
    /**
     * <p>
     * Represents the maximum number of projects the member can be registered for.
     * </p>
     * 
     * <p>
    ...
    /**
     * Initializes the resource bundle and the maximum number of projects the member can be registered for based on the
     * input parameters.
     * 
     * @param bundleInfo The resource bundle info
     * @param maxRegistrationCount the maximum number of projects the member can be registered for
     * @throws IllegalArgumentException if any of the parameters in bundleInfo is null or an empty string (for string
     *         parameters)
     * @throws IllegalArgumentException if maxRegistrationCount is negative
     */
    public MemberNotExceededMaxProjectRegistrationLimitValidator(BundleInfo bundleInfo, int maxRegistrationCount) {
        super(bundleInfo);
        RegistrationValidationHelper.validateNotNegative(maxRegistrationCount, "maxRegistrationCount");
        this.maxRegistrationCount = maxRegistrationCount;
    }

    /**
     * Initializes the resource bundle and the maximum number of projects the member can be registered for from
     * configuration information from the <b>ConfigManager</b>.
     * 
     * 
     * @param namespace The configuration namespace
     * @throws IllegalArgumentException If namespace is null/empty
     * @throws RegistrationValidationConfigurationException If any configuration error occurs, such as unknown namespace
     *         or missing required values.
     */
    public MemberNotExceededMaxProjectRegistrationLimitValidator(String namespace) {
        super("temporary");

        RegistrationValidationHelper.validateStringNotNullOrEmpty(namespace, "namespace");

        // Creates BundleInfo from configuration information
        BundleInfo bundleInfo = RegistrationValidationHelper.createBundleInfo(namespace);
        // Sets the BundleInfo
        ...
        this.maxRegistrationCount = RegistrationValidationHelper.getInteger(namespace,
            MAX_REGISTRATION_COUNT_PROPERTYNAME).intValue();

    }

    /**
     * <p>
     * Checks that a member has not exceeded the maximum amount of projects one can register for.
     * </p>
     * 
    ...
    public String getMessage(Object obj) {
        ...
        try {
            ...
            RegistrationValidationHelper.validateIsValidationInfo(obj, "Object to validate");
            ...
            int registrationCount = countProjectRegistration(validationInfo.getRegistration().getUserId(),
                validationInfo.getRegistration().getProjectId(), logger);
            ...
            if (registrationCount > this.maxRegistrationCount) {
                ...
                String data = RegistrationValidationHelper.buildStandInfo(validationInfo);
                data = RegistrationValidationHelper.appendInnerDataValue(data, "MAX_REGISTRATION_COUNT", ""
                    + this.maxRegistrationCount);
                // Log variable value
                RegistrationValidationHelper.log(logger, Level.DEBUG, "the value of data stirng is [" + data + "]");

                message = RegistrationValidationHelper.fillMessage(messageTemplate, data, logger, methodName);
            }

            // Log return value
            RegistrationValidationHelper.log(logger, Level.DEBUG, "return value is [" + message + "]");
            // Log exit
            RegistrationValidationHelper.log(logger, Level.INFO, "exit " + methodName);
            return message;
        } catch (ValidationProcessingException e) {
            RegistrationValidationHelper.log(logger, Level.ERROR, "Error[" + e.getClass().getName() + "] occurs in "
    ...
    /**
     * <p>
     * Counts all projects whose activeProject.resources contains this resource.id.
     * </p>
     * 
     * 
     ...
     * @throws ValidationProcessingException If an unexpected system error occurs
     */
    private int countProjectRegistration(long userId, long projectId, Log logger) {
