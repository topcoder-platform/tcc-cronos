 ...
 *
 * <p>
 * version 1.1 change notes: Two test methods
 * <code>testPerformWithoutSubmission</code> and
 * <code>testPerformWithSubmissions</code> are added to test post-mortem
 * insertion
 * </p>
 *
...
public class SubmissionPhaseHandlerTest extends BaseTest {
    ...
    /**
     * Tests the SubmissionPhaseHandler() constructor and canPerform with
     * Scheduled statuses.
     *
     * @throws Exception not under test.
     */
    public void testCanPerformWithScheduled() throws Exception {
        ...
        try {
            ...
            submission.getAllDependencies()[0].getDependency().setPhaseStatus(
                            PhaseStatus.CLOSED);
    ...
    public void testCanPerformHandlerWithOpen() throws Exception {
        ...
        try {
            ...
            // time has not passed, nor dependencies met
            assertFalse("canPerform should have returned false", handler
                            .canPerform(submissionPhase));

            // time has passed, but dependency not met.
            submissionPhase.setActualStartDate(new Date(System
            ...
            submissionPhase.getAllDependencies()[0].getDependency()
            ...
            Resource submitter = super.createResource(1, submissionPhase
                            .getId(), 1, 1);
            Resource reviewer1 = super
                            .createResource(2, screeningPhaseId, 1, 2);
            Resource reviewer2 = super
                            .createResource(3, screeningPhaseId, 1, 3);

            Upload upload = createUpload(1, 1, submitter.getId(), 1, 1,
                            "parameter");
            Submission submission = createSubmission(1, 1, 1);
            Scorecard scorecard = createScorecard(1, 1, 1, 1, "name", "1.0",
                            75.0f, 100.0f);
            Review review = createReview(1, reviewer1.getId(), submission
                            .getId(), scorecard.getId(), true, 80.0f);

            insertResources(conn, new Resource[] {submitter, reviewer1,
                            reviewer2});
    ...
    /**
     * Tests the perform with Scheduled and Open statuses and checks whether a
     * post-mortem phase is inserted when there is no submission.
     *
    ...
    public void testPerformWithoutSubmission() throws Exception {
        ...
        try {
            ...
            // test with scheduled status.
            Phase submissionPhase = createPhase(1, 1, "Scheduled", 2,
                            "Submission");
    ...
    /**
     * Tests the perform with Open statuses and a post-mortem phase should NOT
     * be inserted when there are submissions.
     *
     * @throws Exception to JUnit.
     *
     * @since 1.1
    ...
    public void testPerformWithSubmissions() throws Exception {
        ...
        try {
            ...
            Upload upload = super.createUpload(1, project.getId(), resource
                            .getId(), 1, 1, "Paramter");
            super.insertUploads(conn, new Upload[] {upload});
            Submission submission = super
                            .createSubmission(1, upload.getId(), 1);
