 ...
 * <li>fix some check-style warning about missing braces in if construct.</li>
 * </ol>
 * </p>
 *
...
public class ReviewRegistrationPhaseHandler extends AbstractPhaseHandler {
    ...
    public ReviewRegistrationPhaseHandler(String namespace) throws ConfigurationException {
        ...
        primaryReviewerResourceId = parseStringToT(16L, namespace, "primaryReviewerResourceId", Long.class)
            .longValue();

        secondReviewerResourceId = parseStringToT(17L, namespace, "secondReviewerResourceId", Long.class)
            .longValue();
    }
    ...
    public OperationCheckResult canPerform(Phase phase) throws PhaseHandlingException {
        ...
        try {
            ...
            OperationCheckResult ocrSuccess;
            if (toStart) {
                // check if the phase can start in terms of dependency
                // check if all parent projects are completed
                ocrSuccess = PhasesHelper.checkPhaseCanStart(phase);
                if (!ocrSuccess.isSuccess()) {
                    return ocrSuccess;
                }
                boolean completed = PhasesHelper.areParentProjectsCompleted(phase,
                    createConnection(), getManagerHelper(), LOG);
                return new OperationCheckResult(completed,
                    completed ? null : "Parent Project not completed");
            }
            ...
            // fully  filled:
            ocrSuccess = PhasesHelper.checkPhaseDependenciesMet(phase, false);
            String errmsg = "Phase dependencies not completed yet";
            if (ocrSuccess.isSuccess()) {
                if (!PhasesHelper.reachedPhaseEndTime(phase)) {
                    errmsg = "Phase not ended yet";
                } else if (!isReviewBoardFilled(phase)) {
                    errmsg = "Review board not fully filled";
                } else {
                    return ocrSuccess;
                }
            }
