 ...
 * <ol>
 * <li>Update {@link #updateScores(Phase, String, Map)} method
 * to fix the bug of setting submission initial score.</li>
 * </ol>
 * </p>
 * <p>
 ...
 * <li>add whitespace after the comma in method perform.</li>
 * <li>fix line longer than 120 in method updateScores.</li>
 * </ol>
 * </p>
 *
...
public class PrimaryReviewEvaluationPhaseHandler extends AbstractPhaseHandler {
    ...
    public OperationCheckResult canPerform(Phase phase) throws PhaseHandlingException {
        ...
        try {
            ...
            OperationCheckResult dependenciesMet = PhasesHelper.checkPhaseDependenciesMet(phase, false);
            ...
            try {
                ...
                Phase reviewPhase = PhasesHelper.locatePhase(phase,
                    PhasesHelper.SECONDARY_REVIEWER_REVIEW, false, false);
                // Search the reviewIds
                Resource[] reviewers = PhasesHelper.searchResourcesForRoleNames(getManagerHelper(),
                    conn, new String[]{PhasesHelper.SECONDARY_REVIEWER_ROLE_NAME}, reviewPhase.getId());
                ...
                Resource[] reviewEvaluators = PhasesHelper.searchResourcesForRoleNames(getManagerHelper(),
                    conn, new String[]{primaryReviewerRoleName}, phase.getId());
                if (reviewEvaluators.length == 0) {
                    errmsg = "no reviewers for project: " + phase.getProject().getId();
                    LOG.log(Level.INFO, errmsg);
                    return new OperationCheckResult(false, errmsg);
                }
                Review[] reviewEvaluations = PhasesHelper.searchReviewsForResourceRoles(conn,
                    getManagerHelper(), phase.getId(), new String[]{primaryReviewerRoleName}, null);
                ...
                if (reviewEvaluations != null && reviewEvaluations.length > 0) {
                    ...
                    errmsg = "Can't end phase. No Evaluation reviews found";
                    LOG.log(Level.ERROR, errmsg);
                    return new OperationCheckResult(false, errmsg);
                }
                // There will be one review evaluation per review
                if (reviews.length != reviewEvaluations.length) {
                    errmsg = "Can't end phase. Not all reviews has been evaluated. There are "
                        + reviews.length + " reviews and only "
                        + reviewEvaluations.length + " evaluation completed";
                    LOG.log(Level.ERROR, errmsg);
                    return new OperationCheckResult(false, errmsg);
                }
                // return true if all dependencies are met and review evaluation is submitted:
                if (dependenciesMet.isSuccess() && allReviewsCommitted) {
    ...
    private void updateScores(Phase phase, String operator, Map<String, Object> values)
        ...
        try {
            ...
            // Search the reviewIds
            Resource[] reviewers = PhasesHelper.searchResourcesForRoleNames(getManagerHelper(),
                conn, new String[]{PhasesHelper.PRIMARY_REVIEW_EVALUATOR_ROLE_NAME}, phase.getId());
