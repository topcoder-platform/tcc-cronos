...
-- this is the missing table (1.6.2)
--
...
INSERT INTO evaluation_type_lu (evaluation_type_id, name, description) VALUES(1, 'name', 'description');
...
CREATE TABLE 'informix'.scorecard_assignment_lu (
...
CREATE TABLE 'informix'.project_scorecard (
  project_id                    INTEGER                         NOT NULL,
  scorecard_id                  INTEGER                         NOT NULL,
  scorecard_assignment_id       INTEGER                         NOT NULL,
  create_user                   VARCHAR(64)                     NOT NULL,
...
-- added 1.6.2 : Missing review_evaluation_id
-- Caused by: com.topcoder.management.review.ReviewPersistenceException: Error occurs while executing query [UPDATE review SET resource_id=?, submission_id=?, scorecard_id=?, committed=?, score=?, initial_score=?,review_evaluation_id=?, modify_user=?, modify_date=CURRENT WHERE review_id=?] using the query arguments [101, 1, 1, 1, 90.0, null, null, System, 11].
-- at com.cronos.onlinereview.phases.AggregationReviewPhaseHandler.approvePendingAggregationReview(AggregationReviewPhaseHandler.java:319)
-- Caused by: java.sql.SQLException: Column (review_evaluation_id) not found in any table in the query (or SLV is undefined).

CREATE TABLE review (
  review_id                     INTEGER                         NOT NULL,
  resource_id                   INTEGER                         NOT NULL,
  submission_id                 INTEGER,
  review_evaluation_id          INTEGER,
  scorecard_id                  INTEGER                         NOT NULL,
...
-- added in 1.6.2 column 'evaluation_type_id'
-- Caused by: com.topcoder.management.review.ReviewPersistenceException: Error occurs while executing query 
-- [SELECT review_comment_id, resource_id, review_id, comment_type_id, 
-- evaluation_type_id, content, extra_info 
-- FROM review_comment
-- WHERE review_id IN (1)
-- ORDER BY review_id, sort] 
-- using the query arguments [].
-- Caused by: java.sql.SQLException: Column (evaluation_type_id) not found in any table in the query (or SLV is undefined).

-- Testcase: testDemoV14_SpecificationReview took 85.86 sec: Caused an ERROR
-- Cannot insert a null into column (review_comment.evaluation_type_id).
-- java.sql.SQLException: Cannot insert a null into column (review_comment.evaluation_type_id).
