...
/**
 * <p>
 * This class implements PhaseHandler interface to provide methods to check if a phase can be executed and to
 * add extra logic to execute a phase. It will be used by Phase Management component. It is configurable using
 * an input namespace. The configurable parameters include database connection and email sending. This class
 * handle the appeals response phase. If the input is of other phase types, PhaseNotSupportedException will be
 * thrown.
 * </p>
 * <p>
 ...
 * <ul>
 * <li>Added capability to support different email template for different role (e.g. Submitter, Reviewer,
 * Manager, etc).</li>
 * <li>Support for more information in the email generated: for start/stop, puts the submissions info/scores
 * into the values map for email generation.</li>
 * </ul>
 * </p>
 * <p>
 ...
 * <ol>
 * <li>Updated {@link #perform(Phase, String)} method to calculate the number of aggregators for project and
...
public class AppealsResponsePhaseHandler extends AbstractPhaseHandler {
     ...
     *             if errors occurred while loading
     ...
     * will be executed. This method will be called by canStart() and canEnd() methods of PhaseManager
     * implementations in Phase Management component.
     * </p>
     * <p>
     * If the input phase status is Scheduled, then it will check if the phase can be started using the
     * following conditions: The dependencies are met
     * </p>
     * <p>
     ...
     *             if there is any error occurred while
    ...
    /**
     * <p>
     * Provides additional logic to execute a phase. This method will be called by start() and end() methods
     * of PhaseManager implementations in Phase Management component. This method can send email to a group os
     * users associated with timeline notification for the project. The email can be send on start phase or
     * end phase base on configuration settings.
     * </p>
     * <p>
     * If the input phase status is Scheduled, then it will do nothing.
     * </p>
     * <p>
     * If the input phase status is Open, then it will perform the following additional logic to stop the
     * phase:
     * </p>
     * <ul>
     * <li>All submissions with failed review scores will be set to the status &quot;Failed Review&quot;.</li>
     * <li>Overall score for the passing submissions will be calculated and saved to the submitters&#146;
     * resource properties together with their placements. The property names are &quot;Final Score&quot; and
     * &quot;Placement&quot;.</li>
     * <li>The winner and runner-up will be populated in the project properties. The property names are
     * &quot;Winner External Reference ID&quot; and &quot;Runner-up External Reference ID&quot;.</li>
     * <li>Submissions that do not win will be set to the status &quot;Completed Without Win&quot;.</li>
     ...
     * </p>
     * <p>
     * Version 1.1. change notes: Add a post-mortem phase after this phase if there is no submission passes
     * review after appeal response.
     * </p>
     * <p>
     * version 1.2. change notes: put the scores of the submissions/submitter handle into the values map. for
     * each submission, there will be a sub-map, all in a list with key 'SUBMITTER'.
     * </p>
     *
     * @param phase
     *            The input phase to check.
     ...
     *             processing the phase.
     * @throws IllegalArgumentException
     *             if the input parameters is null or empty
     *             string.
     */
    public void perform(Phase phase, String operator) throws PhaseHandlingException {
        ...
        if (toStart) {
            ...
            Resource[] aggregators = getAggregators(PhasesHelper
                .locatePhase(phase, "Aggregation", true, true));
     ...
     * </ul>
     * <p>
     * version 1.1. change notes: Change return type from void to boolean, returning true indicates there is
     * at least one submission passes review after appeal response, false otherwise.
     * </p>
     * <p>
     * version 1.2. change notes: put the scores of the submissions/submitter handle into the values map. for
     * each submission, there will be a sub-map, all in a list with key 'SUBMITTER'.
    ...
    private boolean updateSubmissions(Phase phase, String operator, Map < String, Object > values)
        ...
        try {
            ...
            for (Submission submission : subs) {
                ...
                infos.put("SUBMITTER_HANDLE", PhasesHelper.notNullValue(getManagerHelper()
                    .getResourceManager().getResource(submission.getUpload().getOwner()).getProperty(
                        PhasesHelper.HANDLE)));
