 ...
 * both the Screening and Milestone Screening phases. If the input is of other phase types, PhaseNotSupportedException
 * will be thrown.
 * </p>
 * <p>
 ...
 * <li>There are active milestone or contest submissions.</li>
 * </ul>
 ...
 * "Failed Screening" or "Failed Milestone Screening".</li>
...
abstract class BaseScreeningPhaseHandler extends AbstractPhaseHandler {
    ...
    /**
     * Indicates whether the handler is for the "Milestone Screening" phase or for the "Screening" phase.
     */
    final private boolean isMilestone;
     ...
     * @param isMilestone True if the phase type is "Milestone Screening" and false for "Screening" phase type
     * @throws ConfigurationException if errors occurred while loading configuration settings.
     * @throws IllegalArgumentException if the input is null or empty string.
     */
    protected BaseScreeningPhaseHandler(String namespace, boolean isMilestone) throws ConfigurationException {
        super(namespace);
        this.isMilestone = isMilestone;
        
        if (isMilestone) {
            phaseTypeName = Constants.PHASE_MILESTONE_SCREENING;
            submissionTypeName = Constants.SUBMISSION_TYPE_MILESTONE_SUBMISSION;
            screenerRoleName = Constants.ROLE_MILESTONE_SCREENER;
        } else {
     ...
     * <li>The dependencies are met.</li>
     * <li>Phase start date/time is reached (if specified).</li>
     * <li>There active milestone or contest submissions.</li>
     * </ul>
     * </p>
     * <p>
     ...
     * <ul>
     * <li>The dependencies are met</li>
     * <li>All active (miestone) submissions have one screening scorecard committed.</li>
     * </ul>
     * </p>
     * <p>
     ...
     * @throws PhaseNotSupportedException if the input phase type is not "Screening" for non-milestone phase
     *             and "Milestone Screening" for milestone phase.
    ...
    public void perform(Phase phase, String operator) throws PhaseHandlingException {
        ...
        if (toStart) {
            ...
            Review[] screenReviews = PhasesHelper.searchReviewsForResourceRoles(
                getManagerHelper(), phase.getId(), new String[] {screenerRoleName}, null);
            ...
            // For non-milestone screening if there no submission passed screening, insert post-mortem phase
            if (!isMilestone && noScreeningPass) {
    ...
    private boolean processAndUpdateSubmission(Submission submission,
        ...
        try {
            ...
            // set submission status to "Failed Screening" or "Failed Milestone Screening"
            if (screeningScore < minScore) {
                SubmissionStatus subStatus = LookupHelper.getSubmissionStatus(uploadManager,
                    isMilestone ? Constants.SUBMISSION_STATUS_FAILED_MILESTONE_SCREENING : Constants.SUBMISSION_STATUS_FAILED_SCREENING);
