...
/**
 * <p>
 * This abstract class is used as a base class for all phase handlers. This
 * class contains logic in the constructor to load configuration settings for a
 * phase handler. Settings includes database connection, email template and
 ...
 * <li>Property projectDetailsBaseURL added.</li>
 * </ol>
 * </p>
 *
 * @author tuenm, bose_java, pulky, argolite, TCSDEVELOPER
 * @version 1.1
 */
public abstract class AbstractPhaseHandler implements PhaseHandler {
    ...
    /**
     * format for the email timestamp. Will format as
     * "Fri, Jul 28, 2006 01:34 PM EST".
     */
    private static final String EMAIL_TIMESTAMP_FORMAT = "EEE, MMM d, yyyy hh:mm a z";

    /**
     * This constant stores Online Review's project details page url property
     * name
     *
     * @since 1.1
     */
     ...
     * of this project. This field is never null.
     */
    private final DBConnectionFactory factory;

    /**
     * <p>
     * This field is used to retrieve manager instances to use in phase
     * handlers. It is initialized in the constructor and never change after
    ...
    /**
     * Reperesents the connection name used to create connection to the database
     * using DBConnectionFactory. This variable can be null. When it is null,
     * default connection is be created. This variable can be initialized in the
     * constructor and never change after that.
     */
    private final String connectionName;

    /**
     * This flag indicates whether to send start phase email. is initialized in
     * the constructor and never change after that. Default value is false.
     */
    private final boolean sendStartPhaseEmail;

    /**
     * Represents the template source to load email template for sending start
     * phase email. Template sources can be file or database, etc. This field is
     * initialized in the constructor and never change after that. It can be
     * null but not empty. It is null when there is no configuration seting for
     * sending email.
     */
    private final String startEmailTemplateSource;

    /**
     * Represents the template name to load email template for sending start
     * phase email. Template name can be a file name for example. This field is
     * initialized in the constructor and never change after that. It can be
     * null but not empty. It is null when there is no configuration seting for
     * sending email.
     */
    ...
    /**
     * Represents the email subject for sending start phase email. This field is
     * initialized in the constructor and never change after that. It can be
     * null but not empty. It is null when there is no configuration seting for
     * sending email.
     */
    ...
    /**
     * Represents the email address for sending start phase email. This field is
     * initialized in the constructor and never change after that. It can be
     * null but not empty. It is null when there is no configuration seting for
     * sending email.
     */
    ...
    /**
     * This flag indicates whether to send end phase email. is initialized in
     * the constructor and never change after that. Default value is false.
     */
    private final boolean sendEndPhaseEmail;

    /**
     * Represents the template source to load email template for sending end
     * phase email. Template sources can be file or database, etc. This field is
     * initialized in the constructor and never change after that. It can be
     * null but not empty. It is null when there is no configuration seting for
     * sending email.
    ...
    /**
     * Represents the template name to load email template for sending end phase
     * email. Template name can be a file name for example. This field is
     * initialized in the constructor and never change after that. It can be
     * null but not empty. It is null when there is no configuration seting for
     * sending email.
     */
    ...
    /**
     * Represents the email subject for sending end phase email. This field is
     * initialized in the constructor and never change after that. It can be
     * null but not empty. It is null when there is no configuration seting for
     * sending email.
     */
    ...
    /**
     * Represents the email address for sending end phase email. This field is
     * initialized in the constructor and never change after that. It can be
     * null but not empty. It is null when there is no configuration seting for
     * sending email.
     */
    ...
    /**
     * <p>
     * Creates a new instance of AbstractPhaseHandler using the given namespace
     * for loading configuration settings.
     * </p>
     * <p>
     * It initializes the DB connection factory, connection name, Manager
     * Helper, start and end phase email variables from configuration properties
     * </p>
     *
     ...
     */
    protected AbstractPhaseHandler(String namespace)
                    throws ConfigurationException {
        PhasesHelper.checkString(namespace, "namespace");

        // initialize DBConnectionFactory from given namespace, throw exception
        // if property is missing.
        this.factory = PhasesHelper.createDBConnectionFactory(namespace,
                        PROP_CONNECTION_FACTORY_NS);

        // initialize ManagerHelper from given namespace if provided.
        String managerHelperNamespace = PhasesHelper.getPropertyValue(
                        namespace, PROP_MANAGER_HELPER_NAMESPACE, false);
        ...
        String connName = PhasesHelper.getPropertyValue(namespace,
        ...
        // initialize start phase email variables with property values if
        // provided.
        this.sendStartPhaseEmail = PhasesHelper.doesPropertyExist(namespace,
                        PROP_START_PHASE_EMAIL);

        if (this.sendStartPhaseEmail) {
            this.startEmailTemplateSource = PhasesHelper.getPropertyValue(
                            namespace, PROP_START_EMAIL_TEMPLATE_SOURCE, true);
            this.startEmailTemplateName = PhasesHelper.getPropertyValue(
                            namespace, PROP_START_EMAIL_TEMPLATE_NAME, true);
            this.startEmailSubject = PhasesHelper.getPropertyValue(namespace,
                            PROP_START_EMAIL_SUBJECT, true);
            this.startEmailFromAddress = PhasesHelper.getPropertyValue(
                            namespace, PROP_START_EMAIL_FROM_ADDRESS, true);
        } else {
        ...
        // initialize end phase email variables with property values if
        // provided.
        this.sendEndPhaseEmail = PhasesHelper.doesPropertyExist(namespace,
                        PROP_END_PHASE_EMAIL);

        if (this.sendEndPhaseEmail) {
            this.endEmailTemplateSource = PhasesHelper.getPropertyValue(
                            namespace, PROP_END_EMAIL_TEMPLATE_SOURCE, true);
            this.endEmailTemplateName = PhasesHelper.getPropertyValue(
                            namespace, PROP_END_EMAIL_TEMPLATE_NAME, true);
            this.endEmailSubject = PhasesHelper.getPropertyValue(namespace,
                            PROP_END_EMAIL_SUBJECT, true);
            this.endEmailFromAddress = PhasesHelper.getPropertyValue(namespace,
                            PROP_END_EMAIL_FROM_ADDRESS, true);
        } else {
        ...
        this.projectDetailsBaseURL = PhasesHelper.getPropertyValue(
                        managerHelperNamespace, PROP_PROJECT_DETAILS_URL, true);
    }

    /**
     * <p>
     * Sends email at the start/end of a phase. If the phase is in "Scheduled"
     * state, start phase email will be sent. If the phase is in "Open" state,
     * end phase email will be sent. If the phase is in any other state, this
     * method does nothing. Besides, this method will not send any email if it
     * is not configured to do so.
     * </p>
     * <p>
     * This method first retrieves all the ExternalUsers to whom the
     * notification is to be sent as well as the project information. It then
     * instantiates document generator to create the email body content and
     * sends out an email to each user using the Email Engine component.
     * </p>
     *
    ...
    /**
     * <p>
     * This method is used by the subclass to create the connection to access
     * database. The connection needs to be closed after use.
     * </p>
     *
    ...
    /**
     * Sends email at the start/end of a phase. This method first retrieves all
     * the ExternalUsers to whom the notification is to be sent as well as the
     * project information. It then instantiates document generator to create
     * the email body content and sends out an email to each user using the
     * Email Engine component.
     *
     * @param phase phase instance.
     * @param bStart true if phase is to start, false if phase is to end.
     *
     * @throws PhaseHandlingException if there was an error retrieving
     *         information or sending email.
     */
    private void sendEmail(Phase phase, boolean bStart)
        ...
        try {
            ...
            long notificationTypeId = NotificationTypeLookupUtility.lookUpId(
                            conn, NOTIFICATION_TYPE_TIMELINE_NOTIFICATION);

            // retrieve users to be notified
            long[] externalIds = managerHelper.getResourceManager()
                            .getNotifications(projectId, notificationTypeId);
            ...
            throw new PhaseHandlingException(
                            "There was a problem with resource retrieval", ex);
        } catch (RetrievalException ex) {
            throw new PhaseHandlingException(
                            "There was a problem with user retrieval", ex);
        } catch (PersistenceException ex) {
            throw new PhaseHandlingException(
                            "There was a problem with project retrieval", ex);
        } finally {
        ...
        try {
            ...
            if (bStart) {
                template = docGenerator.getTemplate(startEmailTemplateSource,
                                startEmailTemplateName);
            } else {
                template = docGenerator.getTemplate(endEmailTemplateSource,
                                endEmailTemplateName);
            }
            ...
            for (int iUser = 0; iUser < users.length; iUser++) {
                ...
                TemplateFields root = setTemplateFieldValues(docGenerator
                                .getFields(template), user, project, phase,
                                bStart);
                String emailContent = docGenerator.applyTemplate(root);

                TCSEmailMessage message = new TCSEmailMessage();
                message.setSubject((bStart ? startEmailSubject
                                : endEmailSubject)
                                + ": "
                                + (String) project.getProperty(PROJECT_NAME));
                message.setBody(emailContent);
                message.setFromAddress(bStart ? startEmailFromAddress
                                : endEmailFromAddress);
                message.setToAddress(user.getEmail(), TCSEmailMessage.TO);
                EmailEngine.send(message);
            }
        } catch (ConfigManagerException e) {
            throw new PhaseHandlingException("There was a configuration error",
                            e);
            ...
            throw new PhaseHandlingException(
                            "Problem with template data format", e);
    ...
    /**
     * This method sets the values of the template fields with user and project
     * information based on bStart variable which is true if phase is to start,
     * false if phase is to end.
     *
     ...
     */
    private TemplateFields setTemplateFieldValues(TemplateFields root,
                    ExternalUser user, Project project, Phase phase,
                    boolean bStart) {
        ...
        for (int i = 0; i < nodes.length; i++) {
            if (nodes[i] instanceof Field) {
                ...
                if ("PHASE_TIMESTAMP".equals(field.getName())) {
                    ...
                    field.setValue((String) project
                                    .getProperty(PROJECT_VERSION));
                    ...
                    field.setValue("<![CDATA[" + projectDetailsBaseURL
                                    + project.getId() + "]]>");
                }
            }
        }
    ...
    /**
     * Returns the date formatted as "Fri, Jul 28, 2006 01:34 PM EST" for
     * example.
     *
