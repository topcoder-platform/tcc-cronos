...
public class AggregationPhaseHandlerTest extends BaseTest {
    ...
    /**
     * Tests the AggregationPhaseHandler() constructor and canPerform with
     * Scheduled statuses.
     *
     * @throws Exception not under test.
     */
    public void testCanPerformWithScheduled() throws Exception {
        ...
        try {
            ...
            aggregationPhase.getAllDependencies()[0].getDependency()
            ...
            Resource aggregator = createResource(3,
                            aggregationPhase.getId(), project.getId(), 8);
    ...
    /**
     * Tests the AggregationPhaseHandler() constructor and canPerform with Open
     * statuses.
     *
     * @throws Exception not under test.
     */
    ...
    public void testPerformWithScheduled() throws Exception {
        ...
        try {
            ...
            Resource winner = createResource(11, submissionPhase.getId(),
                            project.getId(), 1);
            Upload winnerUpload = createUpload(1, project.getId(), winner
                            .getId(), 1, 1, "parameter");
            Submission winSubmission = createSubmission(1,
                            winnerUpload.getId(), 1);
            winSubmission.setPlacement(new Long(1));

            Resource submitter = createResource(12, submissionPhase.getId(),
                            project.getId(), 1);
            Upload noWinUpload = createUpload(2, project.getId(), submitter
                            .getId(), 1, 1, "parameter");
            Submission noWinSubmission = createSubmission(2, noWinUpload
                            .getId(), 1);
            noWinSubmission.setPlacement(new Long(1));

            // aggregator resource
            Resource aggregator = createResource(1, aggregationPhase.getId(),
                            project.getId(), 8);

            // reviewer resource and related review
            Resource reviewer1 = createResource(2, reviewPhase.getId(), project
                            .getId(), 5);
            Scorecard scorecard1 = createScorecard(1, 1, 2, 1, "name", "1.0",
                            75.0f, 100.0f);
            Review review1 = createReview(111, reviewer1.getId(), winSubmission
                            .getId(), scorecard1.getId(), true, 90.0f);
            review1.addComment(createComment(1, reviewer1.getId(),
                            "Good Design", 1, "Comment"));

            Resource reviewer2 = createResource(3, reviewPhase.getId(), project
                            .getId(), 5);
            Scorecard scorecard2 = createScorecard(2, 1, 2, 1, "name", "1.0",
                            75.0f, 100.0f);
            Review review2 = createReview(112, reviewer2.getId(),
                            noWinSubmission.getId(), scorecard2.getId(), true,
                            80.0f);

            Item[] reviewItems = new Item[4];
            reviewItems[0] = createReviewItem(11, "Answer 1", review1.getId(),
                            1);
            reviewItems[1] = createReviewItem(12, "Answer 2", review1.getId(),
                            1);
            reviewItems[2] = createReviewItem(13, "Answer 3", review2.getId(),
                            1);
            reviewItems[3] = createReviewItem(14, "Answer 4", review2.getId(),
                            1);

            Comment[] reviewItemComments = new Comment[4];
            reviewItemComments[0] = createComment(11, reviewer1.getId(),
                            "Item 1", 1, "Comment");
            reviewItemComments[1] = createComment(12, reviewer1.getId(),
                            "Item 2", 1, "Comment");
            reviewItemComments[2] = createComment(13, reviewer2.getId(),
                            "Item 3", 1, "Comment");
            reviewItemComments[3] = createComment(14, reviewer2.getId(),
                            "Item 4", 1, "Comment");

            Connection conn = getConnection();

            // insert records
            insertResources(conn, new Resource[] {winner, submitter,
                            aggregator, reviewer1, reviewer2});
            insertResourceInfo(conn, winner.getId(), 12, "1"); // winner
            // placement
            // info
            insertUploads(conn, new Upload[] {winnerUpload, noWinUpload});
            insertSubmissions(conn, new Submission[] {winSubmission,
                            noWinSubmission});
            insertResourceSubmission(conn, winner.getId(), winSubmission
                            .getId());
            insertScorecards(conn, new Scorecard[] {scorecard1, scorecard2});
            insertReviews(conn, new Review[] {review1, review2});
            insertComments(conn, new long[] {101, 102}, new long[] {
                            reviewer1.getId(), reviewer2.getId()}, new long[] {
                            review1.getId(), review2.getId()}, new String[] {
                            "comment 1", "comment 2"}, new long[] {1, 3});
            ...
            // insert winner into project info
            insertResourceInfo(conn, winner.getId(), 1, "1001");
            insertProjectInfo(conn, project.getId(), new long[] {23},
                            new String[] {"1001"});

            // check no agg worksheet exists before calling perform method
            Review aggWorksheet = PhasesHelper.getAggregationWorksheet(conn,
                            handler.getManagerHelper(), aggregationPhase
                                            .getId());
            assertNull(
                            "No Aggregation worksheet should exist before this test",
                            aggWorksheet);
