 ...
 *
 * <p>
 * version 1.1 changes note : Fixed test issues according to the new logic and
 * requirement:
...
public class FinalFixPhaseHandlerTest extends BaseTest {
    ...
    /**
     * Tests the FinalFixPhaseHandler() constructor and canPerform with
     * Scheduled statuses.
     *
     * @throws Exception not under test.
     */
    public void testCanPerformWithScheduled() throws Exception {
        ...
        try {
            ...
            finalFixPhase.getAllDependencies()[0].getDependency()
                            .setPhaseStatus(PhaseStatus.CLOSED);
            assertFalse(
                            "canPerform should have returned false when there is no final reviewer",
                            handler.canPerform(finalFixPhase));

            // Let's add a final reviewer here
            Resource finalReviewer = createResource(101, finalFixPhase.getId(),
                            project.getId(), 9);
            ...
            assertFalse(
                            "canPerform should have returned true when final reviewer is assigned.",
                            handler.canPerform(finalFixPhase));
    ...
    /**
     * Tests the FinalFixPhaseHandler() constructor and canPerform with Open
     * statuses.
     *
     * @throws Exception not under test.
     */
    ...
    public void testPerformWithScheduled() throws Exception {
        ...
        try {
            ...
            // aggregator resource
            Resource finalReviewer = createResource(11, finalReviewPhase
                            .getId(), project.getId(), 9);
            Resource aggregator = createResource(1, aggregationPhase.getId(),
                            project.getId(), 8);

            // reviewer resource and related review
            Upload upload1 = createUpload(1, project.getId(), aggregator
                            .getId(), 4, 1, "parameter");
            Submission submission1 = createSubmission(1, upload1.getId(), 1);
            Scorecard scorecard1 = createScorecard(1, 1, 2, 1, "name", "1.0",
                            75.0f, 100.0f);
            Review review1 = createReview(1111, aggregator.getId(), submission1
                            .getId(), scorecard1.getId(), true, 80.0f);
            review1.addComment(createComment(21, aggregator.getId(),
            ...
            reviewItemComments[0] = createComment(11, aggregator.getId(),
                            "Item 1", 1, "Comment");
            reviewItemComments[1] = createComment(12, aggregator.getId(),
                            "Item 2", 1, "Comment");
            ...
            insertComments(conn, new long[] {103},
            ...
            Review finalWorksheet = PhasesHelper.getFinalReviewWorksheet(conn,
                            handler.getManagerHelper(), finalReviewPhase
                                            .getId());
            assertNull("No final worksheet should exist before this test",
                            finalWorksheet);
            ...
            finalWorksheet = PhasesHelper.getFinalReviewWorksheet(conn, handler
                            .getManagerHelper(), finalReviewPhase.getId());
            assertNotNull("Final worksheet should exist after perform()",
                            finalWorksheet);
            assertEquals("review items not copied", finalWorksheet
                            .getAllItems().length, reviewItems.length);
            assertEquals(
                            "review item comments not accepted should not be copied",
                            0,
                            finalWorksheet.getItem(0).getAllComments().length);
