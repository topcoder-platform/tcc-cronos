   ...
   /**
    ...
    public CreateCompetitonResult createCompetition(String requestorHandle, CompetitionData competitionData,
        ...
        for (String supportHandle : supportHandles) {
            ...
            try
            {
                if (!userService.checkUserTerm(user.getUserId(), notusObserverTermsId))
                {
                    
                    throw logError(new LiquidPortalServiceException(
                            "support handle has not agreed Notues Observer Term", LiquidPortalServiceException.EC_INVALID_USER_HANDLES), methodName);
                }
            }
            catch (UserServiceException ue)
            {
                sessionContext.setRollbackOnly();
                throw logError(new LiquidPortalServiceException(
                            "Exception in checking user term "+ ue, LiquidPortalServiceException.EC_INVALID_USER_HANDLES), methodName);
            }
