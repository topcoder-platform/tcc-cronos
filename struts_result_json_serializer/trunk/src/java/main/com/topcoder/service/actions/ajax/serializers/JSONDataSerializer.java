...
import com.topcoder.service.actions.ajax.Helper;

/**
 * <p>
 * This implementation provides a JSON serialization of a Java Object. There are two possible outcomes: a
 * successful result or an error result.
 * </p>
 *
 * <p>
...
public class JSONDataSerializer implements AJAXDataSerializer {
    ...
    /**
     * <p>
     * This some specific data that could be used to dynamically populate the two format templates
     * (jsonResultTemplate and/or jsonErrorTemplate).
     * </p>
     *
     * <p>
     * Can not be set to null but can be left as null (meaning not set). Keys: cannot be null/empty, Values:
     * cannot be null.
     * </p>
     */
    ...
    /**
     * <p>
     * This is a configurable JSON format for the result string coming from this serializer. Can not be set to
    ...
    /**
     * <p>
     * This is a configurable JSON format for the error string coming from this serializer.Can not be set to
     * null or empty string.
     * </p>
     */
    ...
    /**
     * <p>
     * Serialize the given data to string representation(typically a JSON string). We delegate everything to
     * JSON library except for one aspect which is the specialized serialization of the Exception class.
     * </p>
     *
     * @param data
     *            the data to be serialized
     * @param actionName
     *            the name of the action that is serializing the data (null is allowed)
     ...
     *             if there were issues with serialization. NOTE: We do not worry about not replaced or
     *             not-found tokens.
     */
    public String serializeData(String actionName, Object data) throws AJAXDataSerializationException {
        ...
        if (data == null) {
            ...
            jsonReturnString = jsonErrorTemplate.replace(ERROR_TOKEN_REG, error == null ? EMPTY_STRING
                    : error);
        } else {
            // other cases
            try {
        ...
        if (formatData != null) {
            ...
            for (Entry<String, String> entry : formatData.entrySet()) {
                jsonReturnString = jsonReturnString.replace("$[" + entry.getKey() + "]", entry
                        .getValue());
            }
        }
        // if the actionName is null, empty string is used
        jsonReturnString = jsonReturnString.replace(ACTION_TOKEN_REG, actionName == null ? EMPTY_STRING
                : actionName);
     ...
     *
     * @param data
     *            data to be used with the formatted templates. It contains keys as substitution tokens (to be
     *            matched in the templates) and the values are the actual values to be inserted into the
     *            formatted string.
     *
     * @throws IllegalArgumentException
     *             If given data is null, or the keys contain null/empty element, or the values contain null
     */
     ...
     *            The JSON template string to be used
     * @throws IllegalArgumentException
     *             If given input is null/empty
     */
