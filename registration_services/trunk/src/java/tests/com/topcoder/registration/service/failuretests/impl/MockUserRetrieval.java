...
/**
 * The mock class for <code>UserRetrieval</code>.
 *
 * @author liulike
 * @version 1.0
 */
public class MockUserRetrieval implements UserRetrieval {
    ...
    public ExternalUser retrieveUser(long id) throws RetrievalException {
        return new ExternalUserImpl(1, "liulike", "f", "l", "liulike@tc");
    }
     ...
     * @return an array of external users who have the given ids. If none of the given ids were found, an
     *         empty array will be returned. The index of the entries in the array will not necessarily
     ...
     * @return an array of external users who have the given handles. If none of the given handles were found,
     *         an empty array will be returned. The entries in the array will not necessarily directly
     *         correspond to the entries in the handles array.
     * @throws IllegalArgumentException if handles is <code>null</code> or any entry is <code>null</code>,
     *         or empty after trim.
     * @throws RetrievalException if any exception occurred during processing; it will wrap the underlying
     *         exception.
     */
     ...
     * </p>
     *
     * @param handles the handles of the users we are interested in finding by the lower-case version of their
     *        handle.
     * @return an array of external users who have the given handles. If none of the given handles were found,
     *         an empty array will be returned. The entries in the array will not necessarily directly
     *         correspond to the entries in the handles array.
     ...
     * </p>
     *
     * @param firstName the first name of the user(s) to find, or the empty string to represent "any first
     *        name". Both firstName and lastName cannot be empty.
     * @param lastName the last name of the user(s) to find, or the empty string to represent "any last name".
     *        Both firstName and lastName cannot be empty.
     * @return an array of external users whose first name and last name start with the given first and last
     *         name. If no users match, an empty array will be returned.
     * @throws IllegalArgumentException if firstName or lastName is <code>null</code>, or if BOTH are empty
     *         after trim. (I.e., if one is empty after trim, and the other is not empty after trim, this is
     *         not an exception.)
     * @throws RetrievalException if any exception occurred during processing; it will wrap the underlying
     *         exception.
     */
