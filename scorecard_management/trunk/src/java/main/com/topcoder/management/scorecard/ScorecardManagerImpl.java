...
public class ScorecardManagerImpl implements ScorecardManager {
    ...
    public ScorecardManagerImpl(String ns) throws ConfigurationException {
        ...
        try {
            ...
            persistence = (ScorecardPersistence) createObjectWithNamespace(cm.getString(ns, PERSISTENCE_CLASS), cm
                .getString(ns, PERSISTENCE_NAMESPACE), "ScorecardPersistence");

            // Create an instance from the class name and the namespace for validator.
            validator = (ScorecardValidator) createObjectWithNamespace(cm.getString(ns, VALIDATOR_CLASS), cm
                .getString(ns, VALIDATOR_NAMESPACE), "ScorecardValidator");

            // Create a SearchBundleManager using the fetched SearchBuidlerNamespace.
            SearchBundleManager manager;
            try {
                manager = new SearchBundleManager(cm.getString(ns, SEARCHBUILDER_NAMESPACE));
            } catch (SearchBuilderConfigurationException e) {
                throw new ConfigurationException("error while linking the SearchBundleManager", e);
            } catch (NullPointerException e) {
                throw new ConfigurationException("namespace property for SearchBundleManager cannot be found", e);
            }

            // Get the searchBundle from the manager.
            searchBundle = manager.getSearchBundle("ScorecardSearchBundle");
            if (searchBundle == null) {
                throw new ConfigurationException("searchBundle cannot be built correctly");
            }
            ...
            validationMap.put(ScorecardSearchBundle.SCORECARD_STATUS_NAME, StringValidator.hasLength(IntegerValidator
                .lessThanOrEqualTo(SCORECARD_STATUS_NAME_MAXLENGTH)));
            validationMap.put(ScorecardSearchBundle.SCORECARD_TYPE_NAME, StringValidator.hasLength(IntegerValidator
                .lessThanOrEqualTo(SCORECARD_TYPE_NAME_MAXLENGTH)));
            validationMap.put(ScorecardSearchBundle.SCORECARD_NAME, StringValidator.hasLength(IntegerValidator
                .lessThanOrEqualTo(SCORECARD_NAME_MAXLENGTH)));
            validationMap.put(ScorecardSearchBundle.SCORECARD_VERSION, StringValidator.hasLength(IntegerValidator
                .lessThanOrEqualTo(SCORECARD_VERSION_MAXLENGTH)));

            // Set the SearchableFields with this validationMap.
            searchBundle.setSearchableFields(validationMap);

        } catch (UnknownNamespaceException e) {
            throw new ConfigurationException("namespace '" + ns + "' is unknown.", e);
        } catch (IllegalArgumentException e) {
            throw new ConfigurationException("the configuration values are invalid.", e);
     ...
     */
    private Object createObjectWithNamespace(String className, String namespace, String objectName)
        throws ConfigurationException {
        ...
        try {
            ...
            return klass.getConstructor(new Class[] {String.class}).newInstance(new Object[] {namespace});
        } catch (ClassNotFoundException e) {
            throw new ConfigurationException("unable to create the " + objectName + " object.", e);
    ...
    public Scorecard[] searchScorecards(Filter filter, boolean complete) throws PersistenceException {
        ...
        try {
            ...
            // Fetch items from the CustomResultSet result, and store them into ids.
            int i = 0;
            while (result.next()) {
                try {
                    ids[i++] = result.getLong(1);
                } catch (InvalidCursorStateException icse) {
                    // We checked the result.next() already, but still received InvalidCursorStateException. Something
                    // must be wrong with the CustomResultSet.
                    throw new PersistenceException(
                        "result.next() returns true but we cannot fetch any element from result", icse);
                }
            }
