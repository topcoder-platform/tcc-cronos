...
public class PersistenceResourceManager implements ResourceManager {
    ...
    public Resource[] searchResources(Filter filter) throws ResourcePersistenceException,
        ...
        // retrieve the ids from the first column of the resultSet
        long[] resourceIds = retrieveIds(resultSet);

        // load the resources from the ids
    ...
    public ResourceRole[] getAllResourceRoles() throws ResourcePersistenceException {
        ...
        Filter filter = SearchBundle.buildGreaterThanOrEqualToFilter(
                ResourceRoleFilterBuilder.RESOURCE_ROLE_ID_FIELD_NAME, new Long(1));
    ...
    public ResourceRole[] searchResourceRoles(Filter filter)
        ...
        // load the resource roles from the ids
    ...
    public long[] getNotifications(long project, long notificationType) throws ResourcePersistenceException {
        ...
        Filter andFilter = SearchBundle.buildAndFilter(filter1, filter2);
    ...
    public Notification[] searchNotifications(Filter filter)
        ...
        // the number of the total records
        int totRecord = resultSet.getRecordCount();

        // create the arrays to store the answers
        long[] userIds = new long[totRecord];

        long[] projectIds = new long[totRecord];

        long[] notificationTypeIds = new long[totRecord];

        int tot = 0;

        // iterate through to get the answer
        while (resultSet.next()) {
            try {
                // get the user id
                userIds[tot] = resultSet.getInt(INDEX1);

                // get the project id
                projectIds[tot] = resultSet.getInt(INDEX2);

                // get the notification type id
                notificationTypeIds[tot++] = resultSet.getInt(INDEX3);

            } catch (ClassCastException e) {
                // wrap it to SearchBuilderConfigurationException
                throw new SearchBuilderConfigurationException("The results should be of the type: long or int", e);
            } catch (InvalidCursorStateException e) {
                // wrap it to SearchBuilderConfigurationException
                throw new SearchBuilderConfigurationException("Should contains 3 columns.", e);
            } catch (NullPointerException e) {
                // wrap it to SearchBuilderConfigurationException
                // This is caused by the CustomResultSet class, when the columnIndex > getCoumnCount
                // null will returned to getObject().
                // if the search bundle contains only two selected column, like:
                // SELECT external_ref_id, notification_type_id FROM notification WHERE
                // this exception would be captured.
                throw new SearchBuilderConfigurationException("Should contains 3 columns.", e);
            }
        }

        // Improvement:
        // We use loadNotifications instead of a loop of loadNotification, because
        // according to the Component Specification, loadNotifications is much more efficient
        // than calling loadNotification for many times.
        // Because loadNotifications hits the database only once.
        return persistence.loadNotifications(userIds, projectIds, notificationTypeIds);
    }
    ...
    public NotificationType[] searchNotificationTypes(Filter filter)
        ...
        // retrieve the ids from the first column of the resultSet
        long[] notificaionTypeIds = retrieveIds(resultSet);

        // load the resources from the ids
        return persistence.loadNotificationTypes(notificaionTypeIds);
    }

    /**
     * Retrieve all the unique ids from first column of the given <code>CustomResultSet</code>.
     *
     * @param resultSet the resultSet to retrieve id from
     * @return an array of ids.
     * @throws SearchBuilderConfigurationException if it fails to retrieve id.
     */
    private long[] retrieveIds(CustomResultSet resultSet) throws SearchBuilderConfigurationException {
        // Should contain only one column (with long ids)
        // Use a set to remove duplicates
        Set ids = new HashSet();
        while (resultSet.next()) {
            try {
                // according to the schema of the database, the result is of type: Integer
                // wrap it to long
                ids.add(new Long(resultSet.getInt(INDEX1)));

            } catch (ClassCastException e) {
                // wrap it to SearchBuilderConfigurationException
                throw new SearchBuilderConfigurationException("The results should be of the type: long or int", e);
            } catch (InvalidCursorStateException e) {
                // never happen, just wrap it for consistency
                throw new SearchBuilderConfigurationException("The cursor is in invalid state.", e);
            }
        }

        // transform the set to the array of long
        long[] idsArray = new long[ids.size()];

        int tot = 0;

        // iterator through the set to get the ids
        for (Iterator it = ids.iterator(); it.hasNext();) {
            idsArray[tot++] = ((Long) it.next()).longValue();
        }
        return idsArray;
    }
    ...
    public NotificationType[] getAllNotificationTypes() throws ResourcePersistenceException {
        ...
        Filter filter = SearchBundle.buildGreaterThanOrEqualToFilter(
                NotificationTypeFilterBuilder.NOTIFICATION_TYPE_ID_FIELD_NAME, new Long(1));
