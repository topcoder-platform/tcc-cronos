...
public final class TestHelper {
    ...
    public static void sortDataObjects(List list) {
        for (int i = 0; i < list.size(); ++i) {
            for (int j = 0; j < i; ++j) {
                if (((CommonInfo) list.get(i)).getId() < ((CommonInfo) list.get(j)).getId()) {
     ...
     */
    public static void assertEquals(CommonInfo expected, CommonInfo actual) {
    ...
    public static void assertEquals(ExpenseEntry expected, ExpenseEntry actual) {
        assertEquals((CommonInfo) expected, (CommonInfo) actual);
