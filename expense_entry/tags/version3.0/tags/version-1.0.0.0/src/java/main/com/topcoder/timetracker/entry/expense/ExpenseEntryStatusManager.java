 ...
 * the expense entry status, if the ID is unspecified (-1), a random ID is generated. The creation date and modification
 * date are modified and persisted automatically according to the current time. The manager uses
...
public class ExpenseEntryStatusManager {
     ...
     * 'connection_producer_name' represents the connection producer name in the persistence class to create the default
     * database connection from DB connection factory. If it is <code>null</code>, there is no default database
     * connection. The namespace to create DB connection factory is defined in property 'connection_factory_namespace'.
     * It is mandatory, and cannot be empty string.
     * </p>
     *
