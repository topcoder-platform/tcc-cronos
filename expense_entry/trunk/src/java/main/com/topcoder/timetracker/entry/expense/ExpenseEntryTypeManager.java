...
/**
 * <p>
 * Defines functionality to manage expense entry types in persistence. The features include adding, updating,
 * deleting and retrieving a single expense entry type or all expense entry types in the persistence. When adding
 * the expense entry type, if the ID is unspecified (-1), a random ID is generated. The creation date and
...
public class ExpenseEntryTypeManager {
    ...
    /**
     * <p>
     * Creates a new instance of <code>ExpenseEntryTypeManager</code> class. The persistence class name is loaded
     * from the configuration property 'entry_type_persistence_class' under the given namespace. It is mandatory and
     * must be an implenentation of <code>ExpenseEntryTypePersistence</code>. Optional property
    ...
    public ExpenseEntryTypeManager(String namespace) throws ConfigurationException {
        ...
        typePersistence = (ExpenseEntryTypePersistence) ExpenseEntryHelper.createInstance(className,
                connProducerName, dbNamespace, ExpenseEntryTypePersistence.class);
    }
     ...
     * fields in the instance must be set except creation date and modification date, which must be
     * <code>null</code> and be set as the current time.
     * </p>
     *
