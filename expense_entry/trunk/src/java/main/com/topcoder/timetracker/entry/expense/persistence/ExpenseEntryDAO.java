...
/**
 * <p>
 * This interface defines the contract for the complete persistence management of an expense entry. It provides single
 * and batch CRUD operations with the choice to perform audits on all writeable operations. Furthermore, all batch
 * operations support selective operations, so these calls can be either atomic or non-atomic. Atomic mode means that
 * a failure on one entry causes the entire operation to fail. Non-atomic means that a failure in one entry doesn't
 * affect the other and the user has a way to know which ones failed. It also supports robust searches. It has one
 * implementation in this design: <code>InformixExpenseEntryDAO</code>.
 * </p>
 *
 * <p>
