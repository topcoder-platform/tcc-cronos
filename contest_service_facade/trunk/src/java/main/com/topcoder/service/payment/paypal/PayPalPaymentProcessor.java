...
/**
 * <p>
 * This implementation class processes the payment (<code>PaymentData</code>) through PayPal SOAP APIs
 * </p>
 * 
 * @author TCSDEVELOPER
 */
public class PayPalPaymentProcessor implements PaymentProcessor {
    ...
    public PayPalPaymentProcessor(String apiUserName, String apiPassword, String apiSignature, String apiEnvironment)
        ...
        try {
            ...
            Logger.getLogger(this.getClass()).debug("PayPal API Password: " + apiPassword);
            Logger.getLogger(this.getClass()).debug("PayPal API Signature: " + apiSignature);
            Logger.getLogger(this.getClass()).debug("PayPal API Environment: " + apiEnvironment);
    ...
    /**
     * <p>
     * An implementation method of the <code>PaymentProcessor</code> interface. This method simply delegates the
     * responsibility to internal doDirectPayment(..) method. On successful payment, internal method returns PayPal
     * transaction id. This method wraps the transaction id in PaymentResult and return it to the caller.
     * </p>
     * 
     * @param payment
     *            the payment data that need to be processed.
     * @return a <code>PaymentResult</code> the payment result. In this case transaction id from paypal.
     * @throws PaymentException
    ...
    public PaymentResult process(PaymentData payment) throws PaymentException {
        try {
            ...
            throw new PaymentException("Error in processing payment through PayPalPaymentProcessor", e);
        }
    }

    /**
     * <p>
     * This internal method processes the given <code>CreditCardPaymentData</code> through PayPal SOAP SDK API.
     * Implementation simply sets various bean properties, as required by DoDirectPayment PayPal API and then calls the
     * API.
     * </p>
     * 
     * @param paymentData
     *            the given <code>CreditCardPaymentData</code> that need to be processed.
     ...
     *             on payment failure, it captures Error message and Error code for the failure case.
     */
    private String doDirectPayment(CreditCardPaymentData paymentData) throws PayPalException, PaymentException {
        ...
        DoDirectPaymentResponseType response = (DoDirectPaymentResponseType) paypalServicesCaller.call(
        ...
        if (response.getAck().equals(AckCodeType.Success) || response.getAck().equals(AckCodeType.SuccessWithWarning)) {
            ...
            // the exception.
            throw new PaymentException(response.getErrors()[0].getErrorCode().toString(), response.getErrors()[0]
                    .getLongMessage());
        }
    }
}
